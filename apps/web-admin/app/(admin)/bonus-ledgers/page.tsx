...
export default function BonusLedgersPage() {
  ...
  async function load() {
    ...
    try {
      ...
      if (!res.ok) { setMessage(safeMessage(data?.message, 'โหลดบัญชีโบนัสไม่สำเร็จ')); return; }
      setItems(Array.isArray(data?.items) ? data.items : []);
      setMessage('');
    } catch {
      setMessage('เชื่อมต่อระบบบัญชีโบนัสไม่สำเร็จ กรุณาลองใหม่');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function addProgress(id: string) {
    ...
    setBusyId(id);
    setMessage('กำลังบันทึกยอดทำรายการ...');
    try {
      ...
      const data = await res.json().catch(() => null);
      if (!res.ok) { setMessage(safeMessage(data?.message, 'อัปเดตยอดทำรายการไม่สำเร็จ')); return; }
      ...
      setMessage('เชื่อมต่อระบบอัปเดตยอดทำรายการไม่สำเร็จ กรุณาลองใหม่');
    } finally {
      setBusyId('');
    }
  }
  ...
  return <AdminPage eyebrow="งานโบนัส" title="บัญชีโบนัส" description="ติดตามยอดโบนัส ยอดทำรายการ และสถานะการจ่ายเข้ากระเป๋าเงินของสมาชิก" actions={<AdminButton disabled={loading || Boolean(busyId)} onClick={() => void load()}>{loading ? 'กำลังโหลด...' : 'รีเฟรช'}</AdminButton>}>
    ...
    <AdminToolbar><select aria-label="กรองบัญชีโบนัสตามสถานะ" disabled={loading || Boolean(busyId)} value={status} onChange={(event) => setStatus(event.target.value)} style={selectStyle}><option value="ALL">ทุกสถานะ</option><option value="OPEN">กำลังใช้งาน</option><option value="REVIEWING">กำลังตรวจ</option><option value="RESOLVED">ครบเงื่อนไขหรือจ่ายแล้ว</option><option value="DISMISSED">หมดอายุหรือยกเลิก</option></select></AdminToolbar>
    {loading && items.length === 0 ? <AdminSkeleton lines={6} /> : <AdminGrid>{items.map((item) => <AdminCard key={item.id} title={item.campaign?.title ?? item.campaignId} description={`${memberLabel(item)} · ${new Date(item.createdAt).toLocaleString('th-TH')}`} tone={cardTone(item)}><AdminStack>
      ...
      <div style={progressOuterStyle} aria-label={`ยอดทำรายการ ${progressPercent(item)} เปอร์เซ็นต์`} role="progressbar" aria-valuemin={0} aria-valuemax={100} aria-valuenow={progressPercent(item)}><div style={{ ...progressInnerStyle, width: `${progressPercent(item)}%` }} /></div>
      <section aria-label="ประวัติบัญชีโบนัส" style={timelineStyle}>{(item.events ?? []).slice(-5).map((event, index) => <div key={`${event.createdAt}-${index}`} style={eventStyle}><AdminBadge tone={event.by === 'admin' ? 'success' : event.by === 'member' ? 'warning' : 'neutral'}>{senderLabel(event.by)}</AdminBadge><strong>{eventActionLabel(event.action)}</strong><span>{event.amount ? money(event.amount) : event.message || '-'}</span><small>{new Date(event.createdAt).toLocaleString('th-TH')}</small></div>)}</section>
      {!isClosed(item) && item.lifecycleStatus !== 'SETTLED' && <input aria-label={`หมายเหตุสำหรับ ${memberLabel(item)}`} disabled={Boolean(busyId)} value={notes[item.id] ?? ''} onChange={(event) => setNotes((current) => ({ ...current, [item.id]: event.target.value }))} placeholder="หมายเหตุหรือเหตุผล" style={inputStyle} />}
      {!item.turnoverCompleted && !isClosed(item) && <div style={formGridStyle}><input aria-label={`เพิ่มยอดทำรายการสำหรับ ${memberLabel(item)}`} disabled={Boolean(busyId)} type="number" value={amounts[item.id] ?? ''} onChange={(event) => setAmounts((current) => ({ ...current, [item.id]: event.target.value }))} placeholder="เพิ่มยอดทำรายการ" style={inputStyle} /><AdminButton disabled={Boolean(busyId)} onClick={() => void addProgress(item.id)}>{busyId === item.id ? 'กำลังบันทึก...' : 'บันทึกยอดเพิ่ม'}</AdminButton></div>}
    ...
    </AdminStack></AdminCard>)}{!loading && items.length === 0 && <AdminEmpty>ยังไม่มีบัญชีโบนัส</AdminEmpty>}</AdminGrid>}
...
function badgeTone(status: string): 'success' | 'danger' | 'warning' { if (status === 'RELEASE_READY' || status === 'TURNOVER_COMPLETED' || status === 'COMPLETED' || status === 'SETTLED') return 'success'; if (status === 'EXPIRED' || status === 'REVOKED') return 'danger'; return 'warning'; }
function cardTone(item: BonusLedger): 'success' | 'danger' | 'warning' { if (isClosed(item)) return 'danger'; if (item.lifecycleStatus === 'SETTLED' || item.walletCreditStatus === 'CREDITED' || item.turnoverCompleted) return 'success'; return 'warning'; }
...
function safeMessage(value: unknown, fallback: string) { const text = String(value ?? '').trim(); return text && text.length <= 180 ? text : fallback; }
