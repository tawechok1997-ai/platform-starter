...
export function AdminConfirmDialog({ open, tone = 'warning', title, description, confirmLabel, cancelLabel = 'ยกเลิก', details, loading, onConfirm, onCancel }: ConfirmDialogProps) {
  if (!open) return null;
  return <div style={overlayStyle} role="presentation" onClick={onCancel}>
    <section style={dialogStyle} role="dialog" aria-modal="true" aria-labelledby="admin-confirm-title" onClick={(event) => event.stopPropagation()}>
      <div style={headerStyle}>
        <span style={{ ...iconStyle, ...iconToneStyle[tone] }}>{tone === 'success' ? '✓' : tone === 'danger' ? '!' : '?'}</span>
        <div style={{ minWidth: 0 }}>
          <h2 id="admin-confirm-title" style={titleStyle}>{title}</h2>
...
const dialogStyle = { width: 'min(100%, 520px)', border: '1px solid rgba(148,163,184,.22)', borderRadius: 22, background: '#111a24', color: '#f8fafc', padding: 18, boxShadow: '0 30px 90px rgba(0,0,0,.5)', display: 'grid', gap: 14, overflow: 'hidden' as const };
