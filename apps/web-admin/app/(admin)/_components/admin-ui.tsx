...
export function AdminCard({ title, description, action, children, tone = 'neutral' }: CardProps) {
  return <section className={`admin-ui-card admin-ui-surface--${tone}`} style={safeTextContainerStyle}>{(title || description || action) && <header className="admin-ui-card__head" style={safeTextContainerStyle}><div style={safeTextContainerStyle}>{title && <h2>{title}</h2>}{description && <p>{description}</p>}</div>{action && <div className="admin-ui-card__action" style={safeTextContainerStyle}>{action}</div>}</header>}<div className="admin-ui-stack" style={cardBodyStyle}>{children}</div></section>;
}
