...
export default function AdminRolesPage() {
  ...
  return <AdminPage eyebrow="Security" title="Roles & Permissions" description="ตรวจโครงสร้าง Role และ Permission โดยไม่ปะปนกับหน้าบัญชีผู้ดูแล">
    ...
    <AdminDrawer open={Boolean(previewPermission)} title={previewPermission?.code ?? 'Permission preview'} description={previewPermission ? `${previewPermission.module} · ${previewPermission.name}` : undefined} closeLabel="ปิด" size="compact" onClose={() => setPreviewPermission(null)}>
      {previewPermission && <AdminStack><div style={detailStyle}><span>ชื่อ</span><strong>{previewPermission.name}</strong></div><div style={detailStyle}><span>โมดูล</span><strong>{previewPermission.module}</strong></div><div style={detailStyle}><span>คำอธิบาย</span><strong>{previewPermission.description || 'ไม่มีคำอธิบาย'}</strong></div><AdminNotice tone="warning">หน้านี้เป็น read-only preview ยังไม่มีการแก้ไขสิทธิ์หรือบันทึก Role</AdminNotice></AdminStack>}
    </AdminDrawer>
  </AdminPage>;
}
