...
export default function ReconciliationCenterPage() {
  ...
  async function load() { setLoading(true); showMessage('กำลังโหลดการตรวจยอด...', 'neutral'); const res = await adminApiFetch('/admin/provider-wallet-snapshots'); const data = await res.json().catch(() => null); setLoading(false); if (!res.ok) { showMessage(data?.message ?? 'โหลดการตรวจยอดไม่สำเร็จ', 'danger'); return; } setPayload(data ?? {}); showMessage(''); }
  async function runReconcile() { if (!sessionId.trim()) { showMessage('กรอก Game Session ID ก่อน', 'warning'); return; } setLoading(true); showMessage('กำลังตรวจยอด...', 'neutral'); const res = await adminApiFetch(`/admin/game-sessions/${sessionId.trim()}/reconcile`, { method: 'POST' }); const data = await res.json().catch(() => null); setLoading(false); if (!res.ok) { showMessage(data?.message ?? 'ตรวจยอดไม่สำเร็จ', 'danger'); return; } showMessage(data.ok ? 'ยอดตรงกัน' : `ยอดยังไม่ตรง · ส่วนต่าง ${data.snapshot?.difference ?? '-'}`, data.ok ? 'success' : 'danger'); setSessionId(''); await load(); }
  async function reviewSnapshot(item: Snapshot, status: 'REVIEWING' | 'RESOLVED') { const note = window.prompt(status === 'RESOLVED' ? 'หมายเหตุการแก้ปัญหา' : 'หมายเหตุตรวจสอบ'); if (!note) return; setLoading(true); showMessage('กำลังบันทึกสถานะการตรวจ...', 'neutral'); const res = await adminApiFetch(`/admin/provider-wallet-snapshots/${item.id}/review`, { method: 'PATCH', body: JSON.stringify({ note, status }) }); const data = await res.json().catch(() => null); setLoading(false); if (!res.ok) { showMessage(data?.message ?? 'บันทึกไม่สำเร็จ', 'danger'); return; } showMessage(status === 'RESOLVED' ? 'ปิดเคสยอดไม่ตรงแล้ว' : 'บันทึกว่ากำลังตรวจแล้ว', 'success'); await load(); }
