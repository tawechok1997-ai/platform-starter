...
export default function ExportsPage() {
  ...
  useEffect(() => { if (jobs.length) localStorage.setItem(STORAGE_KEY, JSON.stringify(jobs.slice(0, 100))); }, [jobs]);
  ...
  function retry(job: ExportJob) { const source = exports.find((item) => item.path === job.path); if (source) void download(source); }
  ...
  return <AdminPage eyebrow="Data Governance" title="Export Center" description="สร้างไฟล์ ติดตามสถานะ ดูประวัติ และดาวน์โหลดข้อมูลตามสิทธิ์ของผู้ดูแล">
    ...
    <AdminGrid>{exports.map((item) => <AdminCard key={item.path} title={item.title} description={`${item.text} · ${item.fileType}`}><AdminButton onClick={() => void download(item)}>สร้างและดาวน์โหลด</AdminButton></AdminCard>)}</AdminGrid>
    ...
    <AdminCard title="ประวัติการส่งออก" description="แสดง 100 งานล่าสุดบนอุปกรณ์นี้">
      <div className="admin-data-table-wrap"><table className="admin-data-table"><thead><tr><th>ไฟล์</th><th>ชนิด</th><th>สถานะ</th><th>จำนวนแถว</th><th>เริ่มเมื่อ</th><th>เสร็จเมื่อ</th><th>การทำงาน</th></tr></thead><tbody>{visibleJobs.map((job) => <tr key={job.id}><td>{job.title}</td><td>CSV</td><td><AdminBadge tone={job.status === 'COMPLETED' ? 'success' : job.status === 'FAILED' ? 'danger' : 'warning'}>{job.status}</AdminBadge></td><td>{job.rows?.toLocaleString('th-TH') ?? '-'}</td><td>{new Date(job.createdAt).toLocaleString('th-TH')}</td><td>{job.completedAt ? new Date(job.completedAt).toLocaleString('th-TH') : '-'}</td><td>{job.status === 'FAILED' ? <AdminButton size="compact" tone="secondary" onClick={() => retry(job)}>ลองใหม่</AdminButton> : '-'}</td></tr>)}{jobs.length === 0 && <tr><td colSpan={7}>ยังไม่มีประวัติการส่งออก</td></tr>}</tbody></table></div>{jobs.length > PAGE_SIZE && <div style={paginationStyle}><AdminButton size="compact" tone="secondary" disabled={page <= 1} onClick={() => setPage((value) => value - 1)}>ก่อนหน้า</AdminButton><span>หน้า {page}/{totalPages}</span><AdminButton size="compact" tone="secondary" disabled={page >= totalPages} onClick={() => setPage((value) => value + 1)}>ถัดไป</AdminButton></div>}
    </AdminCard>
  </AdminPage>;
}
const paginationStyle = { display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 8, marginTop: 12 } as const;
