...
import { AdminTechnicalDetails } from '../_components/admin-technical-details';
import { AdminBadge, AdminButton, AdminCard, AdminCode, AdminDataValue, AdminEmpty, AdminFilterBar, AdminMetric, AdminMetricGrid, AdminNotice, AdminPage, AdminPagination, AdminPayloadViewer, AdminRow, AdminSkeleton, AdminStack, AdminToolbar } from '../_components/admin-ui';
...
type NoticeState = { text: string; tone: 'neutral' | 'warning' | 'danger' };
...
const quickFilters = [
  ...
  { value: 'DUPLICATE', label: 'รายการซ้ำ' },
...
export default function WebhookLogsPage() {
  ...
  async function loadLogs() {
    ...
    try {
      ...
      const response = await adminApiFetch(`/admin/webhook-logs${suffix}`);
      const data = await response.json().catch(() => null);
      if (!response.ok || !data) throw new Error('logs');
      ...
      if (requestId !== requestSequence.current) return;
      setNotice({ text: payload.items.length > 0 ? 'รีเฟรชไม่สำเร็จ ข้อมูลเดิมยังแสดงอยู่ กรุณาลองใหม่' : 'โหลด Webhook ไม่สำเร็จ กรุณาลองใหม่', tone: 'danger' });
    } finally {
  ...
  const initialLoading = loading && payload.items.length === 0 && !notice;

  return <AdminPage
    eyebrow="ระบบเชื่อมต่อ"
    title="Webhook จากค่าย"
    description="ตรวจ Callback ลายเซ็น รายการซ้ำ และเหตุการณ์ที่ต้องติดตาม"
    actions={<>
      <AdminButton size="compact" tone="ghost" onClick={clearFilters} disabled={loading}>ล้างตัวกรอง</AdminButton>
      <AdminButton size="compact" onClick={() => void loadLogs()} disabled={loading}>{loading ? 'กำลังโหลด...' : 'รีเฟรช'}</AdminButton>
    </>}
  >
    {notice && <AdminNotice tone={notice.tone}><div style={noticeStyle}><span>{notice.text}</span><AdminButton size="compact" tone="secondary" onClick={() => void loadLogs()} disabled={loading}>ลองใหม่</AdminButton></div></AdminNotice>}

    {initialLoading ? <AdminCard title="กำลังโหลด Webhook" description="กำลังตรวจรายการและสรุปสถานะล่าสุด"><AdminSkeleton lines={7} /></AdminCard> : <>
      <AdminMetricGrid>
        <AdminMetric title="ทั้งหมด" value={String(metrics.total)} helper="ตามตัวกรอง" />
        <AdminMetric title="ประมวลผลแล้ว" value={String(metrics.processed)} tone="success" />
        <AdminMetric title="รายการซ้ำ" value={String(metrics.duplicate)} tone={metrics.duplicate ? 'warning' : 'success'} />
        <AdminMetric title="มีปัญหา" value={String(metrics.failed)} tone={metrics.failed ? 'danger' : 'success'} />
      </AdminMetricGrid>

      <AdminToolbar>
        <strong>ตัวกรองด่วน</strong>
        <div style={quickFilterStyle}>
          {quickFilters.map((item) => <AdminButton
            key={item.value}
            size="compact"
            tone={status === item.value ? 'primary' : 'ghost'}
            disabled={loading}
            onClick={() => updateStatus(item.value)}
          >{item.label}</AdminButton>)}
        </div>
      </AdminToolbar>

      <AdminFilterBar resultText={loading ? 'กำลังอัปเดต...' : `${payload.items.length}/${payload.total} รายการ`}>
        <input disabled={loading} value={query} onChange={(event) => updateQuery(event.target.value)} placeholder="ค้นหาค่าย รหัสรายการ หรือเหตุการณ์" />
        <select disabled={loading} value={status} onChange={(event) => updateStatus(event.target.value)}>
          <option value="all">ทุกสถานะ</option>
          <option value="RECEIVED">รับข้อมูลแล้ว</option>
          <option value="PROCESSED">ประมวลผลแล้ว</option>
          <option value="FAILED">มีปัญหา</option>
          <option value="DUPLICATE">รายการซ้ำ</option>
          <option value="IGNORED">ข้ามรายการ</option>
          <option value="RESOLVED">แก้ไขแล้ว</option>
        </select>
        <select disabled={loading} aria-label="จำนวนรายการต่อหน้า" value={list.pageSize} onChange={(event) => list.setPageSize(Number(event.target.value))}>
          {list.allowedPageSizes.map((size) => <option key={size} value={size}>{size} / หน้า</option>)}
        </select>
      </AdminFilterBar>

      <AdminStack>
        {payload.items.map((item) => <AdminCard key={item.id} compact tone={item.status === 'FAILED' || !item.signatureValid ? 'danger' : 'neutral'}>
          <AdminRow>
            <div style={mainInfoStyle}><h2 style={titleStyle}>{eventLabel(item.eventType)}</h2><span style={mutedStyle}>{item.provider?.name ?? item.provider?.code ?? '-'} · HTTP {item.responseStatus ?? '-'}</span></div>
            <div style={badgeStackStyle}><AdminBadge tone={statusTone(item.status)}>{humanStatus(item.status)}</AdminBadge><AdminBadge tone={item.signatureValid ? 'success' : 'danger'}>{item.signatureValid ? 'ลายเซ็นถูกต้อง' : 'ลายเซ็นไม่ถูกต้อง'}</AdminBadge></div>
          </AdminRow>
          ...
          <AdminTechnicalDetails summary="ข้อมูลเทคนิค">
            <AdminPayloadViewer payload={redactAdminPayload({ rawPayload: item.rawPayload, normalizedPayload: item.normalizedPayload })} maxHeight={360} />
          </AdminTechnicalDetails>
        </AdminCard>)}
        {!loading && payload.items.length === 0 && <AdminEmpty>ไม่พบ Webhook ตามตัวกรอง</AdminEmpty>}
      </AdminStack>
...
const quickFilterStyle = { display: 'flex', gap: 6, flexWrap: 'wrap' as const, justifyContent: 'flex-end' as const };
