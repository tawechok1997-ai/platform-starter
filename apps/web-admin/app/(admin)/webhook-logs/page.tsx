...
export default function WebhookLogsPage() {
  ...
  async function loadLogs() {
    ...
    try {
      ...
      if (!res.ok || !data) throw new Error('logs');
      ...
      setExpanded('');
      setMessage('โหลด Webhook ไม่สำเร็จ กรุณาลองใหม่');
    } finally {
      setLoading(false);
    }
  }

  function clearFilters() { if (loading) return; setQuery(''); setStatus('all'); list.resetPage(); }
  function updateQuery(value: string) { if (loading) return; setQuery(value); list.resetPage(); }
  function updateStatus(value: string) { if (loading) return; setStatus(value); list.resetPage(); }

  const metrics = payload.summary;
  return <AdminPage eyebrow="ระบบเชื่อมต่อ" title="Webhook จากค่าย" description="ตรวจ callback ลายเซ็น และรายการที่ต้องติดตาม" actions={<><AdminButton size="compact" tone="ghost" onClick={clearFilters} disabled={loading}>ล้างตัวกรอง</AdminButton><AdminButton size="compact" onClick={() => void loadLogs()} disabled={loading}>{loading ? 'กำลังโหลด...' : 'รีเฟรช'}</AdminButton></>}>
    ...
    <AdminFilterBar resultText={loading ? 'กำลังโหลด...' : `${payload.items.length}/${payload.total} รายการ`}><input disabled={loading} value={query} onChange={(event) => updateQuery(event.target.value)} placeholder="ค้นหาค่าย รหัสรายการ หรือเหตุการณ์" /><select disabled={loading} value={status} onChange={(event) => updateStatus(event.target.value)}><option value="all">ทุกสถานะ</option><option value="RECEIVED">รับข้อมูลแล้ว</option><option value="PROCESSED">ประมวลผลแล้ว</option><option value="FAILED">มีปัญหา</option><option value="DUPLICATE">รายการซ้ำ</option><option value="IGNORED">ข้ามรายการ</option><option value="RESOLVED">แก้ไขแล้ว</option></select><select disabled={loading} aria-label="จำนวนรายการต่อหน้า" value={list.pageSize} onChange={(event) => list.setPageSize(Number(event.target.value))}>{list.allowedPageSizes.map((size) => <option key={size} value={size}>{size} / หน้า</option>)}</select></AdminFilterBar>
