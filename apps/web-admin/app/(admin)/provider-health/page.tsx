...
export default function ProviderHealthPage() {
  ...
  const columns: AdminDataColumn<Provider>[] = [
    ...
    { key: 'status', title: 'Status', render: (provider) => <AdminBadge tone={providerTone(provider, health[provider.id])}>{health[provider.id]?.payload?.status ?? provider.status}</AdminBadge>, sortValue: (provider) => health[provider.id]?.payload?.status ?? provider.status },
    { key: 'latency', title: 'Latency', render: (provider) => health[provider.id]?.payload?.latencyMs != null ? `${health[provider.id].payload?.latencyMs} ms` : '-', sortValue: (provider) => Number(health[provider.id]?.payload?.latencyMs ?? 0), align: 'right' },
    { key: 'readiness', title: 'Readiness', render: (provider) => health[provider.id]?.readiness ? `${health[provider.id].readiness?.passed ?? 0}/${health[provider.id].readiness?.total ?? 0}` : '-', sortValue: (provider) => Number(health[provider.id]?.readiness?.passed ?? 0), align: 'center' },
