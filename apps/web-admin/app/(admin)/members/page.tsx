...
export default function MembersPage() {
  ...
  function closeDrawer() { setDrawerId(''); setDrawerData(null); }
  ...
  const columns = useMemo<readonly AdminDataColumn<MemberItem>[]>(() => [
    { id: 'member', header: copy.member, mobileLabel: copy.member, priority: 'primary', width: '24%', cell: (item) => <button type="button" className="admin-members-name-button" onClick={() => void openDrawer(item)}><strong>{item.username}</strong><small>{item.displayName || copy.noDisplayName} · {item.shortId}</small></button> },
    { id: 'status', header: copy.status, mobileLabel: copy.status, priority: 'secondary', width: '12%', cell: (item) => <AdminBadge tone={statusTone(item.status)}>{statusLabel(item.status, copy)}</AdminBadge> },
    { id: 'contact', header: copy.contact, mobileLabel: copy.contact, priority: 'secondary', width: '20%', cell: (item) => <span>{maskPhone(item.phone, canViewPii)}<br />{maskEmail(item.email, canViewPii)}</span> },
    { id: 'available', header: copy.available, mobileLabel: copy.available, priority: 'secondary', align: 'end', width: '14%', cell: (item) => canViewBalances ? formatMoney(item.availableBalance) : '••••••' },
    { id: 'lastLogin', header: copy.lastLogin, mobileLabel: copy.lastLogin, priority: 'secondary', width: '16%', cell: (item) => item.lastLoginAt ? new Date(item.lastLoginAt).toLocaleString(dateLocale) : copy.neverLoggedIn },
    { id: 'actions', header: copy.actions, mobileLabel: copy.actions, priority: 'secondary', align: 'end', width: '14%', cell: (item) => <div className="admin-members-row-actions"><AdminButton size="compact" tone="secondary" onClick={() => void openDrawer(item)}>{copy.details}</AdminButton><AdminButton size="compact" tone="success" disabled={busyId === item.id || item.status === 'ACTIVE'} onClick={() => requestStatus(item, 'ACTIVE')}>{copy.open}</AdminButton><AdminButton size="compact" tone="danger" disabled={busyId === item.id || item.status === 'SUSPENDED'} onClick={() => requestStatus(item, 'SUSPENDED')}>{copy.suspend}</AdminButton></div> },
  ], [busyId, canViewBalances, canViewPii, copy, dateLocale]);
  ...
  return <AdminPage eyebrow={copy.eyebrow} title={copy.title} description={copy.description} actions={<AdminButton onClick={() => void loadItems(page, submittedSearch, status)}>{copy.refresh}</AdminButton>}>
    ...
    <AdminCard title={copy.directory} description={copy.directoryDescription}>
      <AdminDataTable
        ariaLabel={copy.directory}
        columns={columns}
        rows={items}
        rowKey={(item) => item.id}
        loading={loading}
        ...
        labels={{ loading: copy.loading, empty: copy.noMembers, previousPage: copy.previous, nextPage: copy.next, page: (value) => `${copy.page} ${formatNumber(value, locale)}`, rowsPerPage: copy.currentPageOnly, range: (from, to, count) => locale === 'th' ? `${formatNumber(from, locale)}–${formatNumber(to, locale)} จาก ${formatNumber(count, locale)}` : `${formatNumber(from, locale)}–${formatNumber(to, locale)} of ${formatNumber(count, locale)}` }}
      />
    </AdminCard>
    <AdminDrawer open={Boolean(drawerId)} title={drawerData?.user.username ?? copy.memberDetail} description={copy.drawerAria} closeLabel={copy.close} size="wide" busy={drawerLoading} onClose={closeDrawer}>
      {drawerLoading ? <AdminEmpty>{copy.loading}</AdminEmpty> : null}
      {drawerData ? <AdminStack><AdminRow><span>{copy.status}</span><AdminBadge tone={statusTone(drawerData.user.status)}>{statusLabel(drawerData.user.status, copy)}</AdminBadge></AdminRow><AdminRow><span>{copy.phone}</span><strong>{maskPhone(drawerData.user.phone, canViewPii)}</strong></AdminRow><AdminRow><span>{copy.email}</span><strong>{maskEmail(drawerData.user.email, canViewPii)}</strong></AdminRow><AdminRow><span>{copy.joined}</span><span>{new Date(drawerData.user.createdAt).toLocaleString(dateLocale)}</span></AdminRow><AdminRow><span>{copy.lastLogin}</span><span>{drawerData.user.lastLoginAt ? new Date(drawerData.user.lastLoginAt).toLocaleString(dateLocale) : copy.neverLoggedIn}</span></AdminRow><AdminCard title={copy.wallet} compact>{drawerData.wallet ? <AdminStack><AdminRow><span>{copy.total}</span><strong>{canViewBalances ? formatMoney(drawerData.wallet.balance) : '••••••'}</strong></AdminRow><AdminRow><span>{copy.locked}</span><strong>{canViewBalances ? formatMoney(drawerData.wallet.lockedBalance) : '••••••'}</strong></AdminRow><AdminRow><span>{copy.available}</span><strong>{canViewBalances ? formatMoney(drawerData.wallet.availableBalance) : '••••••'}</strong></AdminRow></AdminStack> : <AdminEmpty>{copy.noWallet}</AdminEmpty>}</AdminCard></AdminStack> : null}
    </AdminDrawer>
