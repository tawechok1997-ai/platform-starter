...
export default function CommissionLedgersPage() {
  ...
  return <AdminPage eyebrow="Affiliate Ops" title="Commission Ledger" description="คำนวณและตรวจ commission แบบ ledger review เท่านั้น ยังไม่จ่ายเงินจริงเข้า wallet" actions={<AdminButton disabled={Boolean(busyId)} onClick={() => void load()}>รีเฟรช</AdminButton>}>
    ...
    <AdminCard title="คำนวณ / สร้าง Commission" tone="warning"><AdminStack><div style={formGridStyle}><select value={agentProfileId} onChange={(event) => setAgentProfileId(event.target.value)} style={inputStyle}><option value="">เลือกตัวแทนที่อนุมัติแล้ว</option>{agents.map((agent) => <option key={agent.id} value={agent.id}>{agent.displayName} · {agent.referralCode}</option>)}</select><input type="number" value={basisAmount} onChange={(event) => setBasisAmount(event.target.value)} placeholder="Basis amount เช่น ยอดฝากใต้สาย" style={inputStyle} /><input type="number" value={ratePercent} onChange={(event) => setRatePercent(event.target.value)} placeholder="Rate %" style={inputStyle} /><input type="number" value={capAmount} onChange={(event) => setCapAmount(event.target.value)} placeholder="Cap amount ไม่บังคับ" style={inputStyle} /><input value={basis} onChange={(event) => setBasis(event.target.value)} placeholder="basis" style={inputStyle} /><input type="number" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="ยอด final override ได้" style={inputStyle} /><input value={note} onChange={(event) => setNote(event.target.value)} placeholder="หมายเหตุ" style={inputStyle} /></div>{preview && <section style={previewStyle}><AdminRow><strong>Preview amount</strong><span>{money(preview.amount)} {preview.currency}</span></AdminRow><AdminRow><strong>สูตร</strong><span>{money(preview.rule.basisAmount)} × {preview.rule.ratePercent}% {preview.rule.capAmount ? `cap ${money(preview.rule.capAmount)}` : ''}</span></AdminRow><AdminRow><strong>Downline</strong><span>{preview.downlineCount} คน</span></AdminRow><AdminRow><strong>Payout</strong><span>{preview.payoutStatus}</span></AdminRow></section>}<div style={actionRowStyle}><AdminButton disabled={Boolean(busyId)} tone="secondary" onClick={() => void calculate()}>{busyId === 'preview' ? 'กำลังคำนวณ...' : 'Preview'}</AdminButton><AdminButton disabled={Boolean(busyId)} onClick={() => setPendingAction({ kind: 'create' })}>สร้าง Ledger</AdminButton></div></AdminStack></AdminCard>
    <AdminToolbar><select value={status} onChange={(event) => setStatus(event.target.value)} style={inputStyle}><option value="ALL">ทุกสถานะ</option><option value="OPEN">รอตรวจ</option><option value="REVIEWING">กำลังตรวจ</option><option value="RESOLVED">อนุมัติ</option><option value="DISMISSED">ปฏิเสธ</option></select><input value={rejectReason} onChange={(event) => setRejectReason(event.target.value)} placeholder="เหตุผลที่ปฏิเสธ (บังคับเมื่อ Reject)" style={inputStyle} /></AdminToolbar>
    <AdminGrid>{items.map((item) => <AdminCard key={item.id} title={`${item.referralCode} · ${money(item.amount)}`} description={`${memberLabel(item)} · ${new Date(item.createdAt).toLocaleString('th-TH')}`} tone={item.status === 'APPROVED' ? 'success' : item.status === 'REJECTED' ? 'danger' : 'warning'}><AdminStack><AdminRow><strong>สถานะ</strong><AdminBadge tone={statusTone(item.status)}>{statusLabel(item.status)}</AdminBadge></AdminRow><AdminRow><strong>Basis</strong><span>{item.basis}</span></AdminRow><AdminRow><strong>สูตร</strong><span>{money(item.basisAmount ?? 0)} × {item.ratePercent ?? 0}% {item.capAmount ? `cap ${money(item.capAmount)}` : ''}</span></AdminRow><AdminRow><strong>Calculated</strong><span>{money(item.calculatedAmount ?? item.amount)} {item.manualOverride ? '· override' : ''}</span></AdminRow><AdminRow><strong>Payout</strong><span>{item.payoutStatus}</span></AdminRow>{item.note && <AdminRow><strong>หมายเหตุ</strong><span>{item.note}</span></AdminRow>}<div style={actionRowStyle}><AdminButton disabled={Boolean(busyId)} tone="secondary" onClick={() => setPendingAction({ kind: 'review', id: item.id, next: 'APPROVED' })}>อนุมัติ</AdminButton><AdminButton disabled={Boolean(busyId)} tone="danger" onClick={() => setPendingAction({ kind: 'review', id: item.id, next: 'REJECTED' })}>ปฏิเสธ</AdminButton></div></AdminStack></AdminCard>)}{items.length === 0 && <AdminEmpty>ยังไม่มี commission ledger</AdminEmpty>}</AdminGrid>
    <AdminConfirmDialog open={Boolean(pendingAction)} title={confirmTitle} description={confirmDescription} confirmLabel="ยืนยัน" tone={pendingAction?.kind === 'review' && pendingAction.next === 'REJECTED' ? 'danger' : 'primary'} busy={Boolean(busyId)} onCancel={() => setPendingAction(null)} onConfirm={() => { if (pendingAction?.kind === 'create') void create(); else if (pendingAction?.kind === 'review') void review(pendingAction.id, pendingAction.next); }} />
  </AdminPage>;
}
