...
export default function ReportsPage() {
  ...
  function clearRange() {
    ...
    setRange(emptyRange);
    void loadReports(trendDays, emptyRange);
  }
  ...
  return <AdminPage eyebrow="การเงินและการตรวจสอบ" title="รายงานระบบ" description="ดูภาพรวมเงินเข้าออก ยอดกระเป๋า รายการค้าง และผลตรวจสอบยอดในหน้าเดียว" actions={<><AdminButton disabled={loading} onClick={() => void loadReports()}>รีเฟรช</AdminButton><AdminLinkButton href="/exports">ศูนย์ส่งออกข้อมูล</AdminLinkButton></>}>
    ...
    <AdminCard title="ช่วงวันที่" description="ใช้กับรายงานสรุปรายวัน ส่วนแนวโน้มเลือกช่วงเวลาแยกด้านล่าง">
      <AdminFilterBar resultText={range.from || range.to ? 'กำหนดช่วงวันที่แล้ว' : 'ใช้ช่วงวันที่เริ่มต้น'}>
        <label style={filterLabelStyle}><span>ตั้งแต่วันที่</span><input type="date" value={range.from} onChange={(event) => setRange((current) => ({ ...current, from: event.target.value }))} /></label>
        <label style={filterLabelStyle}><span>ถึงวันที่</span><input type="date" value={range.to} onChange={(event) => setRange((current) => ({ ...current, to: event.target.value }))} /></label>
...
const reconAmountGridStyle = { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(170px, 100%), 1fr))', gap: 10, minWidth: 0 };
...
const filterLabelStyle = { display: 'grid', gap: 6, fontWeight: 850, minWidth: 0 } as const;
