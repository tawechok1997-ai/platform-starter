...
export default function WalletLedgersPage() {
  ...
  return <AdminPage eyebrow={copy.eyebrow} title={copy.title} description={copy.description} actions={<><AdminButton tone="secondary" onClick={exportCsv} disabled={!visibleItems.length}>{copy.exportCsv}</AdminButton><AdminButton onClick={() => void load()} disabled={loading}>{copy.refresh}</AdminButton></>}>
    ...
    <AdminToolbar><label className="admin-ledger-field"><span>{copy.search}</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={copy.searchPlaceholder} /></label><label className="admin-ledger-field"><span>{copy.direction}</span><select value={direction} onChange={(event) => setDirection(event.target.value)}><option value="ALL">{copy.all}</option><option value="CREDIT">{copy.directions.CREDIT}</option><option value="DEBIT">{copy.directions.DEBIT}</option></select></label><label className="admin-ledger-field"><span>{copy.type}</span><select value={type} onChange={(event) => setType(event.target.value)}><option value="ALL">{copy.all}</option>{typeOptions.map((value) => <option key={value} value={value}>{copy.types[value] ?? value}</option>)}</select></label><label className="admin-ledger-field"><span>{copy.dateFrom}</span><input type="date" value={dateFrom} onChange={(event) => setDateFrom(event.target.value)} /></label><label className="admin-ledger-field"><span>{copy.dateTo}</span><input type="date" value={dateTo} min={dateFrom || undefined} onChange={(event) => setDateTo(event.target.value)} /></label><div className="admin-ledger-guidance">{copy.guidance}</div></AdminToolbar>
