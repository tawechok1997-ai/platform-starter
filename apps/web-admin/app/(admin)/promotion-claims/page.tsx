...
export default function PromotionClaimsPage() {
  ...
  return <AdminPage eyebrow="งานโปรโมชัน" title="คำขอรับโปรโมชัน" description="ตรวจคำขอของสมาชิก ดูหลักฐานการฝาก และอนุมัติหรือปฏิเสธพร้อมบันทึกเหตุผล" actions={<AdminButton disabled={isLoading} onClick={() => void load()}>{isLoading ? 'กำลังโหลด...' : 'รีเฟรช'}</AdminButton>}>
    ...
    {selectedClaim && <div style={drawerLayerStyle} role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelectedClaim(null); }}><aside style={drawerStyle} aria-label="Promotion claim review"><AdminStack><AdminRow><div><p style={{ margin: 0, color: '#94a3b8' }}>Promotion claim review</p><h2 style={{ margin: '4px 0 0' }}>{selectedClaim.campaign?.title ?? selectedClaim.campaignId}</h2></div><AdminButton tone="ghost" onClick={() => setSelectedClaim(null)}>ปิด</AdminButton></AdminRow><AdminRow><span>สมาชิก</span><strong>{memberLabel(selectedClaim)}</strong></AdminRow><AdminRow><span>หลักฐานรายการฝาก</span><strong>{selectedClaim.linkedTopup ? `${money(Number(selectedClaim.linkedTopup.amount))} · ${selectedClaim.linkedTopup.referenceCode ?? selectedClaim.linkedTopup.id}` : 'ไม่พบรายการฝากที่เชื่อมโยง'}</strong></AdminRow><AdminRow><span>สถานะรายการฝาก</span><AdminBadge tone={selectedClaim.linkedTopup?.status === 'COMPLETED' ? 'success' : 'warning'}>{selectedClaim.linkedTopup ? topupStatusLabel(selectedClaim.linkedTopup.status) : '-'}</AdminBadge></AdminRow><AdminRow><span>SLA</span><AdminBadge tone={slaTone(selectedClaim)}>{slaLabel(selectedClaim)}</AdminBadge></AdminRow><AdminCard title="Timeline" compact><AdminStack>{(selectedClaim.events ?? []).map((event, index) => <AdminRow key={`${event.createdAt}-${index}`}><div><strong>{eventActionLabel(event.action)}</strong><p style={{ margin: '4px 0 0', color: '#94a3b8' }}>{event.message || '-'}</p></div><small>{new Date(event.createdAt).toLocaleString('th-TH')}</small></AdminRow>)}{(selectedClaim.events ?? []).length === 0 && <AdminEmpty>ยังไม่มี timeline</AdminEmpty>}</AdminStack></AdminCard><label style={{ display: 'grid', gap: 6, fontWeight: 800 }}>เหตุผลผู้ดูแล<textarea value={notes[selectedClaim.id] ?? ''} onChange={(event) => setNotes((current) => ({ ...current, [selectedClaim.id]: event.target.value }))} placeholder="จำเป็นเมื่อปฏิเสธ" style={textareaStyle} /></label><div style={actionRowStyle}><AdminButton tone="success" disabled={busyId === selectedClaim.id || selectedClaim.status === 'APPROVED'} onClick={() => requestReview(selectedClaim.id, 'APPROVED')}>อนุมัติ</AdminButton><AdminButton tone="danger" disabled={busyId === selectedClaim.id || selectedClaim.status === 'REJECTED'} onClick={() => requestReview(selectedClaim.id, 'REJECTED')}>ปฏิเสธ</AdminButton></div></AdminStack></aside></div>}
  </AdminPage>;
}
...
const drawerStyle = { width: 'min(620px, 100%)', height: '100%', overflow: 'auto' as const, padding: 24, background: '#111823', borderLeft: '1px solid rgba(148,163,184,.22)' };
