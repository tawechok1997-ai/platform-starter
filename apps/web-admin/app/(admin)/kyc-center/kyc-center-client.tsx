...
type KycSummary = {
  ...
  duplicateGroups?: Array<{ accountNumber: string; count: number; items: MemberBank[] }>;
  riskyAccounts?: MemberBank[];
};

type ReviewStatus = 'ACTIVE' | 'REJECTED' | 'DISABLED';
type PendingReview = { item: MemberBank; status: ReviewStatus } | null;
type KycCase = {
  id: string;
  status: string;
...
export default function KycCenterPage() {
  ...
  const [openingKycId, setOpeningKycId] = useState('');
  ...
  const queue = useMemo(
    () => accounts.filter((item) => item.status === 'PENDING_REVIEW'),
    [accounts],
  );
  ...
  async function load() {
    ...
    try {
      ...
      if (!summaryRes.ok || !accountRes.ok) throw new Error('load');
  ...
  async function loadKycCases(nextStatus = kycStatus) {
    ...
    try {
      ...
      setKycCases(Array.isArray(data?.items) ? data.items : []);
    } catch {
      setKycCases([]);
      setMessage('โหลดรายการ KYC ไม่สำเร็จ');
    } finally {
      setKycLoading(false);
    }
  }
  ...
  async function openKycCase(id: string) {
    if (openingKycId) return;
    setOpeningKycId(id);
    setMessage('');
    try {
      const res = await adminApiFetch(`/admin/kyc/cases/${id}`);
      const data = await res.json().catch(() => null);
      if (!res.ok || !data?.item || !Array.isArray(data?.documents)) throw new Error('detail');
      setSelectedKyc(data as KycCaseDetail);
      setKycNote('');
    } catch {
      setMessage('โหลดรายละเอียด KYC ไม่สำเร็จ');
    } finally {
      setOpeningKycId('');
    }
  }
  ...
  async function confirmKycReview() {
    if (!pendingKycReview || busyId) return;
    const intent = pendingKycReview;
    const note = kycNote.trim();
    if (intent.status === 'REJECTED' && note.length < 5) {
      setMessage('ต้องระบุเหตุผลปฏิเสธ KYC อย่างน้อย 5 ตัวอักษร');
      return;
    }
    setBusyId(intent.item.id);
    setMessage('');
    try {
      const res = await adminApiFetch(`/admin/kyc/cases/${intent.item.id}/review`, {
        method: 'PATCH',
        body: JSON.stringify({ status: intent.status, note: note || undefined, version: intent.item.version }),
      });
      const data = await res.json().catch(() => null);
      if (!res.ok || !data?.item) throw new Error('review');
      setPendingKycReview(null);
      setSelectedKyc((current) => current ? { ...current, item: { ...current.item, ...data.item } } : current);
      setMessage(`อัปเดต KYC เป็น ${kycStatusLabel(intent.status)} แล้ว`);
      await loadKycCases();
    } catch {
      setMessage('อัปเดต KYC ไม่สำเร็จ');
    } finally {
      setBusyId('');
    }
  }
  ...
  const pageBusy = accountLoading || kycLoading || Boolean(busyId) || Boolean(openingKycId);

  return (
    <AdminPage eyebrow="ตรวจสอบสมาชิก" title="ตรวจบัญชีธนาคาร" description="ตรวจชื่อบัญชี เลขบัญชีซ้ำ และข้อมูลเสี่ยงก่อนอนุญาตให้ถอนเงิน">
      ...
      <AdminCard title="KYC review queue" description="ตรวจเอกสาร ยืนยันความเสี่ยง และอนุมัติเป็นขั้นตอน">
        <AdminToolbar>
          <label style={fieldStyle}>สถานะ KYC
            <select disabled={pageBusy} value={kycStatus} onChange={(event) => { const value = event.target.value; setKycStatus(value); void loadKycCases(value); }}>
              {KYC_STATUSES.map((value) => <option key={value} value={value}>{kycStatusLabel(value)}</option>)}
            </select>
          </label>
          <AdminButton tone="secondary" disabled={pageBusy} onClick={() => void loadKycCases()}>{kycLoading ? 'กำลังโหลด...' : 'รีเฟรช KYC'}</AdminButton>
        </AdminToolbar>
        <AdminStack>
          {kycCases.map((item) => (
            <AdminRow key={item.id}>
              <div>
                <div style={flagRowStyle}>
                  <AdminBadge tone={kycTone(item.status)}>{kycStatusLabel(item.status)}</AdminBadge>
                  {item.riskLevel && <AdminBadge tone={item.riskLevel === 'HIGH' || item.riskLevel === 'CRITICAL' ? 'danger' : 'warning'}>Risk: {item.riskLevel}</AdminBadge>}
                </div>
                <strong>{item.member?.username ?? item.id.slice(0, 8)}</strong>
                <p style={mutedStyle}>{item.documentCount ?? 0} เอกสาร · ส่งเมื่อ {new Date(item.createdAt).toLocaleString('th-TH')}</p>
                {item.reviewNote && <p style={mutedStyle}>เหตุผล: {item.reviewNote}</p>}
              </div>
              <AdminButton tone="secondary" disabled={pageBusy} onClick={() => void openKycCase(item.id)}>{openingKycId === item.id ? 'กำลังเปิด...' : 'เปิด checklist'}</AdminButton>
            </AdminRow>
          ))}
          {!kycLoading && kycCases.length === 0 && <AdminEmpty>ไม่มีรายการ KYC ตามตัวกรอง</AdminEmpty>}
        </AdminStack>
      </AdminCard>

      <AdminToolbar>
        <input disabled={pageBusy} value={search} onChange={(event) => setSearch(event.target.value)} placeholder="ค้นหาชื่อสมาชิก ธนาคาร หรือเลขบัญชี" style={inputStyle} />
        <AdminButton disabled={pageBusy} onClick={() => void load()}>{accountLoading ? 'กำลังค้นหา...' : 'ค้นหา'}</AdminButton>
      </AdminToolbar>

      {(summary.duplicateGroups ?? []).length > 0 && (
        <AdminCard title="เลขบัญชีซ้ำ" description="ตรวจเจ้าของบัญชีและข้อมูลสมาชิกทุกคนก่อนอนุมัติ" tone="danger">
          <AdminStack>{summary.duplicateGroups?.map((group) => <AdminRow key={group.accountNumber}><strong>{maskAccountNumber(group.accountNumber)}</strong><span>{group.count} บัญชี</span></AdminRow>)}</AdminStack>
        </AdminCard>
      )}

      <AdminGrid>
        <AdminCard title="บัญชีรอตรวจ" description="รายการที่ยังไม่ได้อนุมัติ" tone={queue.length > 0 ? 'warning' : 'success'}>
          <AdminStack>{queue.map((item) => <BankReviewCard key={item.id} item={item} note={notes[item.id] ?? ''} busy={pageBusy} onNote={(value) => setNotes((current) => ({ ...current, [item.id]: value }))} onReview={(status) => requestReview(item, status)} />)}{queue.length === 0 && <AdminEmpty>ไม่มีบัญชีรอตรวจ</AdminEmpty>}</AdminStack>
        </AdminCard>
      ...
      <AdminCard title="บัญชีทั้งหมด" description="รายการล่าสุดไม่เกิน 200 รายการ">
        <AdminStack>{accounts.map((item) => <BankReviewCard key={item.id} item={item} note={notes[item.id] ?? ''} busy={pageBusy} onNote={(value) => setNotes((current) => ({ ...current, [item.id]: value }))} onReview={(status) => requestReview(item, status)} compact />)}{accounts.length === 0 && <AdminEmpty>ยังไม่มีบัญชีสมาชิก</AdminEmpty>}</AdminStack>
      </AdminCard>
      ...
      <AdminConfirmDialog
        ...
        onCancel={() => { if (!busyId) setPendingReview(null); }}
      ...
      <AdminDrawer
        ...
        title={selectedKyc ? `KYC · ${selectedKyc.item.member?.username ?? selectedKyc.item.id.slice(0, 8)}` : 'รายละเอียด KYC'}
        description="ตรวจเอกสาร ความเสี่ยง และบันทึกเหตุผลก่อนเปลี่ยนสถานะ"
        busy={Boolean(selectedKyc && busyId === selectedKyc.item.id)}
        onClose={() => { if (!busyId) { setSelectedKyc(null); setPendingKycReview(null); } }}
      >
        {selectedKyc && <AdminStack>
          <AdminRow><span>สถานะ</span><AdminBadge tone={kycTone(selectedKyc.item.status)}>{kycStatusLabel(selectedKyc.item.status)}</AdminBadge></AdminRow>
          {selectedKyc.item.riskLevel && <AdminRow><span>ระดับความเสี่ยง</span><strong>{selectedKyc.item.riskLevel}</strong></AdminRow>}
          <AdminCard title="เอกสาร" description="สถานะตรวจเอกสารแต่ละรายการ" compact>
            <AdminStack>
              {selectedKyc.documents.map((document) => <AdminRow key={document.id}><div><strong>{kycDocumentLabel(document.documentType)}</strong><p style={mutedStyle}>{document.originalName}</p>{document.reviewNote && <p style={mutedStyle}>หมายเหตุ: {document.reviewNote}</p>}</div><AdminBadge tone={document.status === 'ACCEPTED' ? 'success' : document.status === 'REJECTED' ? 'danger' : 'warning'}>{document.status}</AdminBadge></AdminRow>)}
              {selectedKyc.documents.length === 0 && <AdminEmpty>ยังไม่มีเอกสาร</AdminEmpty>}
            </AdminStack>
          </AdminCard>
          <label style={fieldStyle}>เหตุผลสำหรับการตัดสินใจ<textarea disabled={Boolean(busyId)} value={kycNote} onChange={(event) => setKycNote(event.target.value)} style={textareaStyle} placeholder="จำเป็นเมื่อปฏิเสธ" /></label>
          <div style={actionRowStyle}>
            <AdminButton tone="secondary" disabled={Boolean(busyId) || selectedKyc.item.status === 'REVIEWING'} onClick={() => setPendingKycReview({ item: selectedKyc.item, status: 'REVIEWING' })}>เริ่มตรวจ</AdminButton>
            <AdminButton tone="success" disabled={Boolean(busyId) || selectedKyc.item.status === 'APPROVED'} onClick={() => setPendingKycReview({ item: selectedKyc.item, status: 'APPROVED' })}>อนุมัติ</AdminButton>
            <AdminButton tone="danger" disabled={Boolean(busyId) || selectedKyc.item.status === 'REJECTED'} onClick={() => setPendingKycReview({ item: selectedKyc.item, status: 'REJECTED' })}>ปฏิเสธ</AdminButton>
          </div>
        </AdminStack>}
      </AdminDrawer>

      <AdminConfirmDialog
        open={Boolean(pendingKycReview)}
        title={pendingKycReview ? `${kycStatusLabel(pendingKycReview.status)} KYC` : ''}
        description={pendingKycReview?.status === 'REJECTED' ? 'ต้องระบุเหตุผลก่อนปฏิเสธ KYC' : 'ยืนยันการเปลี่ยนสถานะ KYC'}
        confirmLabel="ยืนยัน"
        tone={pendingKycReview?.status === 'REJECTED' ? 'danger' : pendingKycReview?.status === 'APPROVED' ? 'success' : 'primary'}
        busy={Boolean(pendingKycReview && busyId === pendingKycReview.item.id)}
        onCancel={() => { if (!busyId) setPendingKycReview(null); }}
        onConfirm={() => void confirmKycReview()}
        details={pendingKycReview?.status === 'REJECTED' ? <p>เหตุผล: {kycNote.trim() || '-'}</p> : undefined}
      />
    </AdminPage>
  );
}
