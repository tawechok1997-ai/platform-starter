...
export default function AdminLedgersPage() {
  ...
  useEffect(() => { const params = new URLSearchParams(window.location.search); const nextIdentifier = params.get('identifier') ?? params.get('userId') ?? ''; setIdentifier(nextIdentifier); loadItems(nextIdentifier, 1); }, []);
  useEffect(() => { loadItems(identifier, page); }, [page]);
