...
export default function RiskAlertsPage() {
  ...
  async function bulkDismiss() {
    ...
    const res = await adminApiFetch('/admin/risk-alerts/bulk-dismiss', { method: 'POST', body: JSON.stringify({ ids: selectedIds }) });
    const data = await res.json().catch(() => null);
    if (!res.ok) setMessage(data?.message ?? 'ปิดรายการไม่สำเร็จ');
    else { setMessage(`ปิดรายการแล้ว ${Number(data?.updated ?? 0)} รายการ`); setSelectedIds([]); await load(page); }
  }
