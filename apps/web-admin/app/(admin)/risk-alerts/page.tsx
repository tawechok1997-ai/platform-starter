...
export default function RiskAlertsPage() {
  ...
  function clearFilters() { setStatus('OPEN'); setSeverity(''); setType(''); setMemberId(''); setCreatedFrom(''); setCreatedTo(''); setPage(1); setTimeout(() => load(1), 0); }
  ...
  return <AdminPage eyebrow="งานที่ต้องตรวจ" title="ปัญหาที่ต้องดู" description="รวมรายการผิดปกติ เช่น ฝาก/ถอนถี่ ยอดสูง ยอดเงินไม่ตรง ให้แอดมินตรวจและปิดเคส" actions={<AdminButton onClick={scan} disabled={scanDisabled}>{scanning ? 'กำลังสแกน...' : cooldownRemaining > 0 ? `รอ ${cooldownRemaining}s` : 'สแกนปัญหา'}</AdminButton>}>
    ...
    <AdminToolbar><label style={fieldStyle}>สถานะ<select value={status} onChange={(event) => setStatus(event.target.value)} style={inputStyle}>{statusOptions.map((value) => <option key={value} value={value}>{value ? humanStatus(value) : 'ทั้งหมด'}</option>)}</select></label><label style={fieldStyle}>ความรุนแรง<select value={severity} onChange={(event) => setSeverity(event.target.value)} style={inputStyle}>{severityOptions.map((value) => <option key={value} value={value}>{value ? severityLabel(value) : 'ทั้งหมด'}</option>)}</select></label><label style={fieldStyle}>ประเภท<select value={type} onChange={(event) => setType(event.target.value)} style={inputStyle}>{typeOptions.map((value) => <option key={value} value={value}>{value ? riskTypeLabel(value) : 'ทั้งหมด'}</option>)}</select></label><label style={fieldStyle}>สมาชิก<input value={memberId} onChange={(event) => setMemberId(event.target.value)} placeholder="UUID สมาชิก" style={inputStyle} /></label><label style={fieldStyle}>จาก<input type="date" value={createdFrom} onChange={(event) => setCreatedFrom(event.target.value)} style={inputStyle} /></label><label style={fieldStyle}>ถึง<input type="date" value={createdTo} onChange={(event) => setCreatedTo(event.target.value)} style={inputStyle} /></label><div style={filterActionStyle}><AdminButton tone="secondary" onClick={applyFilters}>ใช้ตัวกรอง</AdminButton><AdminButton tone="secondary" onClick={clearFilters}>ล้าง</AdminButton></div><div style={pagerStyle}><AdminButton disabled={loading || page <= 1} onClick={() => setPage((value) => Math.max(value - 1, 1))}>ก่อนหน้า</AdminButton><span>หน้า {page} / {pageCount}</span><AdminButton disabled={loading || page >= pageCount} onClick={() => setPage((value) => Math.min(value + 1, pageCount))}>ถัดไป</AdminButton></div></AdminToolbar>
    {message && <AdminNotice>{message}</AdminNotice>}
    <AdminCard title="รายการปัญหา" description="เปิดดูรายละเอียดก่อน Resolve/Dismiss โดยเฉพาะความเสี่ยงสูง"><AdminStack>{loading ? <AdminEmpty>กำลังโหลด...</AdminEmpty> : items.length === 0 ? <AdminEmpty>ไม่มีปัญหาตามตัวกรองนี้</AdminEmpty> : items.map((item) => <AdminRow key={item.id}><div style={alertBodyStyle}><div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}><AdminBadge tone={severityTone(item.severity)}>{severityLabel(item.severity)}</AdminBadge><AdminBadge tone={statusTone(item.status)}>{humanStatus(item.status)}</AdminBadge><AdminBadge>{riskTypeLabel(item.type)}</AdminBadge></div><strong>{item.title}</strong>{item.description && <span style={mutedStyle}>{item.description}</span>}<div style={detailGridStyle}><span>สมาชิก: {item.memberId ? <a href={`/members/${item.memberId}`} style={linkStyle}>{item.shortMemberId ?? item.memberId.slice(0, 8)}</a> : '-'}</span><span>อ้างอิง: {item.refId ? <a href={riskTargetHref(item)} style={linkStyle}>{item.refType ?? 'รายการ'} / {item.refId.slice(0, 8)}</a> : '-'}</span><span>สร้างเมื่อ: {new Date(item.createdAt).toLocaleString('th-TH')}</span></div>{item.metadata && <details style={detailsStyle}><summary>ข้อมูลเทคนิค</summary><RiskMetadataView metadata={item.metadata} /><RiskMetadataRaw metadata={item.metadata} /></details>}</div><div style={actionStyle}><AdminLinkButton href={`/risk-alerts/${item.id}`}>ดู</AdminLinkButton><AdminButton tone="secondary" disabled={item.status === 'REVIEWING'} onClick={() => updateStatus(item.id, 'REVIEWING')}>กำลังตรวจ</AdminButton><AdminButton tone="success" disabled={item.status === 'RESOLVED'} onClick={() => updateStatus(item.id, 'RESOLVED')}>แก้แล้ว</AdminButton><AdminButton tone="danger" disabled={item.status === 'DISMISSED'} onClick={() => updateStatus(item.id, 'DISMISSED')}>ปิด</AdminButton></div></AdminRow>)}</AdminStack></AdminCard>
  </AdminPage>;
}
...
function riskTypeLabel(type: string) { const map: Record<string, string> = { REPEATED_TOPUPS: 'ฝากถี่ผิดปกติ', RAPID_DEPOSIT_WITHDRAWAL: 'ฝากแล้วถอนเร็ว', HIGH_WITHDRAWAL: 'ถอนยอดสูง', BANK_CHANGE_WITHDRAWAL: 'เปลี่ยนบัญชีแล้วถอน', MULTIPLE_PENDING_TOPUPS: 'ฝากค้างหลายรายการ', WALLET_LEDGER_MISMATCH: 'ยอดเงินไม่ตรง', DUPLICATE_DEPOSIT_SLIP: 'พบสลิปฝากซ้ำ', REPEATED_DUPLICATE_DEPOSIT_SLIP: 'ใช้สลิปฝากซ้ำหลายครั้ง' }; return map[type] ?? type; }
