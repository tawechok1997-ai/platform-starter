...
type TransferResponse = { walletLedgerId?: string; walletDebitLedgerId?: string; walletRollbackLedgerId?: string; walletBalanceAfter?: string | number; walletBalanceAfterRollback?: string | number; manualReverse?: { ledgerId?: string; balanceAfter?: string | number } } & Record<string, unknown>;
type Transfer = { id: string; type: string; status: string; amount: string; currency: string; idempotencyKey: string; providerTransactionId?: string | null; errorCode?: string | null; errorMessage?: string | null; requestPayload?: unknown; responsePayload?: TransferResponse; createdAt: string; resolvedAt?: string | null; user?: { username?: string | null; phone?: string | null } | null; provider?: { name?: string | null; code?: string | null } | null; session?: { id: string; providerSessionId?: string | null; game?: { name?: string | null; providerGameCode?: string | null } | null } | null };
...
export default function GameTransferDetailPage({ params }: Props) {
  ...
  async function reloadAfterAction() {
    const res = await adminApiFetch(`/admin/game-transfers/${id}`);
    const data = await res.json().catch(() => null);
    if (!res.ok || !data?.id) throw new Error('reload');
    setItem(data as Transfer);
  }
  ...
  return <AdminPage eyebrow="แพลตฟอร์มเกม" title="รายละเอียดการโยกเงิน" description="ตรวจสถานะ ยอดเงิน และประวัติการทำงานของรายการ" actions={<><AdminLinkButton href="/game-transfers" tone="ghost" size="compact">← กลับ</AdminLinkButton><AdminButton size="compact" onClick={() => void load()} disabled={loading}>{loading ? 'กำลังโหลด...' : '↻ รีเฟรช'}</AdminButton></>}>
    ...
    {item && <AdminStack>
      ...
      <AdminCard compact title="การดำเนินการ" description={actionHint(item)}><div style={actionRowStyle}><AdminButton size="compact" tone="secondary" onClick={() => requestAction('review')} disabled={loading}>บันทึกผลตรวจ</AdminButton>{item.status === 'FAILED' && <AdminButton size="compact" onClick={() => requestAction('retry')} disabled={loading}>ทดสอบใหม่</AdminButton>}{item.status === 'SUCCESS' && <AdminButton size="compact" tone="danger" onClick={() => requestAction('reverse')} disabled={loading}>ย้อนรายการ</AdminButton>}{item.status === 'PENDING' && <AdminButton size="compact" tone="danger" onClick={() => requestAction('forceFail')} disabled={loading}>ปิดเป็นล้มเหลว</AdminButton>}</div></AdminCard>
      ...
      <AdminCard title="ประวัติกระเป๋าเงิน" description="เลขบัญชีแยกประเภทที่สร้างจากรายการนี้">
        <LedgerRow label="รายการหลัก" id={item.responsePayload?.walletLedgerId} /><LedgerRow label="รายการตัดยอด" id={item.responsePayload?.walletDebitLedgerId} /><LedgerRow label="รายการคืนยอด" id={item.responsePayload?.walletRollbackLedgerId} /><LedgerRow label="รายการย้อนด้วยมือ" id={item.responsePayload?.manualReverse?.ledgerId} />
        <AdminDataValue label="ยอดคงเหลือหลังทำรายการ">{String(item.responsePayload?.walletBalanceAfter ?? item.responsePayload?.walletBalanceAfterRollback ?? item.responsePayload?.manualReverse?.balanceAfter ?? '-')}</AdminDataValue>
      </AdminCard>
      <JsonCard title="ข้อมูลที่ส่งไปยังค่าย" payload={item.requestPayload} /><JsonCard title="ข้อมูลที่ค่ายตอบกลับ" payload={item.responsePayload} />
    </AdminStack>}
...
function LedgerRow({ label, id }: { label: string; id?: string | null }) { return <AdminRow><div style={{ minWidth: 0 }}><strong style={labelStyle}>{label}</strong><p style={mutedStyle}>{id ? <AdminCode title={id}>{id}</AdminCode> : '-'}</p></div>{id ? <AdminLinkButton href={`/wallet-ledgers/${id}`} tone="ghost" size="compact">เปิดรายการ</AdminLinkButton> : null}</AdminRow>; }
...
function actionFailure(kind: ActionKind) { return ({ review: 'บันทึกผลตรวจไม่สำเร็จ กรุณาลองใหม่', retry: 'ทดสอบรายการใหม่ไม่สำเร็จ กรุณาลองใหม่', reverse: 'ย้อนรายการไม่สำเร็จ กรุณาตรวจยอดก่อนลองใหม่', forceFail: 'ปิดรายการไม่สำเร็จ กรุณาลองใหม่' } as Record<ActionKind, string>)[kind]; }
