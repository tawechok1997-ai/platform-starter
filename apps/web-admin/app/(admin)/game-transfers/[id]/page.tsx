...
export default function GameTransferDetailPage({ params }: Props) {
  ...
  return <AdminPage eyebrow="แพลตฟอร์มเกม" title="รายละเอียดการโยกเงิน" description="ตรวจสถานะ ยอดเงิน และประวัติการทำงานของรายการ" actions={<><AdminLinkButton href="/game-transfers" tone="ghost" size="compact">← กลับ</AdminLinkButton><AdminButton size="compact" onClick={() => void load()} disabled={loading}>↻ รีเฟรช</AdminButton></>}>
    ...
    {item && <AdminStack>
      ...
      {(item.errorCode || item.errorMessage) && <AdminCard compact title="ข้อผิดพลาด" tone="danger"><AdminNotice tone="danger">{item.errorCode ?? '-'} · {item.errorMessage ?? '-'}</AdminNotice></AdminCard>}
    ...
    <AdminConfirmDialog open={Boolean(pendingAction)} title={pendingAction ? actionTitle(pendingAction.kind) : ''} description={pendingAction ? actionDescription(pendingAction.kind) : ''} confirmLabel={pendingAction ? actionConfirmLabel(pendingAction.kind) : 'ยืนยัน'} tone={pendingAction?.kind === 'reverse' || pendingAction?.kind === 'forceFail' ? 'danger' : pendingAction?.kind === 'retry' ? 'primary' : 'success'} busy={loading} onCancel={() => { setPendingAction(null); setNote(''); }} onConfirm={() => void confirmAction()} details={<label style={noteStyle}><span>เหตุผล / หมายเหตุ</span><textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="ระบุข้อมูลให้ผู้ตรวจคนถัดไปเข้าใจ" style={textareaStyle} /></label>} />
  </AdminPage>;
}
