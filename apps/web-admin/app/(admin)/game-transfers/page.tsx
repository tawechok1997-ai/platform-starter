...
export default function GameTransfersPage() {
  ...
  async function confirmAction() {
    ...
    if (reason.length < 8) { setMessage('กรุณาระบุเหตุผลหรือหมายเหตุอย่างน้อย 8 ตัวอักษร'); return; }
    const { item, action } = pendingAction;
    setWorking(item.id);
    setMessage('');
    try {
      const endpoint = action === 'review' ? `/admin/game-transfers/${item.id}/review` : `/admin/game-transfers/${item.id}/retry`;
      const body = action === 'review' ? { note: reason } : { reason };
      const res = await adminApiFetch(endpoint, { method: action === 'review' ? 'PATCH' : 'POST', body: JSON.stringify(body) });
      const data = await res.json().catch(() => null);
      if (!res.ok || (action === 'retry' && data?.ok !== true)) throw new Error('action');
      setPendingAction(null);
      setNote('');
      setMessage(action === 'review' ? 'บันทึกหมายเหตุแล้ว' : 'Retry รายการเงินจริงสำเร็จและบันทึก Audit แล้ว');
      await loadTransfers();
    } catch {
      setMessage(action === 'review' ? 'บันทึกผลตรวจไม่สำเร็จ กรุณาลองใหม่' : 'Retry รายการไม่สำเร็จ ระบบไม่ถือว่าเสร็จจนกว่าจะตรวจสถานะและ Reconciliation');
    } finally {
      setWorking('');
    }
  }
  ...
  return <AdminPage eyebrow="แพลตฟอร์มเกม" title="รายการโยกเงินเกม" description="ตรวจเงินเข้าเกม เงินกลับกระเป๋า การคืนรายการ และรายการที่ต้องติดตาม" actions={<AdminButton onClick={() => void loadTransfers()} disabled={busy}>{loading ? 'กำลังโหลด...' : 'รีเฟรช'}</AdminButton>}>
    ...
    <AdminStack>{filtered.map((item) => <AdminCard key={item.id}><AdminRow><div><h2 style={titleStyle}>{transferLabel(item.type)} · {formatMoney(item.amount, item.currency)}</h2><p style={mutedStyle}>{item.provider?.name ?? '-'} · {item.session?.game?.name ?? '-'} · สมาชิก {item.user?.username ?? item.user?.phone ?? '-'}</p><p style={smallMutedStyle}>รหัสกันรายการซ้ำ: {item.idempotencyKey}</p><p style={smallMutedStyle}>เลขอ้างอิงค่าย: {item.providerTransactionId ?? '-'}</p></div><div style={badgeStackStyle}><AdminBadge tone={statusTone(item.status)}>{humanStatus(item.status)}</AdminBadge><AdminLinkButton href={`/game-transfers/${item.id}`}>ดูรายละเอียด</AdminLinkButton><AdminButton tone="secondary" onClick={() => setExpanded(expanded === item.id ? '' : item.id)} disabled={busy}>{expanded === item.id ? 'ซ่อนข้อมูลเทคนิค' : 'ข้อมูลเทคนิค'}</AdminButton><AdminButton tone="secondary" onClick={() => requestAction(item, 'review')} disabled={busy}>บันทึกผลตรวจ</AdminButton>{item.status === 'FAILED' && <AdminButton tone="danger" onClick={() => requestAction(item, 'retry')} disabled={busy}>Retry รายการเงินจริง</AdminButton>}</div></AdminRow>{item.errorMessage && <AdminNotice tone="danger">{transferErrorLabel(item.errorCode)}</AdminNotice>}{expanded === item.id && <pre style={preStyle}>{stringifyAdminPayload({ requestPayload: item.requestPayload, responsePayload: item.responsePayload })}</pre>}<p style={smallMutedStyle}>สร้างเมื่อ {new Date(item.createdAt).toLocaleString('th-TH')}</p></AdminCard>)}{!loading && filtered.length === 0 && <AdminEmpty>ยังไม่มีรายการโยกเงินตามตัวกรอง</AdminEmpty>}</AdminStack>
    <AdminConfirmDialog open={Boolean(pendingAction)} title={pendingAction ? (pendingAction.action === 'review' ? 'บันทึกผลตรวจรายการ' : 'ยืนยัน Retry รายการเงินจริง') : ''} description={pendingAction?.action === 'review' ? 'หมายเหตุจะถูกบันทึกไว้กับรายการเพื่อใช้ติดตามและตรวจสอบย้อนหลัง' : 'คำสั่งนี้เป็น Real mutation: ระบบอาจเรียก Provider และ Debit/Credit Wallet จริงตามทิศทางของรายการเดิม หาก TRANSFER_IN ล้ม ระบบมี Rollback ledger แต่ยังต้องตรวจ Reconciliation หลังทำรายการ'} confirmLabel={pendingAction?.action === 'review' ? 'บันทึกผลตรวจ' : 'ยืนยัน Retry จริง'} tone={pendingAction?.action === 'retry' ? 'danger' : 'success'} busy={Boolean(pendingAction && working === pendingAction.item.id)} onCancel={() => { if (!working) { setPendingAction(null); setNote(''); } }} onConfirm={() => void confirmAction()} details={<label style={noteStyle}><span>{pendingAction?.action === 'retry' ? 'เหตุผลสำหรับ Sensitive action' : 'เหตุผล / หมายเหตุ'}</span><textarea disabled={Boolean(working)} value={note} onChange={(event) => setNote(event.target.value)} placeholder="ระบุข้อมูลให้ผู้ตรวจคนถัดไปเข้าใจ" style={textareaStyle} /></label>} />
  </AdminPage>;
}
