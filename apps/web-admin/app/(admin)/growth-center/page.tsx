...
import { AdminBadge, AdminButton, AdminCard, AdminLinkButton, AdminMetric, AdminMetricGrid, AdminNotice, AdminPage, AdminSkeleton, AdminStack } from '../_components/admin-ui';
...
export default function GrowthCenterPage() {
  ...
  return <AdminPage eyebrow="Growth" title="ศูนย์งาน Growth" description="รวมคิว Promotion, Bonus, Affiliate, Commission, KYC และ Support แบบอ่านอย่างเดียว" actions={<AdminButton onClick={() => void load()} disabled={loading}>{loading ? 'กำลังโหลด...' : 'รีเฟรช'}</AdminButton>}>
    <div className="admin-growth-center">
      ...
      {loading && totalLoaded(data) === 0 ? <AdminSkeleton lines={6} /> : <>
        <div className="admin-growth-center__metrics">
          <AdminMetricGrid><AdminMetric tone={metrics.promotionPending > 0 ? 'warning' : 'success'} title="คำขอโปรโมชัน" value={metrics.promotionPending.toLocaleString('th-TH')} helper="รอตรวจ" /><AdminMetric tone={metrics.bonusActive > 0 ? 'warning' : 'success'} title="โบนัสใช้งานอยู่" value={metrics.bonusActive.toLocaleString('th-TH')} helper="ยังไม่ปิดวงจร" /><AdminMetric tone={metrics.affiliatePending > 0 ? 'warning' : 'success'} title="ตัวแทนรอตรวจ" value={metrics.affiliatePending.toLocaleString('th-TH')} /><AdminMetric tone={metrics.commissionPending > 0 ? 'warning' : 'success'} title="คอมมิชชันรอตรวจ" value={metrics.commissionPending.toLocaleString('th-TH')} /></AdminMetricGrid>
          <AdminMetricGrid><AdminMetric tone={metrics.supportOpen > 0 ? 'danger' : 'success'} title="Support เปิดอยู่" value={metrics.supportOpen.toLocaleString('th-TH')} /><AdminMetric tone={metrics.kycPending > 0 ? 'warning' : 'success'} title="KYC รอตรวจ" value={metrics.kycPending.toLocaleString('th-TH')} /><AdminMetric title="จ่ายอัตโนมัติ" value="ปิด" helper="ไม่อนุมัติหรือจ่ายจากหน้านี้" tone="danger" /><AdminMetric title="Wallet credit" value="มี Guard" helper="โบนัสต้องผ่านเงื่อนไขก่อน" tone="warning" /></AdminMetricGrid>
        </div>
...
function OpsCard({ title, href, tone, rows, error }: { title: string; href: string; tone: 'warning' | 'success' | 'danger' | 'neutral'; rows: Array<[string, number]>; error?: string | undefined }) { return <div className="admin-growth-queue" data-tone={error ? 'danger' : tone}><AdminCard title={title} tone={error ? 'danger' : tone}><AdminStack>{error && <AdminNotice tone="danger">โหลดข้อมูลไม่สำเร็จ</AdminNotice>}{rows.map(([label, value]) => <div className="admin-growth-queue__row" key={label}><strong>{label}</strong><AdminBadge tone={value > 0 ? tone : 'success'}>{value.toLocaleString('th-TH')}</AdminBadge></div>)}<AdminLinkButton href={href} tone="primary">เปิดคิว</AdminLinkButton></AdminStack></AdminCard></div>; }
