...
function safeQueueMessage(error: unknown, fallback: string) {
  if (error instanceof Error && /^(โหลดคิวไม่สำเร็จ|ทำรายการไม่สำเร็จ)$/u.test(error.message.trim())) return error.message;
  return fallback;
}
...
export default function BulkQueueOperationsPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setItems([]);
      setMessage(safeQueueMessage(error, 'โหลดคิวไม่สำเร็จ กรุณาลองใหม่'));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function executeBatch(ids: string[], reason: string, stepUpCode: string) {
    ...
    try {
      ...
      throw new Error(safeQueueMessage(error, 'ทำรายการไม่สำเร็จ กรุณาตรวจสอบข้อมูลและลองใหม่'));
    }
  }
  ...
  return <AdminPage eyebrow="Finance Workflow" title="Bulk Queue Operations" description="รับหรือปล่อยงานหลายรายการแบบมีเหตุผล คำยืนยัน และผลลัพธ์รายแถว">
    ...
    <AdminToolbar>
      <label>ประเภทคิว <select value={kind} disabled={!ready || loading} onChange={(event) => changeKind(event.target.value as QueueKind)}><option value="topups">ฝาก</option><option value="withdrawals">ถอน</option></select></label>
      <label>การทำงาน <select value={action} disabled={!ready || loading} onChange={(event) => setAction(event.target.value as QueueAction)}><option value="claim">รับงาน</option><option value="release">ปล่อยงาน</option>{kind === 'topups' ? <><option value="APPROVE_SLIP">อนุมัติสลิป</option><option value="REJECT">ปฏิเสธ</option><option value="CONFIRM_CREDIT">ยืนยันเครดิต</option></> : <><option value="APPROVE">อนุมัติจ่ายเงิน</option><option value="REJECT">ปฏิเสธ</option><option value="VERIFY_PAYMENT">ยืนยันการจ่าย</option></>}</select></label>
      <AdminButton tone="secondary" disabled={!ready || loading || items.length === 0} onClick={toggleAll}>{selected.length === items.length && items.length > 0 ? 'ยกเลิกทั้งหมด' : 'เลือกทั้งหมด'}</AdminButton>
      <AdminButton tone="secondary" disabled={!ready || loading} onClick={() => void load()}>{loading ? 'กำลังโหลด...' : 'รีเฟรช'}</AdminButton>
    </AdminToolbar>
