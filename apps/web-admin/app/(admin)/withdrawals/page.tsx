...
export default function AdminWithdrawalsPage() {
  ...
  return <AdminPage eyebrow="การเงิน" title="ตรวจรายการถอน" description="ตรวจคำขอ อนุมัติการจ่าย แนบหลักฐาน และยืนยันตัดยอดตามลำดับ" actions={<AdminButton onClick={() => void loadItems()}>รีเฟรช</AdminButton>}>
    <AdminMetricGrid><AdminMetric tone="warning" title="รอตรวจ" value={`${totals.review}`} /><AdminMetric tone="warning" title="รอโอน" value={`${totals.payment}`} /><AdminMetric tone="warning" title="รอยืนยันหลักฐาน" value={`${totals.verify}`} /><AdminMetric tone="success" title="สำเร็จ" value={`${totals.done}`} /></AdminMetricGrid>
    ...
    <AdminStack>{items.map((item) => {
      ...
      return <AdminCard key={item.id} tone={item.status === 'COMPLETED' ? 'success' : item.status === 'REJECTED' ? 'danger' : 'neutral'}><AdminSectionRow><div><AdminBadge tone={badgeTone(item.status)}>{statusLabel(item.status)}</AdminBadge><h2>{formatMoney(item.amount)}</h2><p>สมาชิก: {item.user?.username ?? item.user?.phone ?? item.userId}</p><p>{item.accountName ?? '-'} · {item.bankName ?? '-'} · {mask(item.accountNumber)}</p></div>{uploadedProof?.dataUrl && <img src={uploadedProof.dataUrl} alt="หลักฐานการโอน" className="admin-topup-modal-slip" />}</AdminSectionRow><AdminCard title="ขั้นตอนการจ่าย" description="ต้องรับงานก่อนทำรายการที่เปลี่ยนยอดเงิน"><AdminRow><strong>เลขอ้างอิง</strong><span>{uploadedProof?.transactionRef ?? item.paymentTransactionRef ?? '-'}</span></AdminRow><label className="admin-topup-note-field"><span>หมายเหตุผู้ดูแล</span><textarea value={notes[item.id] ?? ''} onChange={(event) => setNotes((current) => ({ ...current, [item.id]: event.target.value }))} placeholder="จำเป็นเมื่อปฏิเสธ" /></label></AdminCard>{!['COMPLETED', 'REJECTED', 'CANCELLED'].includes(item.status) ? <div className="admin-topup-operations"><div className="admin-topup-action-grid"><AdminButton disabled={busyId === item.id} onClick={() => void claim(item, 'claim')}>รับงาน</AdminButton><AdminButton tone="secondary" disabled={busyId === item.id || !item.claimedBy} onClick={() => void claim(item, 'release')}>ปล่อยงาน</AdminButton></div>{item.status === 'PENDING_REVIEW' && <AdminButton tone="success" disabled={busyId === item.id || !item.claimedBy} onClick={() => requestAction(item, 'approve-for-payment')}>อนุมัติให้โอนเงิน</AdminButton>}{item.status === 'APPROVED_FOR_PAYMENT' && <div className="admin-topup-action-grid"><input type="file" accept="image/*" onChange={(event) => void selectProof(item, event)} /><input value={draftProof?.transactionRef ?? ''} onChange={(event) => setProofs((current) => ({ ...current, [item.id]: { dataUrl: current[item.id]?.dataUrl ?? '', transactionRef: event.target.value } }))} placeholder="เลขอ้างอิงการโอน" /><AdminButton disabled={busyId === item.id || !item.claimedBy} onClick={() => void uploadProof(item)}>แนบหลักฐาน</AdminButton></div>}{item.status === 'PAYMENT_PROOF_UPLOADED' && <AdminButton tone="success" disabled={busyId === item.id || !item.claimedBy || !uploadedProof?.dataUrl} onClick={() => requestAction(item, 'verify-payment')}>ยืนยันหลักฐานและตัดยอด</AdminButton>}<AdminButton tone="danger" disabled={busyId === item.id || !item.claimedBy} onClick={() => requestAction(item, 'reject')}>ปฏิเสธและคืนยอดล็อก</AdminButton></div> : <AdminNotice tone="warning">รายการนี้สิ้นสุดแล้ว</AdminNotice>}</AdminCard>;
    ...
    <AdminConfirmDialog
      ...
      details={pendingAction ? <><p><strong>สมาชิก:</strong> {pendingAction.item.user?.username ?? pendingAction.item.userId}</p><p><strong>จำนวน:</strong> {formatMoney(pendingAction.item.amount)}</p><p><strong>เหตุผล:</strong> {(notes[pendingAction.item.id] ?? '').trim() || '-'}</p></> : null}
    />
  </AdminPage>;
}
...
function statusLabel(status: WithdrawalStatus) { return ({ PENDING: 'รอดำเนินการ', PENDING_REVIEW: 'รอตรวจ', APPROVED_FOR_PAYMENT: 'รอโอนเงิน', PAYMENT_PROOF_UPLOADED: 'รอยืนยันหลักฐาน', COMPLETED: 'สำเร็จ', REJECTED: 'ไม่อนุมัติ', CANCELLED: 'ยกเลิก' } as Record<WithdrawalStatus, string>)[status]; }
