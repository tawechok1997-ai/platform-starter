...
export default function AdminWithdrawalsPage() {
  ...
  async function loadItems(nextStatus = status, nextPage = page) {
    ...
    if (!res.ok) return setMessage(data?.message ?? 'โหลดรายการถอนไม่สำเร็จ');
  ...
  async function claim(item: Item, action: 'claim' | 'release') {
    ...
    setItems((current) => current.map((entry) => entry.id === item.id ? { ...entry, ...data } : entry)); setMessage(action === 'claim' ? 'รับงานตรวจถอนแล้ว' : 'ปล่อยงานแล้ว');
  }

  async function run(item: Item, action: 'approve-for-payment' | 'verify-payment' | 'reject') {
    const adminNote = (notes[item.id] ?? '').trim();
    if (action === 'reject' && !adminNote) return setMessage('กรุณาระบุเหตุผลก่อนปฏิเสธ');
    if (!window.confirm(action === 'approve-for-payment' ? 'ยืนยันให้เตรียมจ่ายเงิน?' : action === 'verify-payment' ? 'ยืนยันหลักฐานและตัดยอดจริง? การกระทำนี้ย้อนกลับไม่ได้' : 'ยืนยันปฏิเสธและคืนยอดที่ล็อก?')) return;
    setBusyId(item.id); const res = await adminApiFetch(`/admin/withdrawals/${item.id}/${action}`, { method: 'POST', body: JSON.stringify(action === 'reject' ? { adminNote } : { note: adminNote }) }); const data = await res.json().catch(() => null); setBusyId('');
    if (!res.ok) return setMessage(data?.message ?? 'ทำรายการไม่สำเร็จ');
    setMessage(action === 'approve-for-payment' ? 'อนุมัติให้เตรียมจ่ายแล้ว' : action === 'verify-payment' ? 'ตรวจหลักฐานและตัดยอดสำเร็จ' : 'ปฏิเสธและคืนยอดล็อกแล้ว'); void loadItems(status, page);
  }

  async function selectProof(item: Item, event: ChangeEvent<HTMLInputElement>) {
    const file = event.target.files?.[0]; if (!file) return; if (!file.type.startsWith('image/')) return setMessage('กรุณาเลือกไฟล์รูปภาพ');
    const reader = new FileReader(); reader.onload = () => setProofs((current) => ({ ...current, [item.id]: { dataUrl: String(reader.result), transactionRef: current[item.id]?.transactionRef ?? '' } })); reader.onerror = () => setMessage('อ่านไฟล์หลักฐานไม่สำเร็จ'); reader.readAsDataURL(file);
  }

  async function uploadProof(item: Item) {
    const proof = proofs[item.id]; if (!proof?.dataUrl || !proof.transactionRef.trim()) return setMessage('กรุณาแนบหลักฐานและระบุเลขอ้างอิงการโอน');
    ...
    if (!res.ok) return setMessage(data?.message ?? 'อัปโหลดหลักฐานไม่สำเร็จ');
    setUploadedProofs((current) => ({ ...current, [item.id]: { dataUrl: proof.dataUrl, transactionRef: proof.transactionRef } })); setMessage('อัปโหลดหลักฐานแล้ว รอตรวจยืนยัน'); void loadItems(status, page);
  }
  ...
  return <AdminPage eyebrow="Withdrawal Operations" title="ตรวจถอน" description="อนุมัติ เตรียมจ่าย แนบหลักฐาน และยืนยันการจ่ายเป็นขั้นตอน" actions={<AdminButton onClick={() => void loadItems()}>รีเฟรช</AdminButton>}>
    ...
    {message && <AdminNotice>{message}</AdminNotice>}<AdminStack>{items.map((item) => <AdminCard key={item.id} tone={item.status === 'COMPLETED' ? 'success' : item.status === 'REJECTED' ? 'danger' : 'neutral'}><AdminSectionRow><div><AdminBadge tone={badgeTone(item.status)}>{statusLabel(item.status)}</AdminBadge><h2>{formatMoney(item.amount)}</h2><p>สมาชิก: {item.user?.username ?? item.user?.phone ?? item.userId}</p><p>{item.accountName ?? '-'} · {item.bankName ?? '-'} · {mask(item.accountNumber)}</p></div>{uploadedProofs[item.id]?.dataUrl && <img src={uploadedProofs[item.id].dataUrl} alt="หลักฐานการโอน" className="admin-topup-modal-slip" />}</AdminSectionRow><AdminCard title="การดำเนินการ" description="ต้องรับงานก่อนทำขั้นตอนที่เปลี่ยนยอดเงิน"><AdminRow><strong>เลขอ้างอิงจ่าย</strong><span>{uploadedProofs[item.id]?.transactionRef ?? item.paymentTransactionRef ?? '-'}</span></AdminRow><label className="admin-topup-note-field"><span>หมายเหตุแอดมิน</span><textarea value={notes[item.id] ?? ''} onChange={(event) => setNotes((current) => ({ ...current, [item.id]: event.target.value }))} placeholder="จำเป็นเมื่อปฏิเสธ" /></label></AdminCard>{!['COMPLETED', 'REJECTED', 'CANCELLED'].includes(item.status) ? <div className="admin-topup-operations"><div className="admin-topup-action-grid"><AdminButton disabled={busyId === item.id} onClick={() => void claim(item, 'claim')}>รับงาน</AdminButton><AdminButton tone="secondary" disabled={busyId === item.id || !item.claimedBy} onClick={() => void claim(item, 'release')}>ปล่อยงาน</AdminButton></div>{item.status === 'PENDING_REVIEW' && <AdminButton tone="success" disabled={busyId === item.id || !item.claimedBy} onClick={() => void run(item, 'approve-for-payment')}>อนุมัติเตรียมจ่าย</AdminButton>}{item.status === 'APPROVED_FOR_PAYMENT' && <div className="admin-topup-action-grid"><input type="file" accept="image/*" onChange={(event) => void selectProof(item, event)} /><input value={proofs[item.id]?.transactionRef ?? ''} onChange={(event) => setProofs((current) => ({ ...current, [item.id]: { dataUrl: current[item.id]?.dataUrl ?? '', transactionRef: event.target.value } }))} placeholder="เลขอ้างอิงการโอน" /><AdminButton disabled={busyId === item.id || !item.claimedBy} onClick={() => void uploadProof(item)}>อัปโหลดหลักฐาน</AdminButton></div>}{item.status === 'PAYMENT_PROOF_UPLOADED' && <AdminButton tone="success" disabled={busyId === item.id || !item.claimedBy || !uploadedProofs[item.id]?.dataUrl} onClick={() => void run(item, 'verify-payment')}>ตรวจหลักฐานและตัดยอด</AdminButton>}<AdminButton tone="danger" disabled={busyId === item.id || !item.claimedBy} onClick={() => void run(item, 'reject')}>ปฏิเสธและคืนยอดล็อก</AdminButton></div> : <AdminNotice>รายการนี้สิ้นสุดแล้ว</AdminNotice>}</AdminCard>)}{items.length === 0 && <AdminEmpty>ไม่มีรายการในสถานะนี้</AdminEmpty>}</AdminStack>
  </AdminPage>;
}
