  ...
  return <main className="admin-auth-page">
    ...
    <section className="admin-auth-shell">
      ...
      <form onSubmit={onSubmit} className="admin-auth-card" noValidate>
        <div className="admin-auth-card-topbar"><div className="admin-auth-card-brand"><div className="admin-auth-mobile-mark" aria-hidden="true">A</div><strong>Admin Console</strong></div><div className="admin-auth-language" aria-label="Language"><button type="button" onClick={() => changeLocale('th')} aria-pressed={locale === 'th'} className="ui-button ui-button--secondary">ไทย</button><button type="button" onClick={() => changeLocale('en')} aria-pressed={locale === 'en'} className="ui-button ui-button--secondary">EN</button></div></div>
        ...
        <label className="admin-auth-field">{t.username}<input className="ui-input" value={username} onChange={(event) => setUsername(event.target.value)} autoComplete="username" disabled={loading} placeholder={t.usernamePlaceholder} /></label>
        <label className="admin-auth-field">{t.password}<div className="admin-auth-input-wrap"><input className="ui-input" value={secret} onChange={(event) => setSecret(event.target.value)} type={showSecret ? 'text' : 'password'} autoComplete="current-password" disabled={loading} placeholder={t.passwordPlaceholder} /><button type="button" onClick={() => setShowSecret((value) => !value)} disabled={loading} aria-label={showSecret ? t.hidePassword : t.showPassword} className="ui-button ui-button--secondary">{showSecret ? (locale === 'th' ? 'ซ่อน' : 'Hide') : (locale === 'th' ? 'แสดง' : 'Show')}</button></div></label>
        {requiresTwoFactor && <label className="admin-auth-field">{t.twoFactor}<span>{t.twoFactorOptional}</span><input className="ui-input" value={twoFactorCode} onChange={(event) => setTwoFactorCode(event.target.value.replace(/\D/g, '').slice(0, 8))} inputMode="numeric" autoComplete="one-time-code" disabled={loading} placeholder={t.twoFactorPlaceholder} /></label>}
        ...
        <button type="submit" disabled={submitDisabled} className="admin-auth-submit ui-button ui-button--primary">{loading ? t.submitting : t.submit}</button>
