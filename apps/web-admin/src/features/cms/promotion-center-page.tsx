...
export default function PromotionCenterPage() {
  ...
  const sorted = useMemo(
    () => [...campaigns].sort((a, b) => Number(b.priority ?? 0) - Number(a.priority ?? 0)),
    [campaigns],
  );
  ...
  return (
    ...
    >
      ...
      <AdminMetricGrid>
        <AdminMetric
          tone={stats.enabled > 0 ? 'success' : 'warning'}
          title="โปรที่เปิด"
          value={String(stats.enabled)}
          helper={`${stats.total} รายการ`}
        />
        <AdminMetric tone="success" title="Media" value={String(stats.media)} helper="มีรูปหรือไอคอน" />
        <AdminMetric tone="warning" title="Sorted by" value="Priority" helper="เลขมากขึ้นก่อน" />
        <AdminMetric
          tone={stats.warnings > 0 ? 'danger' : 'success'}
          title="Warnings"
      ...
      <AdminGrid>
          ...
          <AdminStack>
            {campaigns.map((item, index) => (
              <CampaignEditor
                ...
                onRemove={() => setCampaigns((current) => current.filter((_, i) => i !== index))}
        ...
        <AdminCard title="Member preview" description="ตัวอย่างการ์ดโปรบนมือถือ เรียงตาม priority">
          <div style={previewShellStyle}>
            {sorted
              .filter((item) => item.enabled)
              .slice(0, 4)
              .map((item) => (
                <PromotionPreview key={item.id} item={item} />
              ))}
            {sorted.filter((item) => item.enabled).length === 0 && (
              <AdminNotice>ยังไม่มีโปรที่เปิดให้สมาชิกเห็น</AdminNotice>
            )}
          </div>
        </AdminCard>
      </AdminGrid>

      <AdminCard title="Guard ก่อนเปิดเงินจริง" tone="danger">
        <AdminStack>
          <AdminRow><strong>Promotion media</strong><AdminBadge tone="success">ใช้งานจริง</AdminBadge></AdminRow>
          <AdminRow><strong>Bonus ledger</strong><AdminBadge tone="warning">แยก workflow</AdminBadge></AdminRow>
          <AdminRow><strong>Turnover tracking</strong><AdminBadge tone="warning">manual / pending auto</AdminBadge></AdminRow>
          <AdminRow><strong>Auto settlement</strong><AdminBadge tone="danger">ปิดไว้ก่อน</AdminBadge></AdminRow>
        </AdminStack>
      </AdminCard>
    ...
    <section style={editorStyle}>
      <AdminRow>
        <div>
          ...
          <p style={mutedStyle}>{item.enabled ? 'เปิดให้สมาชิกเห็น' : 'ปิดอยู่'} · priority {item.priority ?? 0}</p>
        </div>
        <div style={actionRowStyle}>
          ...
          <AdminButton tone="secondary" onClick={() => onPatch({ enabled: !item.enabled })}>{item.enabled ? 'ปิด' : 'เปิด'}</AdminButton>
    ...
    <section style={{ ...previewCardStyle, borderColor: `${accent}66`, background: `linear-gradient(135deg, ${accent}26, rgba(15,23,42,.92))` }}>
      <div style={previewMediaStyle}>
        {item.imageUrl && isValidUrl(item.imageUrl) ? (
          ...
          <div style={{ ...previewFallbackStyle, color: accent }}>
            {item.iconUrl && isValidUrl(item.iconUrl) ? <img src={item.iconUrl} alt="" style={previewIconStyle} /> : '★'}
          </div>
        )}
      </div>
...
function patchCampaign(
  ...
  setCampaigns((current) => current.map((item, i) => (i === index ? { ...item, ...patch } : item)));
}
...
function moveCampaign(
  ...
  setCampaigns((current) => {
    ...
    const sourceItem = current[index];
...
function buildWarnings(campaigns: PromotionCampaign[]): string[] {
  ...
  for (const item of campaigns) {
    ...
    ids.add(item.id);
    if (item.enabled && !item.title.trim()) warnings.push('มีโปรเปิดอยู่แต่ไม่มีชื่อ');
    if (item.enabled && !item.description.trim()) warnings.push(`โปรที่เปิดต้องมีรายละเอียด: ${item.title || item.id}`);
    ...
    if (item.enabled && item.turnoverMultiplier <= 0) warnings.push(`โปรที่เปิดต้องมีเทิร์นมากกว่า 0: ${item.title || item.id}`);
