...
import { AdminBadge, AdminButton, AdminCard, AdminConfirmDialog, AdminFilterBar, AdminGrid, AdminMetric, AdminMetricGrid, AdminNotice, AdminPage, AdminRow, AdminStack, AdminToolbar } from '../../../app/(admin)/_components/admin-ui';
...
const defaultCampaigns: PromotionCampaign[] = [{
  ...
  lifecycle: 'draft', bonusType: 'percent', bonusValue: 10, minDeposit: 100, maxBonus: 500, turnoverMultiplier: 3,
  claimMode: 'manual_review', imageUrl: '', iconUrl: '', badgeText: 'WELCOME', accentColor: '#f5c542', priority: 10,
}];
...
export default function PromotionCenterPage() {
  ...
  const sortedPreview = useMemo(() => [...campaigns].filter((item) => item.lifecycle === 'published' && item.enabled).sort((a, b) => Number(b.priority ?? 0) - Number(a.priority ?? 0)), [campaigns]);
  const stats = useMemo(() => ({
  ...
  async function load() {
    ...
    if (!res.ok) { setMessage(data?.message ?? 'โหลดโปรโมชันไม่สำเร็จ'); return; }
    const nextCampaigns = normalizeCampaigns(data?.settings?.promotion_campaigns);
    setCampaigns(nextCampaigns); setSavedCampaigns(nextCampaigns); setSelectedIds([]); setMessage('');
  }
  ...
  async function save() {
    if (warnings.length > 0) { setMessage(`ยังบันทึกไม่ได้: ${warnings.join(' • ')}`); return; }
    setSaving(true); setMessage('กำลังบันทึกโปรโมชัน...');
    const res = await adminApiFetch('/admin/settings/features', { method: 'PUT', body: JSON.stringify({ promotion_campaigns: campaigns }) });
    const data = await res.json().catch(() => null);
    setSaving(false);
    if (!res.ok) { setMessage(data?.message ?? 'บันทึกโปรโมชันไม่สำเร็จ'); return; }
    setSavedCampaigns(campaigns); setMessage('บันทึกโปรโมชันแล้ว');
  }
  ...
  function archiveSelected() {
    ...
    setSelectedIds([]); setConfirmArchive(false); setMessage(`ย้าย ${ids.size} รายการไปคลังแล้ว กรุณาบันทึกการเปลี่ยนแปลง`);
  }

  return <AdminPage eyebrow="Growth" title="Promotion Center" description="จัดการวงจรชีวิต การแสดงผล สื่อ และเงื่อนไขโปรโมชัน" actions={<><AdminSaveStateBadge state={saveState} /><AdminButton tone="secondary" onClick={requestReload} disabled={saving}>รีเฟรช</AdminButton><AdminButton onClick={() => void save()} disabled={saving || warnings.length > 0 || !isDirty}>{saving ? 'กำลังบันทึก...' : 'บันทึก'}</AdminButton></>}>
    ...
    <AdminFilterBar resultText={`${filtered.length}/${campaigns.length} รายการ`}>
      ...
      <select value={lifecycleFilter} onChange={(event) => setLifecycleFilter(event.target.value as LifecycleFilter)} style={inputStyle}>
        <option value="all">ทุกสถานะ</option><option value="draft">ฉบับร่าง</option><option value="published">เผยแพร่</option><option value="archived">เก็บถาวร</option>
      </select>
      <AdminButton tone="secondary" onClick={() => setSelectedIds(selectedVisible ? [] : filtered.map((item) => item.id))}>{selectedVisible ? 'ยกเลิกเลือกทั้งหมด' : 'เลือกทั้งหมดที่แสดง'}</AdminButton>
      <AdminButton tone="danger" disabled={selectedIds.length === 0} onClick={() => setConfirmArchive(true)}>เก็บถาวร {selectedIds.length || ''}</AdminButton>
    </AdminFilterBar>
    ...
    <AdminGrid>
      <AdminCard title="Campaigns" tone="warning">
        <AdminStack>
          {filtered.map((item) => {
            ...
            return <CampaignEditor key={item.id} item={item} index={index} count={campaigns.length} selected={selectedIds.includes(item.id)} onSelect={(selected) => setSelectedIds((current) => selected ? [...new Set([...current, item.id])] : current.filter((id) => id !== item.id))} onPatch={(patch) => patchCampaign(index, patch, setCampaigns)} onLifecycle={(lifecycle) => setLifecycle(index, lifecycle)} onRemove={() => setCampaigns((current) => current.filter((_, i) => i !== index))} onMove={(direction) => moveCampaign(index, direction, setCampaigns)} />;
          })}
      ...
      <AdminCard title="Member preview" description="แสดงเฉพาะรายการที่เผยแพร่และเปิดใช้งาน">
        <div style={previewShellStyle}>{sortedPreview.slice(0, 4).map((item) => <PromotionPreview key={item.id} item={item} />)}{sortedPreview.length === 0 && <AdminNotice>ยังไม่มีโปรที่เผยแพร่</AdminNotice>}</div>
      </AdminCard>
    </AdminGrid>
    ...
    <AdminConfirmDialog open={confirmArchive} title="เก็บโปรโมชันที่เลือก" description={`ย้าย ${selectedIds.length} รายการไปสถานะเก็บถาวร รายการจะไม่แสดงให้สมาชิกเห็น`} confirmLabel="เก็บถาวร" tone="danger" onCancel={() => setConfirmArchive(false)} onConfirm={archiveSelected} details={<p>ต้องกดบันทึกอีกครั้งจึงจะส่งผลจริง</p>} />
  </AdminPage>;
}

function CampaignEditor({ item, index, count, selected, onSelect, onPatch, onLifecycle, onRemove, onMove }: { item: PromotionCampaign; index: number; count: number; selected: boolean; onSelect: (selected: boolean) => void; onPatch: (patch: Partial<PromotionCampaign>) => void; onLifecycle: (lifecycle: PromotionLifecycle) => void; onRemove: () => void; onMove: (direction: -1 | 1) => void }) {
  ...
  return <section style={editorStyle}>
    <AdminRow><div style={headingStyle}><label style={selectStyle}><input type="checkbox" checked={selected} onChange={(event) => onSelect(event.target.checked)} /> เลือก</label><strong>{item.title || `Promotion ${index + 1}`}</strong><p style={mutedStyle}>{lifecycleLabel(item.lifecycle)} · priority {item.priority ?? 0}</p></div><div style={actionRowStyle}><AdminBadge tone={lifecycleTone(item.lifecycle)}>{lifecycleLabel(item.lifecycle)}</AdminBadge>{index > 0 && <AdminButton tone="secondary" onClick={() => onMove(-1)}>ขึ้น</AdminButton>}{index < count - 1 && <AdminButton tone="secondary" onClick={() => onMove(1)}>ลง</AdminButton>}<select value={item.lifecycle} onChange={(event) => onLifecycle(event.target.value as PromotionLifecycle)} style={compactSelectStyle}><option value="draft">ฉบับร่าง</option><option value="published">เผยแพร่</option><option value="archived">เก็บถาวร</option></select><AdminButton tone="danger" onClick={onRemove}>ลบ</AdminButton></div></AdminRow>
    <div style={fieldGridStyle}>
      <Field label="รหัสโปร" value={item.id} onChange={(value) => onPatch({ id: slug(value) })} /><Field label="ชื่อโปร" value={item.title} onChange={(value) => onPatch({ title: value })} /><Field label="Badge" value={item.badgeText ?? ''} onChange={(value) => onPatch({ badgeText: value })} /><Field label="สี Accent" value={item.accentColor ?? '#f5c542'} onChange={(value) => onPatch({ accentColor: value || '#f5c542' })} /><NumberField label="Priority" value={Number(item.priority ?? 0)} onChange={(value) => onPatch({ priority: value })} /><Field label="รูปโปรโมชัน URL" value={item.imageUrl ?? ''} onChange={(value) => onPatch({ imageUrl: value })} warning={imageWarning} /><Field label="ไอคอน URL" value={item.iconUrl ?? ''} onChange={(value) => onPatch({ iconUrl: value })} warning={iconWarning} /><Field label="รายละเอียด" value={item.description} onChange={(value) => onPatch({ description: value })} textarea />
      ...
      <Field label="เริ่ม" value={item.startsAt ?? ''} onChange={(value) => onPatch({ startsAt: value || undefined })} inputType="date" /><Field label="สิ้นสุด" value={item.endsAt ?? ''} onChange={(value) => onPatch({ endsAt: value || undefined })} inputType="date" />
    </div>{item.imageUrl && isValidUrl(item.imageUrl) && <img src={item.imageUrl} alt="" style={mediaPreviewStyle} />}
  </section>;
}
...
function Field({ label, value, onChange, textarea = false, warning, inputType = 'text' }: { label: string; value: string; onChange: (value: string) => void; textarea?: boolean; warning?: string; inputType?: 'text' | 'date' }) { return <label style={fieldStyle}><span>{label}</span>{textarea ? <textarea value={value} onChange={(event) => onChange(event.target.value)} style={textareaStyle} /> : <input type={inputType} value={value} onChange={(event) => onChange(event.target.value)} style={inputStyle} />}{warning && <small style={warningStyle}>{warning}</small>}</label>; }
...
function normalizeCampaigns(value: unknown): PromotionCampaign[] { if (!Array.isArray(value)) return defaultCampaigns; return value.map((item: Record<string, unknown>, index) => { const lifecycle: PromotionLifecycle = item.lifecycle === 'archived' ? 'archived' : item.lifecycle === 'published' || item.enabled === true ? 'published' : 'draft'; return { id: slug(item.id ?? `promotion-${index + 1}`), title: String(item.title ?? ''), description: String(item.description ?? ''), enabled: lifecycle === 'published' && item.enabled !== false, lifecycle, bonusType: item.bonusType === 'fixed' ? 'fixed' : 'percent', bonusValue: Number(item.bonusValue ?? 0), minDeposit: Number(item.minDeposit ?? 0), maxBonus: Number(item.maxBonus ?? 0), turnoverMultiplier: Number(item.turnoverMultiplier ?? 0), claimMode: item.claimMode === 'auto_pending' ? 'auto_pending' : 'manual_review', imageUrl: String(item.imageUrl ?? ''), iconUrl: String(item.iconUrl ?? ''), badgeText: String(item.badgeText ?? ''), accentColor: String(item.accentColor ?? '#f5c542'), priority: Number(item.priority ?? 0), startsAt: typeof item.startsAt === 'string' ? item.startsAt : undefined, endsAt: typeof item.endsAt === 'string' ? item.endsAt : undefined }; }); }
function createCampaign(index: number): PromotionCampaign { return { id: `promotion-${Date.now()}`, title: `โปรโมชัน ${index}`, description: '', enabled: false, lifecycle: 'draft', bonusType: 'percent', bonusValue: 10, minDeposit: 100, maxBonus: 500, turnoverMultiplier: 3, claimMode: 'manual_review', imageUrl: '', iconUrl: '', badgeText: 'NEW', accentColor: '#f5c542', priority: index * 10 }; }
