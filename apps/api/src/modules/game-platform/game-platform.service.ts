...
export class GamePlatformService {
  ...
  overview() {
    return {
      ...
      availabilityPolicy: 'Admin status and provider-reported status are evaluated separately before catalog display and launch.',
      note: 'Launch/session and wallet transfer scaffolds are enabled for registered adapters. Use provider presets and preflight before real money.',
    };
  }
  ...
  async listProviders() {
    const items = await this.prisma.gameProvider.findMany({
      ...
      include: {
        _count: { select: { endpoints: true, credentials: true, games: true, sessions: true, transfers: true, webhookLogs: true } },
      },
    });
    return {
  ...
  async getProvider(id: string) {
    ...
    return {
      ...item,
      availability: this.gameExperience.providerAvailability(item),
      readiness: this.providerReadiness(item),
      adapterRegistered: this.adapterRegistry.hasAdapter(item.code),
    };
  }
  ...
  async listGames() {
    const items = await this.prisma.game.findMany({
      orderBy: [{ sortOrder: 'asc' }, { name: 'asc' }],
      include: {
        provider: { select: { id: true, name: true, code: true, status: true, metadata: true } },
        media: { orderBy: [{ isOverride: 'desc' }, { createdAt: 'desc' }], take: 6 },
      },
    });
    ...
    const summary = {
      total: withAvailability.length,
      active: withAvailability.filter((item) => item.status === 'ACTIVE').length,
      available: withAvailability.filter((item) => item.availability.available).length,
      providerBlocked: withAvailability.filter((item) => item.status === 'ACTIVE' && !item.availability.available).length,
      featured: withAvailability.filter((item) => item.isFeatured).length,
      popular: withAvailability.filter((item) => item.isPopular).length,
    };
    return { items: withAvailability, summary };
  }

  async getGame(id: string) {
    const item = await this.prisma.game.findUnique({
      where: { id },
      include: {
        provider: { select: { id: true, name: true, code: true, status: true, metadata: true } },
        media: { orderBy: [{ isOverride: 'desc' }, { createdAt: 'desc' }] },
      },
    });
    if (!item) throw new NotFoundException('Game not found');
    return { ...item, availability: this.gameExperience.gameAvailability(item) };
  }

  async createGame(input: NormalizedGameInput, actor: AdminActor, meta: RequestMeta) {
    await this.assertProvider(input.providerId);
    try {
      const item = await this.prisma.game.create({
        data: this.gameData(input) as Prisma.GameUncheckedCreateInput,
        include: { provider: { select: { id: true, name: true, code: true, status: true, metadata: true } } },
      });
      await this.audit(actor, 'game.create', item.id, null, this.safeJson(item), meta);
      return { ...item, availability: this.gameExperience.gameAvailability(item) };
    } catch (error) {
      this.handleProviderWriteError(error, 'Game code already exists for this provider');
    }
  }

  async updateGame(id: string, input: Partial<NormalizedGameInput>, actor: AdminActor, meta: RequestMeta) {
    const current = await this.prisma.game.findUnique({ where: { id } });
    if (!current) throw new NotFoundException('Game not found');
    if (input.providerId) await this.assertProvider(input.providerId);
    try {
      const item = await this.prisma.game.update({
        where: { id },
        data: this.gameData(input) as Prisma.GameUpdateInput,
        include: { provider: { select: { id: true, name: true, code: true, status: true, metadata: true } } },
      });
      await this.audit(actor, 'game.update', item.id, this.safeJson(current), this.safeJson(item), meta);
      return { ...item, availability: this.gameExperience.gameAvailability(item) };
    } catch (error) {
      this.handleProviderWriteError(error, 'Game code already exists for this provider');
    }
  ...
  async createGameMedia(gameId: string, input: NormalizedGameMediaInput, actor: AdminActor, meta: RequestMeta) {
    const game = await this.prisma.game.findUnique({ where: { id: gameId }, select: { id: true, providerId: true } });
    if (!game) throw new NotFoundException('Game not found');
    const item = await this.prisma.gameMedia.create({
      data: { gameId, providerId: game.providerId, ...this.mediaData(input) } as Prisma.GameMediaUncheckedCreateInput,
    });
    await this.audit(actor, 'game_media.create', item.id, null, this.safeJson(item), meta);
    return item;
  }

  async updateGameMedia(gameId: string, mediaId: string, input: Partial<NormalizedGameMediaInput>, actor: AdminActor, meta: RequestMeta) {
    const current = await this.prisma.gameMedia.findFirst({ where: { id: mediaId, gameId } });
    if (!current) throw new NotFoundException('Game media not found');
    const item = await this.prisma.gameMedia.update({ where: { id: mediaId }, data: this.mediaData(input) as Prisma.GameMediaUpdateInput });
    await this.audit(actor, 'game_media.update', item.id, this.safeJson(current), this.safeJson(item), meta);
    return item;
  }

  async syncGamesDryRun(providerId: string, actor: AdminActor, meta: RequestMeta) {
    const provider = await this.prisma.gameProvider.findUnique({
      where: { id: providerId },
      include: {
        endpoints: { where: { isEnabled: true }, orderBy: { type: 'asc' } },
        credentials: { where: { isEnabled: true }, orderBy: { type: 'asc' }, select: this.credentialSecretSelect() },
      },
    });
    ...
    const result = await adapter.syncGames(this.buildAdapterContext(provider));
    if (!result.ok || !result.payload) {
      await this.audit(actor, 'game_provider.sync_games.failed', provider.id, null, this.safeJson({ ok: result.ok, errorCode: result.errorCode, errorMessage: result.errorMessage }), meta);
      return { ok: false, providerCode: provider.code, created: 0, updated: 0, skipped: 0, errorCode: result.errorCode, errorMessage: result.errorMessage };
    }
    let created = 0;
    let updated = 0;
    let skipped = 0;
    for (const payload of result.payload) {
      const outcome = await this.upsertProviderGame(provider.id, payload);
    ...
    await this.audit(actor, 'game_provider.sync_games', provider.id, null, this.safeJson(response), meta);
    return response;
  }

  async listMemberGames() {
    const candidates = await this.prisma.game.findMany({
      where: { status: 'ACTIVE', provider: { status: 'ACTIVE' } },
      orderBy: [{ isFeatured: 'desc' }, { isPopular: 'desc' }, { sortOrder: 'asc' }, { name: 'asc' }],
      include: {
        provider: { select: { id: true, name: true, code: true, status: true, metadata: true } },
        media: {
    ...
    for (const item of items) {
      const current = categoryOrder.get(item.category);
      if (current === undefined || item.sortOrder < current) categoryOrder.set(item.category, item.sortOrder);
    }
    const categories = Array.from(categoryOrder.entries())
    ...
    return {
      ...
      newest: items.filter((item) => item.isNew),
      popular: items.filter((item) => item.isPopular),
    };
  }

  async launchMemberGame(gameId: string, actor: MemberActor, meta: RequestMeta) {
    ...
    if (!game || !this.gameExperience.isGameAvailable(game)) throw new NotFoundException('Game is not available');

    const session = await this.prisma.gameSession.create({
      data: { userId: actor.id, providerId: game.providerId, gameId: game.id, status: 'CREATED', ipAddress: meta.ipAddress, userAgent: meta.userAgent },
    });
    const adapter = this.adapterRegistry.getAdapter(game.provider.code);
    const result = await adapter.launchGame(this.buildAdapterContext(game.provider), {
      userId: actor.id,
    ...
    if (!result.ok || !result.payload?.launchUrl) {
      const failed = await this.prisma.gameSession.update({
        where: { id: session.id },
        data: { status: 'FAILED', errorCode: result.errorCode ?? 'LAUNCH_FAILED', errorMessage: result.errorMessage ?? 'Provider launch failed' },
      });
      return { ok: false, session: failed, errorCode: failed.errorCode, errorMessage: failed.errorMessage };
    }
    const launched = await this.prisma.gameSession.update({
      where: { id: session.id },
      data: {
        status: 'LAUNCHED',
        launchUrl: result.payload.launchUrl,
        providerSessionId: result.payload.providerSessionId,
        startedAt: new Date(),
      },
      include: {
  ...
  async listGameSessions() {
    const items = await this.prisma.gameSession.findMany({
      ...
      include: {
        ...
        transfers: { select: { id: true, type: true, status: true, amount: true, currency: true, createdAt: true }, take: 10, orderBy: { createdAt: 'desc' } },
      },
    });
    return {
      items,
  ...
  async getGameSession(id: string) {
    const item = await this.prisma.gameSession.findUnique({
      where: { id },
      include: {
        user: { select: { id: true, username: true, phone: true } },
        provider: { select: { id: true, name: true, code: true } },
    ...
    if (!item) throw new NotFoundException('Game session not found');
    return item;
  }
  ...
  async healthCheckProvider(providerId: string, actor: AdminActor, meta: RequestMeta) {
    ...
    const providerMetadata = this.gameExperience.mergeProviderHealthMetadata(provider.metadata, result, checkedAt);
    await this.prisma.gameProvider.update({ where: { id: provider.id }, data: { metadata: providerMetadata } });
    const response = {
      ok: result.ok,
      ...
      readiness,
      availability: this.gameExperience.providerAvailability({ status: provider.status, metadata: providerMetadata }),
  ...
  private async upsertProviderGame(providerId: string, payload: ProviderGamePayload) {
    if (!payload.providerGameCode || !payload.name || !payload.category) return 'skipped';
    const existing = await this.prisma.game.findUnique({
      where: { providerId_providerGameCode: { providerId, providerGameCode: payload.providerGameCode } },
      select: { id: true, metadata: true },
    });
    const metadata = this.gameExperience.mergeGameSyncMetadata(existing?.metadata, payload);
    ...
    await this.upsertSyncedMedia(game.id, providerId, payload);
    return existing ? 'updated' : 'created';
  }

  private async upsertSyncedMedia(gameId: string, providerId: string, payload: ProviderGamePayload) {
    const media = [
      { type: 'COVER', url: payload.imageUrl },
      { type: 'ICON', url: payload.iconUrl },
      { type: 'BANNER', url: payload.bannerUrl },
    ].filter((item): item is { type: 'COVER' | 'ICON' | 'BANNER'; url: string } => Boolean(item.url));
    for (const item of media) {
      const current = await this.prisma.gameMedia.findFirst({ where: { gameId, type: item.type, isOverride: false }, select: { id: true } });
      const data = {
        gameId,
        providerId,
        type: item.type,
        ...
        isOverride: false,
        metadata: { source: 'adapter-sync' },
      };
      if (current) await this.prisma.gameMedia.update({ where: { id: current.id }, data });
      else await this.prisma.gameMedia.create({ data });
    }
  }
  ...
  private gameData(input: Partial<NormalizedGameInput>) {
    const data: Record<string, unknown> = {};
    if (input.providerId !== undefined) data.providerId = input.providerId;
    if (input.providerGameCode !== undefined) data.providerGameCode = input.providerGameCode;
  ...
  private mediaData(input: Partial<NormalizedGameMediaInput>) {
    ...
    if (input.cachedUrl !== undefined) data.cachedUrl = input.cachedUrl;
    if (input.status !== undefined) data.status = input.status;
    if (input.isOverride !== undefined) data.isOverride = input.isOverride;
    if (input.metadata !== undefined) data.metadata = input.metadata as Prisma.InputJsonValue;
    return data;
  }
  ...
  private buildAdapterContext(provider: {
    ...
    credentials: Array<{ type: string; maskedValue: string; encryptedValue?: string }>;
  ...
  private providerReadiness(provider: {
    status: string;
    code: string;
    endpoints?: Array<{ type: GameProviderEndpointType; isEnabled: boolean }>;
    credentials?: Array<{ type: string; isEnabled: boolean }>;
  }) {
  ...
  private credentialSelect() {
    return { id: true, providerId: true, type: true, maskedValue: true, isEnabled: true, rotatedAt: true, lastUsedAt: true, createdAt: true, updatedAt: true } as const;
  }

  private credentialSecretSelect() {
    return { id: true, providerId: true, type: true, maskedValue: true, encryptedValue: true, isEnabled: true, rotatedAt: true, lastUsedAt: true, createdAt: true, updatedAt: true } as const;
  }
  ...
  private decryptSecret(value: string) {
    ...
    const decipher = createDecipheriv('aes-256-gcm', this.credentialKey(), Buffer.from(ivRaw, 'base64'));
  ...
  private async markCredentialUse(providerId: string) {
    await this.prisma.gameProviderCredential.updateMany({ where: { providerId, isEnabled: true }, data: { lastUsedAt: new Date() } });
  }
