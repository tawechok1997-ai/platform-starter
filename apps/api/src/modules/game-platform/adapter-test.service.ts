...
export class AdapterTestService {
  ...
  async run(providerId: string, method: TestMethod, payload: Record<string, unknown> = {}) {
    ...
    if ((method === 'transferIn' || method === 'transferOut') && !this.isSafeTransferTestProvider(provider.code)) {
      throw new BadRequestException('Transfer adapter tests are restricted to demo/simulator providers');
    }
    let result: unknown;
  ...
  private isSafeTransferTestProvider(providerCode: string) { return new Set(['demo-provider', 'demo-provider-uat', 'simulator-provider']).has(providerCode.trim().toLowerCase()); }
