...
type RoundRow = { providerRoundId: string; state: string; betTransactionId: string | null };
...
describeWithDatabase('game round persistence concurrency with PostgreSQL', () => {
  ...
  beforeAll(async () => {
    ...
    await prisma.gameProvider.create({ data: { id: providerId, name: `Round Test ${suffix}`, code: `round-test-${suffix}`, status: 'ACTIVE', walletMode: 'SEAMLESS' } });
  }, 30_000);
  ...
  it('accepts only one of two conflicting bets for the same provider round', async () => {
    ...
    const event = (transactionId: string) => ({ eventType: 'BET', providerTransactionId: transactionId, roundId, payload: { amount: '100.00' } });
    ...
    const rows = await prisma.$queryRaw<RoundRow[]>`
      SELECT "provider_round_id" AS "providerRoundId", "state", "bet_transaction_id" AS "betTransactionId"
      FROM "game_rounds"
      WHERE "provider_id" = ${providerId}::uuid AND "provider_round_id" = ${roundId}
    `;
  ...
  it('replays the same concurrent transaction without creating another round', async () => {
    ...
    const event = { eventType: 'PLACE_BET', providerTransactionId: 'same-bet', roundId, payload: { amount: '50.00' } };
