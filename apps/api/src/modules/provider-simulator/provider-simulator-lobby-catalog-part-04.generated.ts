import type { SimulatorGameCatalogItem } from './provider-simulator-catalog';\n\nexport const LOBBY_GAME_CATALOG_PART_04: readonly SimulatorGameCatalogItem[] = [
  {
    "code": "5915",
    "name": "Koi Bingo",
    "provider": "jkgx2.png",
    "platform": "pc",
    "category": "arcade",
    "accent": "#0ea5e9",
    "symbol": "KO",
    "tags": [
      "เกมอาเขต",
      "เกมใหม่"
    ]
  },
  {
    "code": "1075",
    "name": "Scratch Match",
    "provider": "evp.png",
    "platform": "pc",
    "category": "arcade",
    "accent": "#ca8a04",
    "symbol": "SC",
    "tags": [
      "เกมอาเขต",
      "เกมฮิต"
    ]
  },
  {
    "code": "7153",
    "name": "Bac Ball 1",
    "provider": "hotdog.png",
    "platform": "pc",
    "category": "arcade",
    "accent": "#7c3aed",
    "symbol": "BA",
    "tags": [
      "เกมอาเขต"
    ]
  },
  {
    "code": "5193",
    "name": "ลุยรับทรัพย์",
    "provider": "fachai.png",
    "platform": "pc",
    "category": "arcade",
    "accent": "#4f46e5",
    "symbol": "ลุ",
    "tags": [
      "เกมอาเขต",
      "เกมใหม่"
    ]
  },
  {
    "code": "3770",
    "name": "Candyland Bingo",
    "provider": "jl.png",
    "platform": "pc",
    "category": "arcade",
    "accent": "#0ea5e9",
    "symbol": "CA",
    "tags": [
      "เกมอาเขต",
      "เกมใหม่"
    ]
  },
  {
    "code": "537",
    "name": "Save The Hamster",
    "provider": "evp.png",
    "platform": "pc",
    "category": "arcade",
    "accent": "#ca8a04",
    "symbol": "SA",
    "tags": [
      "เกมอาเขต"
    ]
  },
  {
    "code": "5175",
    "name": "Real Slap",
    "provider": "ask.png",
    "platform": "pc",
    "category": "arcade",
    "accent": "#16a34a",
    "symbol": "RE",
    "tags": [
      "เกมอาเขต"
    ]
  },
  {
    "code": "3677",
    "name": "Monkey Climb Tree",
    "provider": "r88.png",
    "platform": "pc",
    "category": "arcade",
    "accent": "#0891b2",
    "symbol": "MO",
    "tags": [
      "เกมอาเขต"
    ]
  },
  {
    "code": "3703",
    "name": "CockFighting",
    "provider": "r88.png",
    "platform": "pc",
    "category": "arcade",
    "accent": "#0891b2",
    "symbol": "CO",
    "tags": [
      "เกมอาเขต"
    ]
  },
  {
    "code": "1174",
    "name": "Dragon Ball Dozer",
    "provider": "cq.png",
    "platform": "pc",
    "category": "arcade",
    "accent": "#4f46e5",
    "symbol": "DR",
    "tags": [
      "เกมอาเขต"
    ]
  },
  {
    "code": "5041",
    "name": "Bounty Smash",
    "provider": "n2.png",
    "platform": "pc",
    "category": "arcade",
    "accent": "#ea580c",
    "symbol": "BO",
    "tags": [
      "เกมอาเขต"
    ]
  },
  {
    "code": "3776",
    "name": "Penalty",
    "provider": "glx.png",
    "platform": "pc",
    "category": "arcade",
    "accent": "#ca8a04",
    "symbol": "PE",
    "tags": [
      "เกมอาเขต"
    ]
  },
  {
    "code": "460",
    "name": "Football Manager",
    "provider": "evp.png",
    "platform": "pc",
    "category": "arcade",
    "accent": "#ca8a04",
    "symbol": "FO",
    "tags": [
      "เกมอาเขต"
    ]
  },
  {
    "code": "3034",
    "name": "Won Won Catching",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "arcade",
    "accent": "#4f46e5",
    "symbol": "WO",
    "tags": [
      "เกมอาเขต"
    ]
  },
  {
    "code": "2545",
    "name": "อาแปะดันเหรียญ",
    "provider": "fachai.png",
    "platform": "pc",
    "category": "arcade",
    "accent": "#4f46e5",
    "symbol": "อา",
    "tags": [
      "เกมอาเขต"
    ]
  },
  {
    "code": "5874",
    "name": "Mahjong 3+",
    "provider": "r88.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#0891b2",
    "symbol": "MA",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมใหม่",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4617",
    "name": "Queen Femida",
    "provider": "l22.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#ea580c",
    "symbol": "QU",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5871",
    "name": "Mahjong Wins 2",
    "provider": "pp.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#ea580c",
    "symbol": "MA",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4323",
    "name": "Clash of the Giants",
    "provider": "spg.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#0891b2",
    "symbol": "CL",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมใหม่",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5021",
    "name": "Star Kingdom",
    "provider": "n2.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#ea580c",
    "symbol": "ST",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5082",
    "name": "Crazy Bounty",
    "provider": "ap.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#db2777",
    "symbol": "CR",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "1776",
    "name": "Roma",
    "provider": "spg.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#0891b2",
    "symbol": "RO",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "1397",
    "name": "Dead or Alive 2 Feature Buy",
    "provider": "netent.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#0ea5e9",
    "symbol": "DE",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2777",
    "name": "Marching Legions",
    "provider": "reg.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#0ea5e9",
    "symbol": "MA",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5269",
    "name": "El Paso Gunfight xNudge",
    "provider": "nlc.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#7c3aed",
    "symbol": "EL",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4999",
    "name": "The King Of Olympus",
    "provider": "pgsus.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#0891b2",
    "symbol": "TH",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2760",
    "name": "Happy Farm",
    "provider": "rsg.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#4f46e5",
    "symbol": "HA",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4569",
    "name": "10000 BC Doublemax",
    "provider": "ygg.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#16a34a",
    "symbol": "10",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5023",
    "name": "Super Sundae",
    "provider": "n2.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#ea580c",
    "symbol": "SU",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2427",
    "name": "Crystal Skull",
    "provider": "edp.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#db2777",
    "symbol": "CR",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมใหม่",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2479",
    "name": "Asgardian Rising",
    "provider": "pgsoft.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#ca8a04",
    "symbol": "AS",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมฮิต",
      "เกมใหม่",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5194",
    "name": "Football Fever",
    "provider": "ap.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#db2777",
    "symbol": "FO",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมใหม่",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5201",
    "name": "Legendary Shaman",
    "provider": "ygr.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#0891b2",
    "symbol": "LE",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมใหม่",
      "เกมสล็อต"
    ]
  },
  {
    "code": "3520",
    "name": "Cash Box",
    "provider": "pp.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#ea580c",
    "symbol": "CA",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมใหม่",
      "เกมสล็อต"
    ]
  },
  {
    "code": "3666",
    "name": "3 Buzzing Wilds",
    "provider": "pp.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#ea580c",
    "symbol": "3B",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมใหม่",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4023",
    "name": "Gold Oasis",
    "provider": "pp.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#ea580c",
    "symbol": "GO",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมใหม่",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4134",
    "name": "Demon Pots",
    "provider": "pp.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#ea580c",
    "symbol": "DE",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมใหม่",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4331",
    "name": "Fortune Ace",
    "provider": "pp.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#ea580c",
    "symbol": "FO",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมใหม่",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4339",
    "name": "Mighty Munching Melons",
    "provider": "pp.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#ea580c",
    "symbol": "MI",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมใหม่",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4648",
    "name": "Cyber Heist",
    "provider": "pp.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#ea580c",
    "symbol": "CY",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมใหม่",
      "เกมสล็อต"
    ]
  },
  {
    "code": "656",
    "name": "Cycle of Luck Bonus Buy",
    "provider": "evp.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#ca8a04",
    "symbol": "CY",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "3587",
    "name": "Boom! Boom! Gold!",
    "provider": "bng.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#16a34a",
    "symbol": "BO",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4104",
    "name": "Dragon Egg",
    "provider": "ygr.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#0891b2",
    "symbol": "DR",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4114",
    "name": "777 More Cash",
    "provider": "ygr.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#0891b2",
    "symbol": "77",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5145",
    "name": "Centillion Gods",
    "provider": "ask.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#16a34a",
    "symbol": "CE",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5154",
    "name": "Wonder Cuisine",
    "provider": "ask.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#16a34a",
    "symbol": "WO",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5166",
    "name": "Ghost House",
    "provider": "ask.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#16a34a",
    "symbol": "GH",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2366",
    "name": "Geisha",
    "provider": "edp.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#db2777",
    "symbol": "GE",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2377",
    "name": "Urartu",
    "provider": "edp.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#db2777",
    "symbol": "UR",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2385",
    "name": "The Ninja",
    "provider": "edp.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#db2777",
    "symbol": "TH",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4949",
    "name": "Wild Wet Win",
    "provider": "fs.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#db2777",
    "symbol": "WI",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4955",
    "name": "Muay Thai Fighter",
    "provider": "fs.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#db2777",
    "symbol": "MU",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2400",
    "name": "Football Superstar",
    "provider": "edp.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#db2777",
    "symbol": "FO",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4453",
    "name": "Legion Hot1",
    "provider": "ygg.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#16a34a",
    "symbol": "LE",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2668",
    "name": "Moonshot",
    "provider": "pp.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#ea580c",
    "symbol": "MO",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2677",
    "name": "Wild Celebrity Bus Megaways",
    "provider": "pp.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#ea580c",
    "symbol": "WI",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4477",
    "name": "Hunters Moon",
    "provider": "ygg.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#16a34a",
    "symbol": "HU",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5762",
    "name": "Make You Rich",
    "provider": "drag.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#0ea5e9",
    "symbol": "MA",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4486",
    "name": "Super Cash Drop Giga Blox",
    "provider": "ygg.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#16a34a",
    "symbol": "SU",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5258",
    "name": "Evil Goblins xBomb",
    "provider": "nlc.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#7c3aed",
    "symbol": "EV",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5262",
    "name": "Gaelic Gold",
    "provider": "nlc.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#7c3aed",
    "symbol": "GA",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5011",
    "name": "Disco Dive",
    "provider": "n2.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#ea580c",
    "symbol": "DI",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2967",
    "name": "Blackbeard Legacy",
    "provider": "wmslot.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#0ea5e9",
    "symbol": "BL",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5017",
    "name": "Pearly Shores",
    "provider": "n2.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#ea580c",
    "symbol": "PE",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5020",
    "name": "Spirit Blast",
    "provider": "n2.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#ea580c",
    "symbol": "SP",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2976",
    "name": "Lucky Rooster",
    "provider": "wmslot.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#0ea5e9",
    "symbol": "LU",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2978",
    "name": "Caishen Riches",
    "provider": "wmslot.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#0ea5e9",
    "symbol": "CA",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4005",
    "name": "Starlight Christmas",
    "provider": "pp.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#ea580c",
    "symbol": "ST",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2473",
    "name": "Cupid",
    "provider": "edp.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#db2777",
    "symbol": "CU",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4525",
    "name": "Firekick! Multimax",
    "provider": "ygg.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#16a34a",
    "symbol": "FI",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5040",
    "name": "Royal Ribbit",
    "provider": "n2.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#ea580c",
    "symbol": "RO",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5301",
    "name": "The Border",
    "provider": "nlc.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#7c3aed",
    "symbol": "TH",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5050",
    "name": "Doge Life",
    "provider": "n2.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#ea580c",
    "symbol": "DO",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4286",
    "name": "Sugar Rush Xmas",
    "provider": "pp.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#ea580c",
    "symbol": "SU",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2242",
    "name": "Lost Treasure",
    "provider": "pokslot.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#4f46e5",
    "symbol": "LO",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "1990",
    "name": "Party Night",
    "provider": "jl.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#0ea5e9",
    "symbol": "PA",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4555",
    "name": "Giganimals Gigablox",
    "provider": "ygg.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#16a34a",
    "symbol": "GI",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4558",
    "name": "Candyfinity",
    "provider": "ygg.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#16a34a",
    "symbol": "CA",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4561",
    "name": "Diamond Symphony Doublemax",
    "provider": "ygg.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#16a34a",
    "symbol": "DI",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4565",
    "name": "Fast Fruits Doublemax",
    "provider": "ygg.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#16a34a",
    "symbol": "FA",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5337",
    "name": "Brick Snake 2000",
    "provider": "nlc.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#7c3aed",
    "symbol": "BR",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2786",
    "name": "Cluster Tumble",
    "provider": "reg.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#0ea5e9",
    "symbol": "CL",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4583",
    "name": "Wild Stack Frenzy",
    "provider": "ygg.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#16a34a",
    "symbol": "WI",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4589",
    "name": "Diggin for Diamonds",
    "provider": "ygg.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#16a34a",
    "symbol": "DI",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4082",
    "name": "Fish Prawn Crab",
    "provider": "ygr.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#0891b2",
    "symbol": "FI",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4343",
    "name": "Power of Ninja",
    "provider": "pp.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#ea580c",
    "symbol": "PO",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "1020",
    "name": "fruit party",
    "provider": "pp.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#ea580c",
    "symbol": "FR",
    "tags": [
      "ซื้อฟรีสปิน",
      "เกมสล็อต"
    ]
  },
  {
    "code": "7163",
    "name": "Wonders of Giza Pyramid",
    "provider": "l22.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#ea580c",
    "symbol": "WO",
    "tags": [
      "เกมฮิต"
    ]
  },
  {
    "code": "3039",
    "name": "Bonus Mania",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#4f46e5",
    "symbol": "BO",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "424",
    "name": "Hot Triple Sevens",
    "provider": "evp.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#ca8a04",
    "symbol": "HO",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5427",
    "name": "Hot Fruits",
    "provider": "jkgx2.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#0ea5e9",
    "symbol": "HO",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "1994",
    "name": "RomaX",
    "provider": "jl.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#0ea5e9",
    "symbol": "RO",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2910",
    "name": "Fa Fa Spin",
    "provider": "wmslot.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#0ea5e9",
    "symbol": "FA",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "1452",
    "name": "Parthenon: Quest for Immortality",
    "provider": "netent.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#0ea5e9",
    "symbol": "PA",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2581",
    "name": "7 สูง 7 ต่ำ",
    "provider": "kingm.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#0ea5e9",
    "symbol": "7ส",
    "tags": [
      "เกมฮิต"
    ]
  },
  {
    "code": "5805",
    "name": "Gold Heist",
    "provider": "drag.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#0ea5e9",
    "symbol": "GO",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2133",
    "name": "Fortune Ox",
    "provider": "pgsoft.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#ca8a04",
    "symbol": "FO",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "3050",
    "name": "Treasure Tiger",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#4f46e5",
    "symbol": "TR",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5454",
    "name": "Neptune Bingo",
    "provider": "jkgx2.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#0ea5e9",
    "symbol": "NE",
    "tags": [
      "เกมฮิต",
      "เกมโต๊ะ"
    ]
  },
  {
    "code": "3546",
    "name": "Tiger Jungle",
    "provider": "bng.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#16a34a",
    "symbol": "TI",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "1942",
    "name": "Boxing King",
    "provider": "jl.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#0ea5e9",
    "symbol": "BO",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "1853",
    "name": "FACE OFF",
    "provider": "ps.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#ca8a04",
    "symbol": "FA",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "991",
    "name": "Jugglenaut",
    "provider": "hbn.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#ea580c",
    "symbol": "JU",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2716",
    "name": "72 Changes",
    "provider": "rsg.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#4f46e5",
    "symbol": "72",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2632",
    "name": "Santa Spins",
    "provider": "redtiger.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#0ea5e9",
    "symbol": "SA",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5126",
    "name": "Battle Royal",
    "provider": "amb.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#ea580c",
    "symbol": "BA",
    "tags": [
      "เกมฮิต",
      "เกมโต๊ะ"
    ]
  },
  {
    "code": "5312",
    "name": "Warrior Graveyard xNudge",
    "provider": "nlc.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#7c3aed",
    "symbol": "WA",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต",
      "ซื้อฟรีสปิน"
    ]
  },
  {
    "code": "2898",
    "name": "Dynamite Jack",
    "provider": "wmslot.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#0ea5e9",
    "symbol": "DY",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2919",
    "name": "Space Blast",
    "provider": "wmslot.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#0ea5e9",
    "symbol": "SP",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2753",
    "name": "Royal 7777",
    "provider": "rsg.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#4f46e5",
    "symbol": "RO",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2723",
    "name": "China Empress",
    "provider": "rsg.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#4f46e5",
    "symbol": "CH",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2699",
    "name": "Jungle",
    "provider": "rsg.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#4f46e5",
    "symbol": "JU",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4004",
    "name": "Mahjong Wins",
    "provider": "pp.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#ea580c",
    "symbol": "MA",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2461",
    "name": "2021 Hit Slot",
    "provider": "edp.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#db2777",
    "symbol": "20",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4424",
    "name": "Lucky Neko",
    "provider": "ygg.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#16a34a",
    "symbol": "LU",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5573",
    "name": "Archer",
    "provider": "vp.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#0ea5e9",
    "symbol": "AR",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2696",
    "name": "Pharaoh",
    "provider": "rsg.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#4f46e5",
    "symbol": "PH",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "3045",
    "name": "Fortune Ganesha",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "hot",
    "accent": "#4f46e5",
    "symbol": "FO",
    "tags": [
      "เกมฮิต",
      "เกมสล็อต"
    ]
  },
  {
    "code": "8369",
    "name": "Storm of Seth 2",
    "provider": "jl.png",
    "platform": "pc",
    "category": "new",
    "accent": "#0ea5e9",
    "symbol": "ST",
    "tags": [
      "เกมใหม่"
    ]
  },
  {
    "code": "6163",
    "name": "King Craft: Menomin",
    "provider": "pokslot.png",
    "platform": "pc",
    "category": "new",
    "accent": "#4f46e5",
    "symbol": "KI",
    "tags": [
      "เกมใหม่"
    ]
  },
  {
    "code": "6175",
    "name": "Rico Gorila",
    "provider": "pokslot.png",
    "platform": "pc",
    "category": "new",
    "accent": "#4f46e5",
    "symbol": "RI",
    "tags": [
      "เกมใหม่"
    ]
  },
  {
    "code": "7227",
    "name": "TREASURES OF ODIN",
    "provider": "fachai.png",
    "platform": "pc",
    "category": "new",
    "accent": "#4f46e5",
    "symbol": "TR",
    "tags": [
      "เกมใหม่"
    ]
  },
  {
    "code": "8021",
    "name": "Aztec Empire 1000",
    "provider": "ask.png",
    "platform": "pc",
    "category": "new",
    "accent": "#16a34a",
    "symbol": "AZ",
    "tags": [
      "เกมใหม่"
    ]
  },
  {
    "code": "8289",
    "name": "Yummy Dumplings: Hold The Spin",
    "provider": "hotdog.png",
    "platform": "pc",
    "category": "new",
    "accent": "#7c3aed",
    "symbol": "YU",
    "tags": [
      "เกมใหม่"
    ]
  },
  {
    "code": "6005",
    "name": "Age of Glory",
    "provider": "edp.png",
    "platform": "pc",
    "category": "new",
    "accent": "#db2777",
    "symbol": "AG",
    "tags": [
      "เกมใหม่",
      "เกมสล็อต"
    ]
  },
  {
    "code": "7549",
    "name": "Mystic Shaman",
    "provider": "hbn.png",
    "platform": "pc",
    "category": "new",
    "accent": "#ea580c",
    "symbol": "MY",
    "tags": [
      "เกมใหม่"
    ]
  },
  {
    "code": "7557",
    "name": "Burmese 6 Animals",
    "provider": "kingm.png",
    "platform": "pc",
    "category": "new",
    "accent": "#0ea5e9",
    "symbol": "BU",
    "tags": [
      "เกมใหม่"
    ]
  },
  {
    "code": "8332",
    "name": "3x5 Double Blazing",
    "provider": "hotdog.png",
    "platform": "pc",
    "category": "new",
    "accent": "#7c3aed",
    "symbol": "3X",
    "tags": [
      "เกมใหม่"
    ]
  },
  {
    "code": "8337",
    "name": "9 Jalapenos",
    "provider": "hotdog.png",
    "platform": "pc",
    "category": "new",
    "accent": "#7c3aed",
    "symbol": "9J",
    "tags": [
      "เกมใหม่"
    ]
  },
  {
    "code": "8345",
    "name": "40 Chilli Fruits Superior",
    "provider": "hotdog.png",
    "platform": "pc",
    "category": "new",
    "accent": "#7c3aed",
    "symbol": "40",
    "tags": [
      "เกมใหม่"
    ]
  },
  {
    "code": "7589",
    "name": "Chill Penguin",
    "provider": "ygr.png",
    "platform": "pc",
    "category": "new",
    "accent": "#0891b2",
    "symbol": "CH",
    "tags": [
      "เกมใหม่"
    ]
  },
  {
    "code": "7886",
    "name": "Mahjong Rich",
    "provider": "hotdog.png",
    "platform": "pc",
    "category": "new",
    "accent": "#7c3aed",
    "symbol": "MA",
    "tags": [
      "เกมใหม่"
    ]
  },
  {
    "code": "7901",
    "name": "Sun of Egypt 5: Hold and Win",
    "provider": "bng.png",
    "platform": "pc",
    "category": "new",
    "accent": "#16a34a",
    "symbol": "SU",
    "tags": [
      "เกมใหม่"
    ]
  },
  {
    "code": "7144",
    "name": "Mahjong Rich Ways",
    "provider": "cq.png",
    "platform": "pc",
    "category": "new",
    "accent": "#4f46e5",
    "symbol": "MA",
    "tags": [
      "เกมใหม่"
    ]
  },
  {
    "code": "6147",
    "name": "Mahjong Legacy",
    "provider": "ask.png",
    "platform": "pc",
    "category": "new",
    "accent": "#16a34a",
    "symbol": "MA",
    "tags": [
      "เกมใหม่",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5902",
    "name": "Outsourced",
    "provider": "nlc.png",
    "platform": "pc",
    "category": "new",
    "accent": "#7c3aed",
    "symbol": "OU",
    "tags": [
      "เกมใหม่",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5880",
    "name": "Mahjong Wins 3 - Black Scatter",
    "provider": "pp.png",
    "platform": "pc",
    "category": "new",
    "accent": "#ea580c",
    "symbol": "MA",
    "tags": [
      "เกมใหม่"
    ]
  },
  {
    "code": "4353",
    "name": "Luchadors",
    "provider": "rsg.png",
    "platform": "pc",
    "category": "new",
    "accent": "#4f46e5",
    "symbol": "LU",
    "tags": [
      "เกมใหม่",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2262",
    "name": "Los Toros",
    "provider": "pokslot.png",
    "platform": "pc",
    "category": "new",
    "accent": "#4f46e5",
    "symbol": "LO",
    "tags": [
      "เกมใหม่",
      "เกมสล็อต"
    ]
  },
  {
    "code": "5187",
    "name": "Front Runner Odds On",
    "provider": "pp.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#ea580c",
    "symbol": "FR",
    "tags": [
      "เกมใหม่",
      "เกมสล็อต",
      "ซื้อฟรีสปิน"
    ]
  },
  {
    "code": "7535",
    "name": "Devils Hotfire Hold and Win",
    "provider": "ygg.png",
    "platform": "pc",
    "category": "new",
    "accent": "#16a34a",
    "symbol": "DE",
    "tags": [
      "เกมใหม่"
    ]
  },
  {
    "code": "5060",
    "name": "อัญมณีแห่งวัลฮัลล่า",
    "provider": "kingm.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#0ea5e9",
    "symbol": "อั",
    "tags": [
      "เกมใหม่",
      "เกมสล็อต",
      "ซื้อฟรีสปิน"
    ]
  },
  {
    "code": "4052",
    "name": "Alien Invasion",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "new",
    "accent": "#4f46e5",
    "symbol": "AL",
    "tags": [
      "เกมใหม่",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4063",
    "name": "The Arena",
    "provider": "wmslot.png",
    "platform": "pc",
    "category": "new",
    "accent": "#0ea5e9",
    "symbol": "TH",
    "tags": [
      "เกมใหม่",
      "เกมสล็อต"
    ]
  },
  {
    "code": "2486",
    "name": "Rave Party Fever",
    "provider": "pgsoft.png",
    "platform": "pc",
    "category": "new",
    "accent": "#ca8a04",
    "symbol": "RA",
    "tags": [
      "เกมใหม่",
      "เกมสล็อต"
    ]
  },
  {
    "code": "1914",
    "name": "FANTASY SOUTHEAST ASIA",
    "provider": "ps.png",
    "platform": "pc",
    "category": "new",
    "accent": "#ca8a04",
    "symbol": "FA",
    "tags": [
      "เกมใหม่",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4069",
    "name": "Cracker",
    "provider": "wmslot.png",
    "platform": "pc",
    "category": "new",
    "accent": "#0ea5e9",
    "symbol": "CR",
    "tags": [
      "เกมใหม่",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4041",
    "name": "Golden Temple",
    "provider": "jl.png",
    "platform": "pc",
    "category": "new",
    "accent": "#0ea5e9",
    "symbol": "GO",
    "tags": [
      "เกมใหม่",
      "เกมสล็อต"
    ]
  },
  {
    "code": "4317",
    "name": "Fortune Dragon",
    "provider": "pp.png",
    "platform": "pc",
    "category": "new",
    "accent": "#ea580c",
    "symbol": "FO",
    "tags": [
      "เกมใหม่",
      "เกมสล็อต"
    ]
  },
  {
    "code": "7150",
    "name": "Baccarat",
    "provider": "hotdog.png",
    "platform": "pc",
    "category": "table",
    "accent": "#7c3aed",
    "symbol": "BA",
    "tags": [
      "เกมโต๊ะ"
    ]
  },
  {
    "code": "2336",
    "name": "Color Game",
    "provider": "jl.png",
    "platform": "pc",
    "category": "table",
    "accent": "#0ea5e9",
    "symbol": "CO",
    "tags": [
      "เกมโต๊ะ"
    ]
  },
  {
    "code": "3673",
    "name": "Color Dish",
    "provider": "r88.png",
    "platform": "pc",
    "category": "table",
    "accent": "#0891b2",
    "symbol": "CO",
    "tags": [
      "เกมโต๊ะ"
    ]
  },
  {
    "code": "1142",
    "name": "Baccarat",
    "provider": "pp.png",
    "platform": "pc",
    "category": "table",
    "accent": "#ea580c",
    "symbol": "BA",
    "tags": [
      "เกมโต๊ะ"
    ]
  },
  {
    "code": "1940",
    "name": "Big small",
    "provider": "jl.png",
    "platform": "pc",
    "category": "table",
    "accent": "#0ea5e9",
    "symbol": "BI",
    "tags": [
      "เกมโต๊ะ"
    ]
  },
  {
    "code": "5802",
    "name": "Dragon Blackjack - Guaranteed Multiplier",
    "provider": "drag.png",
    "platform": "pc",
    "category": "table",
    "accent": "#0ea5e9",
    "symbol": "DR",
    "tags": [
      "เกมโต๊ะ"
    ]
  },
  {
    "code": "5810",
    "name": "Baccarat",
    "provider": "drag.png",
    "platform": "pc",
    "category": "table",
    "accent": "#0ea5e9",
    "symbol": "BA",
    "tags": [
      "เกมโต๊ะ"
    ]
  },
  {
    "code": "3779",
    "name": "Keno 8 (2 Minute)",
    "provider": "glx.png",
    "platform": "pc",
    "category": "table",
    "accent": "#ca8a04",
    "symbol": "KE",
    "tags": [
      "เกมโต๊ะ"
    ]
  },
  {
    "code": "1999",
    "name": "Sic Bo",
    "provider": "jl.png",
    "platform": "pc",
    "category": "table",
    "accent": "#0ea5e9",
    "symbol": "SI",
    "tags": [
      "เกมโต๊ะ"
    ]
  },
  {
    "code": "5350",
    "name": "Baccarat",
    "provider": "jkgx2.png",
    "platform": "pc",
    "category": "table",
    "accent": "#0ea5e9",
    "symbol": "BA",
    "tags": [
      "เกมโต๊ะ"
    ]
  },
  {
    "code": "2813",
    "name": "Roulette Nouveau",
    "provider": "reg.png",
    "platform": "pc",
    "category": "table",
    "accent": "#0ea5e9",
    "symbol": "RO",
    "tags": [
      "เกมโต๊ะ"
    ]
  },
  {
    "code": "1272",
    "name": "Three Card Poker Deluxe",
    "provider": "hbn.png",
    "platform": "pc",
    "category": "table",
    "accent": "#ea580c",
    "symbol": "TH",
    "tags": [
      "เกมโต๊ะ"
    ]
  },
  {
    "code": "1340",
    "name": "Tens or Better 1 Hand",
    "provider": "hbn.png",
    "platform": "pc",
    "category": "table",
    "accent": "#ea580c",
    "symbol": "TE",
    "tags": [
      "เกมโต๊ะ"
    ]
  },
  {
    "code": "608",
    "name": "Tens or Better 10 Hand",
    "provider": "hbn.png",
    "platform": "pc",
    "category": "table",
    "accent": "#ea580c",
    "symbol": "TE",
    "tags": [
      "เกมโต๊ะ"
    ]
  },
  {
    "code": "160",
    "name": "Aces and Eights 1 Hand",
    "provider": "hbn.png",
    "platform": "pc",
    "category": "table",
    "accent": "#ea580c",
    "symbol": "AC",
    "tags": [
      "เกมโต๊ะ"
    ]
  },
  {
    "code": "1008",
    "name": "All American Poker 5 Hand",
    "provider": "hbn.png",
    "platform": "pc",
    "category": "table",
    "accent": "#ea580c",
    "symbol": "AL",
    "tags": [
      "เกมโต๊ะ"
    ]
  },
  {
    "code": "5894",
    "name": "PIGGY MASTER",
    "provider": "ps.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ca8a04",
    "symbol": "PI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "6101",
    "name": "Caesar",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "CA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5108",
    "name": "Farm of Fortune",
    "provider": "fs.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#db2777",
    "symbol": "FA",
    "tags": [
      "เกมสล็อต",
      "ซื้อฟรีสปิน"
    ]
  },
  {
    "code": "5917",
    "name": "ตรุษจีน Moreways",
    "provider": "fachai.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "ตร",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "6011",
    "name": "Hawaii Riches",
    "provider": "bng.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#16a34a",
    "symbol": "HA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5360",
    "name": "Buccaneer Deluxe",
    "provider": "jkgx2.png",
    "platform": "pc",
    "category": "buy-free-spin",
    "accent": "#0ea5e9",
    "symbol": "BU",
    "tags": [
      "เกมสล็อต",
      "ซื้อฟรีสปิน"
    ]
  },
  {
    "code": "5883",
    "name": "Wealth Potion",
    "provider": "ygr.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0891b2",
    "symbol": "WE",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5886",
    "name": "Lucky Fortune God",
    "provider": "ygr.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0891b2",
    "symbol": "LU",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1178",
    "name": "Fruit Nova",
    "provider": "evp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ca8a04",
    "symbol": "FR",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "256",
    "name": "Honey Honey Honey",
    "provider": "pp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ea580c",
    "symbol": "HO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "4609",
    "name": "Sugar Bomb MultiBoost",
    "provider": "ygg.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#16a34a",
    "symbol": "SU",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "772",
    "name": "The Great Chicken Escape",
    "provider": "pp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ea580c",
    "symbol": "TH",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1541",
    "name": "Fortune Fest",
    "provider": "redtiger.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "FO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1543",
    "name": "Fruit Blox",
    "provider": "redtiger.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "FR",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3080",
    "name": "Love Amongst War",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "LO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3081",
    "name": "Charlotte's Web",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "CH",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "267",
    "name": "Panda's Fortune",
    "provider": "pp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ea580c",
    "symbol": "PA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "268",
    "name": "Ji Gong",
    "provider": "spp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ca8a04",
    "symbol": "JI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "2829",
    "name": "FaFaFa",
    "provider": "ame.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ca8a04",
    "symbol": "FA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "783",
    "name": "Bikini Island",
    "provider": "hbn.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ea580c",
    "symbol": "BI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3600",
    "name": "Buffalo Power 2: Hold and Win",
    "provider": "bng.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#16a34a",
    "symbol": "BU",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "4369",
    "name": "Ancient Eclipse",
    "provider": "ygg.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#16a34a",
    "symbol": "AN",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1043",
    "name": "Hollywood Pets",
    "provider": "cq.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "HO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1556",
    "name": "Gonzo's Quest Megaways",
    "provider": "redtiger.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "GO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3093",
    "name": "Space Storm",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "SP",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "791",
    "name": "HappyRichYear",
    "provider": "cq.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "HA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1304",
    "name": "Super 7",
    "provider": "spp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ca8a04",
    "symbol": "SU",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "2841",
    "name": "Alliance 2",
    "provider": "ame.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ca8a04",
    "symbol": "AL",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3098",
    "name": "Red Riding Hood",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "RE",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1564",
    "name": "Jack in a Pot",
    "provider": "redtiger.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "JA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3357",
    "name": "Mirror House",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "MI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "31",
    "name": "Fa Cai Fu Wa",
    "provider": "cq.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "FA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3104",
    "name": "Three Bandits",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "TH",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "546",
    "name": "Wild Fudge",
    "provider": "cq.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "WI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3363",
    "name": "Holy Beast",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "HO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5412",
    "name": "Golden Rooster",
    "provider": "jkgx2.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "GO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3180",
    "name": "Arctic Storm",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "AR",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1901",
    "name": "DA FU GUI",
    "provider": "ps.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ca8a04",
    "symbol": "DA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "878",
    "name": "Cage Fight",
    "provider": "spp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ca8a04",
    "symbol": "CA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "879",
    "name": "Smugglers Cove",
    "provider": "pp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ea580c",
    "symbol": "SM",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "368",
    "name": "Great Rhino Deluxe",
    "provider": "pp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ea580c",
    "symbol": "GR",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1137",
    "name": "Shaolin Fortunes 100",
    "provider": "hbn.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ea580c",
    "symbol": "SH",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3736",
    "name": "Wild Island",
    "provider": "r88.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0891b2",
    "symbol": "WI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "922",
    "name": "Hercules and Pegasus",
    "provider": "pp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ea580c",
    "symbol": "HE",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "2715",
    "name": "HUSA",
    "provider": "rsg.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "HU",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3228",
    "name": "Party Girl",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "PA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "670",
    "name": "5 Lions Gold",
    "provider": "pp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ea580c",
    "symbol": "5L",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1183",
    "name": "TheDragon Brings Wealth",
    "provider": "cq.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "TH",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "2208",
    "name": "Destiny of Sun & Moon",
    "provider": "pgsoft.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ca8a04",
    "symbol": "DE",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3233",
    "name": "The Apes",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "TH",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "675",
    "name": "Fire Chibi 2",
    "provider": "cq.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "FI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1956",
    "name": "Dragon & Tiger",
    "provider": "jl.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "DR",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "2725",
    "name": "Horror Nights",
    "provider": "rsg.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "HO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3494",
    "name": "Zombie Expert Lock 2 Spin",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "ZO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3751",
    "name": "Fortune Dragon",
    "provider": "r88.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0891b2",
    "symbol": "FO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "4520",
    "name": "Wild Fishin' Wild Ways",
    "provider": "ygg.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#16a34a",
    "symbol": "WI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5801",
    "name": "Empire of Riches",
    "provider": "drag.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "EM",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3243",
    "name": "Route 66",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "RO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5548",
    "name": "SEAWORLD LEGENDS IN THE SEA",
    "provider": "vp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "SE",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1454",
    "name": "Pyramid: Quest for Immortality",
    "provider": "netent.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "PY",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "943",
    "name": "Wonderland",
    "provider": "cq.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "WO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "4390",
    "name": "Vikings go Berzerk",
    "provider": "ygg.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#16a34a",
    "symbol": "VI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5159",
    "name": "Shiba Mogul",
    "provider": "ask.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#16a34a",
    "symbol": "SH",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "553",
    "name": "Vampires vs Wolves",
    "provider": "pp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ea580c",
    "symbol": "VA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "2858",
    "name": "Elements",
    "provider": "ame.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ca8a04",
    "symbol": "EL",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "4395",
    "name": "Sunny Shores",
    "provider": "ygg.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#16a34a",
    "symbol": "SU",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "301",
    "name": "Glam Rock",
    "provider": "hbn.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ea580c",
    "symbol": "GL",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "2862",
    "name": "The Myth",
    "provider": "ame.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ca8a04",
    "symbol": "TH",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3375",
    "name": "Sailor Man",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "SA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1073",
    "name": "Rich Reels",
    "provider": "evp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ca8a04",
    "symbol": "RI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1586",
    "name": "Magic Gate",
    "provider": "redtiger.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "MA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3379",
    "name": "Sky Force",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "SK",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5428",
    "name": "Journey To The West",
    "provider": "jkgx2.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "JO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3382",
    "name": "Sexy Lips",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "SE",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1592",
    "name": "Mega Rise",
    "provider": "redtiger.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "ME",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "58",
    "name": "Santa",
    "provider": "pp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ea580c",
    "symbol": "SA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1595",
    "name": "Mystery Reels",
    "provider": "redtiger.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "MY",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3388",
    "name": "Lion Dance",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "LI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "4413",
    "name": "Wolf Hunters",
    "provider": "ygg.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#16a34a",
    "symbol": "WO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "4926",
    "name": "Neko Riches",
    "provider": "fs.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#db2777",
    "symbol": "NE",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "4927",
    "name": "Mining Bonanza",
    "provider": "fs.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#db2777",
    "symbol": "MI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5952",
    "name": "Dragons Legend",
    "provider": "acewin.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#16a34a",
    "symbol": "DR",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5953",
    "name": "Fu Xing Gao Zhao",
    "provider": "acewin.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#16a34a",
    "symbol": "FU",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "835",
    "name": "Super5",
    "provider": "cq.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "SU",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1604",
    "name": "Ocean Fortune",
    "provider": "redtiger.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "OC",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1861",
    "name": "RICH & HAPPY",
    "provider": "ps.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ca8a04",
    "symbol": "RI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3142",
    "name": "Siberian Wolves",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "SI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3399",
    "name": "Legend of the White Snake",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "LE",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5448",
    "name": "MoneyBangBang",
    "provider": "jkgx2.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "MO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "4425",
    "name": "Multifly!",
    "provider": "ygg.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#16a34a",
    "symbol": "MU",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1099",
    "name": "Before Time Runs Out",
    "provider": "hbn.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ea580c",
    "symbol": "BE",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "2636",
    "name": "Cai Shen 168",
    "provider": "redtiger.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "CA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3405",
    "name": "Formosan Birds",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "FO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "4430",
    "name": "Lightning Joker",
    "provider": "ygg.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#16a34a",
    "symbol": "LI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "848",
    "name": "Gu Gu Gu M",
    "provider": "cq.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "GU",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1361",
    "name": "Rise Of Horus",
    "provider": "evp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ca8a04",
    "symbol": "RI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1618",
    "name": "Rainbow Jackpots",
    "provider": "redtiger.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "RA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3411",
    "name": "Samurai Way",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "SA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3412",
    "name": "Shen Nong",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "SH",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "598",
    "name": "Super X",
    "provider": "pp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ea580c",
    "symbol": "SU",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3415",
    "name": "Fortune Feast Fusion Reels",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "FO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5464",
    "name": "Ocean Paradise",
    "provider": "jkgx2.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "OC",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "346",
    "name": "Phoenix Forge",
    "provider": "pp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ea580c",
    "symbol": "PH",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1627",
    "name": "Reptizillions Power Reels",
    "provider": "redtiger.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "RE",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1628",
    "name": "Riddle Of The Sphinx",
    "provider": "redtiger.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "RI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "4445",
    "name": "Blood Moon Wilds",
    "provider": "ygg.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#16a34a",
    "symbol": "BL",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5470",
    "name": "Pan Jin Lian",
    "provider": "jkgx2.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "PA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "4959",
    "name": "LittleWitch",
    "provider": "pgsus.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0891b2",
    "symbol": "LI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5984",
    "name": "Hello Doggy",
    "provider": "acewin.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#16a34a",
    "symbol": "HE",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1634",
    "name": "Snow Wild And The 7 Features",
    "provider": "redtiger.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "SN",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1635",
    "name": "Spin Town",
    "provider": "redtiger.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "SP",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "356",
    "name": "WanBaoDino",
    "provider": "cq.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "WA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5476",
    "name": "Respin Mania",
    "provider": "jkgx2.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "RE",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5733",
    "name": "Viking Reels",
    "provider": "drag.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "VI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1383",
    "name": "Blood Sucker",
    "provider": "netent.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "BL",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "616",
    "name": "Poseidon",
    "provider": "cq.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "PO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5992",
    "name": "Arcane Egypt",
    "provider": "acewin.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#16a34a",
    "symbol": "AR",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5993",
    "name": "Day of the Dead",
    "provider": "acewin.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#16a34a",
    "symbol": "DA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5738",
    "name": "Lucky Golden Joker",
    "provider": "drag.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "LU",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5739",
    "name": "Jingle Jackpots",
    "provider": "drag.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "JI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "4465",
    "name": "TIGER_TIGER",
    "provider": "ygg.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#16a34a",
    "symbol": "TI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "4978",
    "name": "emperor auspicious",
    "provider": "pgsus.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0891b2",
    "symbol": "EM",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3443",
    "name": "Seagull",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "SE",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "4980",
    "name": "Zorro",
    "provider": "pgsus.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0891b2",
    "symbol": "ZO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5493",
    "name": "Tai Shang Lao Jun",
    "provider": "jkgx2.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "TA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "375",
    "name": "The Guard",
    "provider": "spp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ca8a04",
    "symbol": "TH",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5751",
    "name": "Chase the Turkey",
    "provider": "drag.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "CH",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "4984",
    "name": "Iceage Mammoths",
    "provider": "pgsus.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0891b2",
    "symbol": "IC",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5497",
    "name": "Third Prince''s Journey",
    "provider": "jkgx2.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "TH",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "4986",
    "name": "jalapeno",
    "provider": "pgsus.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0891b2",
    "symbol": "JA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3451",
    "name": "Double Fortune",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "DO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "4476",
    "name": "Hypernova 10K Ways",
    "provider": "ygg.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#16a34a",
    "symbol": "HY",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "4989",
    "name": "GodOfFortune",
    "provider": "pgsus.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0891b2",
    "symbol": "GO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5502",
    "name": "Tiger''s Lair",
    "provider": "jkgx2.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "TI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "4991",
    "name": "DragonTiles",
    "provider": "pgsus.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0891b2",
    "symbol": "DR",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "641",
    "name": "FruitKing",
    "provider": "cq.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "FR",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "642",
    "name": "Taberna De Los Muertos",
    "provider": "hbn.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ea580c",
    "symbol": "TA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "2179",
    "name": "Genie \"s 3 Wishes",
    "provider": "pgsoft.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ca8a04",
    "symbol": "GE",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3204",
    "name": "Romance In England",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "RO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5765",
    "name": "Demon Train",
    "provider": "drag.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "DE",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "135",
    "name": "Angels & Demons",
    "provider": "spp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ca8a04",
    "symbol": "AN",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5511",
    "name": "Winter Sweets",
    "provider": "jkgx2.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "WI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "649",
    "name": "USSR Grocery",
    "provider": "evp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ca8a04",
    "symbol": "US",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "2442",
    "name": "4 of a King",
    "provider": "edp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#db2777",
    "symbol": "4O",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3211",
    "name": "Eight Treasures",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "EI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "2700",
    "name": "Captain Hook",
    "provider": "rsg.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "CA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "2701",
    "name": "HUCA",
    "provider": "rsg.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "HU",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5518",
    "name": "Yggdrasil",
    "provider": "jkgx2.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "YG",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "400",
    "name": "Egyptian Dreams Deluxe",
    "provider": "hbn.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ea580c",
    "symbol": "EG",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1425",
    "name": "Grand Spinn",
    "provider": "netent.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "GR",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "2706",
    "name": "Nonstop",
    "provider": "rsg.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "NO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3219",
    "name": "Musketeers",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "MU",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5780",
    "name": "Sea Treasures",
    "provider": "drag.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "SE",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "150",
    "name": "Buggy Bonus",
    "provider": "hbn.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ea580c",
    "symbol": "BU",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1943",
    "name": "Bubble Beauty",
    "provider": "jl.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "BU",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3763",
    "name": "Thor",
    "provider": "r88.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0891b2",
    "symbol": "TH",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "693",
    "name": "Hot Hot Halloween",
    "provider": "hbn.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ea580c",
    "symbol": "HO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1462",
    "name": "Scudamore's Super Stakes",
    "provider": "netent.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "SC",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3255",
    "name": "Spring Blossom",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "SP",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5560",
    "name": "Rally",
    "provider": "vp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "RA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1979",
    "name": "Lucky Coming",
    "provider": "jl.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "LU",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1714",
    "name": "Lucky New Year - Tiger Treasures",
    "provider": "pp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ea580c",
    "symbol": "LU",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1980",
    "name": "Lucky Goldbricks",
    "provider": "jl.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "LU",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1982",
    "name": "Lucky Number",
    "provider": "jl.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "LU",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5567",
    "name": "Magic Pearl",
    "provider": "vp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "MA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1217",
    "name": "King Tut's Tomb",
    "provider": "hbn.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ea580c",
    "symbol": "KI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "4546",
    "name": "Fury of Hyde Megaways",
    "provider": "ygg.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#16a34a",
    "symbol": "FU",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1732",
    "name": "Wild Depths",
    "provider": "pp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ea580c",
    "symbol": "WI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3270",
    "name": "Beautiful Lady",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "BE",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "2248",
    "name": "Sultan's tale",
    "provider": "pokslot.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "SU",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5577",
    "name": "Pool Party",
    "provider": "vp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "PO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1228",
    "name": "Money Roll",
    "provider": "pp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ea580c",
    "symbol": "MO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "5581",
    "name": "Peace Long Life",
    "provider": "vp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "PE",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1487",
    "name": "1942: Sky Warrior",
    "provider": "redtiger.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "19",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1233",
    "name": "GoldenEggs",
    "provider": "cq.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "GO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "467",
    "name": "Syndicate",
    "provider": "evp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ca8a04",
    "symbol": "SY",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3285",
    "name": "A Thirsty Crow",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "AT",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1496",
    "name": "Arcade Bomb",
    "provider": "redtiger.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "AR",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3290",
    "name": "Lion vs. Shark",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "LI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3292",
    "name": "Magician House",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "MA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3294",
    "name": "Wild Vick",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "WI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3552",
    "name": "3 Coins",
    "provider": "bng.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#16a34a",
    "symbol": "3C",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1250",
    "name": "Jellyfish Flow",
    "provider": "hbn.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ea580c",
    "symbol": "JE",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "996",
    "name": "North South Lions",
    "provider": "spp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ca8a04",
    "symbol": "NO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "998",
    "name": "Hot Pinatas",
    "provider": "cq.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "HO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "2792",
    "name": "Deep Descent",
    "provider": "reg.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "DE",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3562",
    "name": "Moon Sisters",
    "provider": "bng.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#16a34a",
    "symbol": "MO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3052",
    "name": "Elite Games",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "EL",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "4077",
    "name": "MonteZuma's Treasure",
    "provider": "ygr.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0891b2",
    "symbol": "MO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1263",
    "name": "Fortune Spirits",
    "provider": "cq.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "FO",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "497",
    "name": "Mustang Gold",
    "provider": "pp.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#ea580c",
    "symbol": "MU",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1522",
    "name": "Dragon's Fire Infinireels",
    "provider": "redtiger.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "DR",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3060",
    "name": "da Vinci",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "DA",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3062",
    "name": "Oktoberfest",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "OK",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "2808",
    "name": "Mega Flip",
    "provider": "reg.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "ME",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "3322",
    "name": "Wild Jungle",
    "provider": "kaga.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#4f46e5",
    "symbol": "WI",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "4604",
    "name": "5 Sevens Hold and Win",
    "provider": "ygg.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#16a34a",
    "symbol": "5S",
    "tags": [
      "เกมสล็อต"
    ]
  },
  {
    "code": "1534",
    "name": "Esqueleto Mariachi",
    "provider": "redtiger.png",
    "platform": "pc",
    "category": "slot",
    "accent": "#0ea5e9",
    "symbol": "ES",
    "tags": [
      "เกมสล็อต"
    ]
  }
] as const;