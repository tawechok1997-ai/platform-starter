...
describeWithDatabase('provider simulator concurrency with PostgreSQL', () => {
  ...
  beforeAll(async () => {
    ...
    await prisma.user.create({ data: { id: userId, username: `game-ci-${suffix}`, email: `game-ci-${suffix}@example.test`, passwordHash: 'not-used-in-concurrency-test' } });
  ...
  it('accepts only one of two conflicting bets for the same round', async () => {
    ...
    const ledgers = await prisma.walletLedger.findMany({ where: { userId, referenceType: 'game_bet' } });
