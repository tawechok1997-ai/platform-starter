...
export class ReportsQueryService {
  ...
  async getDailySummary(query: ReportQuery = {}) {
    ...
    const [topUps, withdrawals, adjustments, walletAgg, ledgerAgg, pendingTopUps, pendingWithdrawals] = await Promise.all([
      this.prisma.topUpRequest.groupBy({ by: ['status'], where: { createdAt: { gte: from, lte: to } }, _count: { _all: true }, _sum: { amount: true } }),
      this.prisma.withdrawalRequest.groupBy({ by: ['status'], where: { createdAt: { gte: from, lte: to } }, _count: { _all: true }, _sum: { amount: true } }),
      this.prisma.walletLedger.groupBy({ by: ['direction'], where: { type: 'ADJUSTMENT', createdAt: { gte: from, lte: to } }, _count: { _all: true }, _sum: { amount: true } }),
    ...
    return {
      ...
      wallets: { count: walletAgg._count._all, totalBalance: walletAgg._sum.balance?.toString() ?? '0', totalLockedBalance: walletAgg._sum.lockedBalance?.toString() ?? '0' },
  ...
  async getTrends(query: ReportQuery = {}) {
    ...
    const [topUps, withdrawals] = await Promise.all([
      this.prisma.topUpRequest.findMany({ where: { status: 'APPROVED', reviewedAt: { gte: from, lte: to } }, select: { amount: true, reviewedAt: true }, orderBy: { reviewedAt: 'asc' } }),
      this.prisma.withdrawalRequest.findMany({ where: { status: 'COMPLETED', reviewedAt: { gte: from, lte: to } }, select: { amount: true, reviewedAt: true }, orderBy: { reviewedAt: 'asc' } }),
    ]);
    ...
    return { range: { days, from: from.toISOString(), to: to.toISOString() }, totals, daily, generatedAt: new Date().toISOString() };
  }

  async getQueueAging() {
    const now = Date.now();
    const [topUps, withdrawals] = await Promise.all([
      this.prisma.topUpRequest.findMany({ where: { status: 'PENDING' }, include: { user: { select: { id: true, username: true, email: true, phone: true } } }, orderBy: { createdAt: 'asc' }, take: 20 }),
      this.prisma.withdrawalRequest.findMany({ where: { status: 'PENDING' }, include: { user: { select: { id: true, username: true, email: true, phone: true } } }, orderBy: { createdAt: 'asc' }, take: 20 }),
    ]);
  ...
  async getReconciliation(query: ReportQuery = {}) {
    ...
    return { items: results, checkedCount: results.length, mismatchCount: results.filter((item) => item.status === 'MISMATCH').length, generatedAt: new Date().toISOString() };
  }
}
