...
export class ReportsQueryService {
  ...
  async getDailySummary(query: ReportQuery = {}) {
    const { from, to } = parseReportDateRange(query);
    ...
    return {
      ...
      topUps: topUps.map(formatReportGroup),
      withdrawals: withdrawals.map(formatReportGroup),
  ...
  async getQueueAging() {
    ...
    const topUpItems = topUps.map((item) => formatAgingItem('TOPUP', item, now));
    const withdrawalItems = withdrawals.map((item) => formatAgingItem('WITHDRAWAL', item, now));
