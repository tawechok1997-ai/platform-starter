...
export class AdminReportReadModel {
  ...
  async loadTrends(query: ReportQuery = {}) {
    ...
    for (const item of topUps) {
      const row = item.reviewedAt ? byDate.get(item.reviewedAt.toISOString().slice(0, 10)) : null;
      if (!row) continue;
      row.topUpCount += 1;
  ...
  async loadQueueAging() {
    const now = Date.now();
    const [topUps, withdrawals] = await Promise.all([
      this.prisma.topUpRequest.findMany({ where: { status: 'PENDING' }, select: QUEUE_AGING_PROJECTION, orderBy: { createdAt: 'asc' }, take: QUEUE_AGING_LIMIT }),
      this.prisma.withdrawalRequest.findMany({ where: { status: 'PENDING' }, select: QUEUE_AGING_PROJECTION, orderBy: { createdAt: 'asc' }, take: QUEUE_AGING_LIMIT }),
    ]);
  ...
  async loadReconciliation(query: ReportQuery = {}) {
    const safeLimit = Math.min(Math.max(Number(query.limit) || RECONCILIATION_DEFAULT_LIMIT, 1), RECONCILIATION_MAX_LIMIT);
    const wallets = await this.prisma.wallet.findMany({ orderBy: { updatedAt: 'desc' }, take: safeLimit, select: RECONCILIATION_WALLET_PROJECTION });
    const results = await Promise.all(wallets.map(async (wallet) => {
      const latestLedger = await this.prisma.walletLedger.findFirst({ where: { walletId: wallet.id }, orderBy: { createdAt: 'desc' }, select: RECONCILIATION_LEDGER_PROJECTION });
