...
export class RiskService {
  ...
  async getSummary() {
    ...
    const walletMismatch = checkedWallets.filter((item) => item.mismatch).slice(0, 20).map((item) => ({
    ...
    const negativeAvailable = checkedWallets.filter((item) => item.negativeAvailable).slice(0, 20).map((item) => ({
    ...
    const staleAlerts = [
      ...staleTopUps.map((item) => ({ type: 'STALE_TOPUP_PENDING', severity: 'MEDIUM', targetId: item.id, userId: item.userId, username: item.user?.username ?? null, amount: item.amount.toString(), createdAt: item.createdAt, message: 'Top-up request pending for more than 24 hours' })),
      ...staleWithdrawals.map((item) => ({ type: 'STALE_WITHDRAWAL_PENDING', severity: 'MEDIUM', targetId: item.id, userId: item.userId, username: item.user?.username ?? null, amount: item.amount.toString(), createdAt: item.createdAt, message: 'Withdrawal request pending for more than 24 hours' })),
    ];
