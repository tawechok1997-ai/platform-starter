...
export class AdminInvitationAdminService {
  ...
  async create(
    ...
    const roleIds = this.normalizeRoleIds(
      Array.isArray(roleIdOrIds) ? roleIdOrIds : [roleIdOrIds],
    );
    ...
    if (roles.length !== roleIds.length) {
      ...
      throw new NotFoundException(
        `Role not found: ${roleIds.filter((roleId) => !foundIds.has(roleId)).join(', ')}`,
      );
    }
    if (existing) {
    ...
    const selection = this.resolveSelection(
      roles.map((role) => this.toPolicyRole(role)),
      options.primaryRoleId,
    );
    ...
    if (!evaluation.allowed) {
      throw new ForbiddenException(evaluation.reason ?? 'Selected roles cannot be granted');
    }
    ...
    const created = await this.prisma.$transaction(async (tx) => {
      const admin = await tx.adminUser.create({
        data: {
          ...
          department,
          roles: {
            create: selection.roles.map((role) => ({ roleId: role.id })),
          },
        },
        select: {
          id: true,
  ...
  async list() {
    ...
    for (const token of tokens) {
      ...
      if (adminUserId && !latestByAdmin.has(adminUserId)) {
        latestByAdmin.set(adminUserId, token);
      }
    }
    ...
    return {
      items: admins.map((admin) => {
        ...
        const state = token.usedAt
          ? 'REVOKED_OR_USED'
          : token.expiresAt <= now
            ? 'EXPIRED'
            : 'ACTIVE';
        const orderedRoles = [...admin.roles].sort(
          (left, right) =>
            left.role.level - right.role.level ||
            left.role.code.localeCompare(right.role.code),
        );
        const primaryRole =
          orderedRoles.find((item) => item.role.code === admin.position)?.role ??
          orderedRoles[0]?.role ??
          null;
        return {
          ...
          usedAt: token.usedAt,
          protected: admin.roles.some((item) =>
            PROTECTED_ROLE_CODES.has(item.role.code),
          ),
          roles: orderedRoles.map((item) => ({
            id: item.role.id,
            code: item.role.code,
            name: item.role.name,
  ...
  async revoke(actorAdminId: string, adminUserId: string) {
    ...
    if (target.status !== 'LOCKED') {
      throw new BadRequestException('Only pending invited accounts can be revoked');
    }
    ...
    await this.audit(actorAdminId, 'REVOKE_ADMIN_INVITATION', adminUserId, {
  ...
  async reissue(actorAdminId: string, adminUserId: string, expiresInHours = 24) {
    ...
    if (!target) throw new NotFoundException('Invited admin account not found');
    if (target.status !== 'LOCKED') {
      throw new BadRequestException('Only pending invited accounts can be reissued');
    }
    if (target.roles.length === 0) {
      throw new BadRequestException('Invited admin account has no role');
    }
    if (target.roles.some((item) => PROTECTED_ROLE_CODES.has(item.role.code))) {
    ...
    if (!evaluation.allowed) {
      throw new ForbiddenException(
        evaluation.reason ?? 'Invitation roles are above the acting admin',
      );
    }
  ...
  private normalizeRoleIds(roleIdsInput: string[]) {
    ...
    if (roleIds.length > MAX_ASSIGNED_ROLES) {
      throw new BadRequestException(
        `An admin user can hold at most ${MAX_ASSIGNED_ROLES} roles`,
      );
    }
    return roleIds;
  }
  ...
  private normalizeDepartment(departmentInput?: string) {
    ...
    if (department.length > 120) {
      throw new BadRequestException('Department must not exceed 120 characters');
    }
    return department;
  }
  ...
  private toPolicyRole(role: {
    ...
    level: number;
    permissions: Array<{
      permission: { code: string; module: string; name: string };
    }>;
  }): AdminRolePolicyRole {
    return {
      id: role.id,
