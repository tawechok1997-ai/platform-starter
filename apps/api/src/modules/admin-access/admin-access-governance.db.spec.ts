...
function assertSafeTestDatabase(url: string) {
  ...
  if (!safeHost && !safeName) {
    throw new Error('Admin governance DB tests require an isolated test database');
  }
}
...
describeWithDatabase('admin team and effective-access governance with PostgreSQL', () => {
  ...
  beforeAll(async () => {
    ...
    await prisma.role.createMany({
      data: [
        {
          id: ownerRoleId,
          code: `governance_owner_${suffix}`,
          name: 'Governance Owner',
          level: 1,
        },
        {
          id: managerRoleId,
          code: `governance_manager_${suffix}`,
          name: 'Governance Manager',
          level: 20,
        },
        {
          id: subordinateRoleId,
          code: `governance_subordinate_${suffix}`,
          name: 'Governance Subordinate',
          level: 80,
        },
      ],
    });
    ...
    await prisma.adminUser.createMany({
      data: [
        {
          id: ownerId,
          username: `governance-owner-${suffix}`,
          email: `governance-owner-${suffix}@example.test`,
          passwordHash: 'not-used',
        },
        {
          id: managerId,
          username: `governance-manager-${suffix}`,
          email: `governance-manager-${suffix}@example.test`,
          passwordHash: 'not-used',
        },
        {
          id: subordinateId,
          username: `governance-subordinate-${suffix}`,
          email: `governance-subordinate-${suffix}@example.test`,
          passwordHash: 'not-used',
        },
      ],
    });
  ...
  afterAll(async () => {
    ...
    if (teamId) {
      await prisma.$executeRawUnsafe(
        'DELETE FROM admin_team_members WHERE team_id = $1::uuid',
        teamId,
      );
      await prisma.$executeRawUnsafe('DELETE FROM admin_teams WHERE id = $1::uuid', teamId);
    }
    await prisma.adminAuditLog.deleteMany({
      where: { adminUserId: { in: [ownerId, managerId, subordinateId] } },
    });
    await prisma.adminUserRole.deleteMany({
      where: { adminUserId: { in: [ownerId, managerId, subordinateId] } },
    });
    await prisma.adminUser.deleteMany({
      where: { id: { in: [ownerId, managerId, subordinateId] } },
    });
    await prisma.rolePermission.deleteMany({
      where: { roleId: { in: [ownerRoleId, managerRoleId, subordinateRoleId] } },
    });
    await prisma.role.deleteMany({
      where: { id: { in: [ownerRoleId, managerRoleId, subordinateRoleId] } },
    });
    await prisma.permission.deleteMany({
      where: { id: { in: [targetPermissionId, missingPermissionId] } },
    });
    if (subordinatePermissionOwned) {
      await prisma.permission.deleteMany({
        where: { id: subordinatePermissionId, roles: { none: {} } },
      });
    }
    if (wildcardPermissionOwned) {
      await prisma.permission.deleteMany({
        where: { id: wildcardPermissionId, roles: { none: {} } },
      });
    }
  ...
  it('persists a managed team, membership and direct reporting line', async () => {
    ...
    expect(storedTeam?.members).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ adminUserId: subordinateId, isLead: true }),
      ]),
    );
    expect(overview.reportingLines).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          managerAdminId: managerId,
          subordinateAdminId: subordinateId,
        }),
      ]),
    );
  }, 30_000);
