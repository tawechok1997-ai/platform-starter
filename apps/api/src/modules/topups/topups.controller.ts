...
export class TopUpsController {
  ...
  @Post('member/topups')
  createMemberRequest(@CurrentUser() user: any, @Body() body: CreateTopUpRequestDto) { return this.topUpsService.createMemberRequest(user.id, body); }

  @UseGuards(MemberAuthGuard)
  @Get('member/topups')
  getMemberRequests(@CurrentUser() user: any) { return this.topUpsService.getMemberRequests(user.id); }

  @UseGuards(MemberAuthGuard)
  @Get('member/topups/:id')
  getMemberRequest(@CurrentUser() user: any, @Param('id') id: string) { return this.topUpsService.getMemberRequest(user.id, id); }
  ...
  claimRequest(@Param('id') id: string, @CurrentUser() user: any, @Req() req: any) { return this.topUpsService.claimRequest(id, user, this.meta(req)); }
