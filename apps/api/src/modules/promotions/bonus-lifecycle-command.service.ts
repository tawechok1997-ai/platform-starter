...
type BonusLifecycleAction = 'RELEASE' | 'RETRY' | 'REVERSE' | 'EXPIRE' | 'REVOKE';
type BonusLifecycleInput = { action?: BonusLifecycleAction; note?: string };
...
export class BonusLifecycleCommandService {
  ...
  async addTurnoverProgress(admin: Actor, id: string, input: TurnoverInput) {
    ...
    if (['REVOKED', 'EXPIRED', 'REVERSED'].includes(metadata.lifecycleStatus)) throw new BadRequestException('Cannot update turnover for inactive bonus');
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      const next = await tx.riskAlert.update({
        where: { id },
        data: { status: completed ? 'RESOLVED' : item.status, resolvedAt: completed ? new Date() : item.resolvedAt, metadata: this.safeJson({ ...metadata, turnoverProgress, turnoverRequired, turnoverCompleted: completed, lifecycleStatus, walletCreditEnabled: false, walletCreditStatus, events }) },
      });
      await tx.adminAuditLog.create({ data: buildAdminAuditData({ adminUserId: admin.id, module: 'promotions', action: 'bonus.turnover.progress', targetId: id, oldData: item, newData: next }) });
      return next;
    });
  ...
  async updateBonusLifecycle(admin: Actor, id: string, input: BonusLifecycleInput) {
    ...
    if (!['RELEASE', 'RETRY', 'REVERSE', 'EXPIRE', 'REVOKE'].includes(rawAction)) {
      throw new BadRequestException('action must be RELEASE, RETRY, REVERSE, EXPIRE, or REVOKE');
    }
    const action = rawAction as BonusLifecycleAction;
    if (action === 'RELEASE' || action === 'RETRY' || action === 'REVERSE') return this.settlementCommands.execute(admin, id, action, input.note);
    ...
    if (['REVOKED', 'EXPIRED', 'SETTLED', 'REVERSED'].includes(metadata.lifecycleStatus)) throw new BadRequestException(`Bonus is already ${metadata.lifecycleStatus.toLowerCase()}`);
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      const next = await tx.riskAlert.update({
        where: { id },
        data: { status: nextStatus, resolvedAt: new Date(), metadata: this.safeJson({ ...metadata, lifecycleStatus, walletCreditEnabled: false, walletCreditStatus, lifecycleNote: note, lifecycleUpdatedAt: new Date().toISOString(), lifecycleUpdatedBy: admin.id, events }) },
      });
      await tx.adminAuditLog.create({ data: buildAdminAuditData({ adminUserId: admin.id, module: 'promotions', action: `bonus.lifecycle.${action.toLowerCase()}`, targetId: id, oldData: item, newData: next }) });
      return next;
    });
