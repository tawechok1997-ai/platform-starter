...
export class BankAccountsService {
  ...
  async assignReceivingAccount(paymentType = 'bank_transfer', amount?: string | number) {
    ...
    const all = await this.prisma.receivingBankAccount.findMany({
      where: { status: 'ACTIVE' },
      orderBy: [{ sortOrder: 'asc' }, { createdAt: 'asc' }],
    });
    const items = all.filter(
      (item) => this.paymentType(item.bankName) === paymentType && this.matchAmount(item, parsedAmount),
    );
  ...
  async updateReceivingAccount(id: string, body: BankBody, admin: any, meta: any) {
    ...
    await this.audit(
      admin?.id,
      'UPDATE_RECEIVING_BANK_ACCOUNT',
      id,
      this.mapReceiving(existing),
      this.mapReceiving(item),
      meta,
    );
    return { item: this.mapReceiving(item) };
  }
  ...
  async listMemberBankAccounts(userId: string) {
    const items = await this.prisma.memberBankAccount.findMany({
      where: { userId },
  ...
  async createMemberBankAccount(userId: string, body: BankBody) {
    ...
    const duplicateActive = await this.prisma.memberBankAccount.findFirst({
      where: { accountNumber: body.accountNumber!.trim(), status: { in: ['ACTIVE', 'PENDING_REVIEW'] } },
    });
    if (duplicateActive) throw new BadRequestException('เลขบัญชีนี้ถูกใช้ในระบบแล้ว');
    const user = await this.prisma.user.findUnique({ where: { id: userId }, include: { profile: true } });
    if (!user) throw new NotFoundException('Member not found');
    const allowedNames = [user.profile?.displayName, user.username]
      .filter(Boolean)
    ...
    const item = await this.prisma.memberBankAccount
      .create({
        data: {
          userId,
          bankName: body.bankName!.trim(),
      ...
      .catch((error: unknown) => {
        if (String((error as { code?: string })?.code ?? '') === 'P2002')
          throw new BadRequestException('สมาชิกหรือเลขบัญชีนี้ถูกใช้ในระบบแล้ว');
        throw error;
      });
  ...
  async listAllMemberBankAccounts(search?: string) {
    const where = search?.trim()
      ? {
          OR: [
            { bankName: { contains: search.trim(), mode: 'insensitive' as const } },
            { accountName: { contains: search.trim(), mode: 'insensitive' as const } },
            { accountNumber: { contains: search.trim(), mode: 'insensitive' as const } },
            { user: { username: { contains: search.trim(), mode: 'insensitive' as const } } },
          ],
        }
      : {};
    const items = await this.prisma.memberBankAccount.findMany({
  ...
  async kycSummary() {
    const [pending, active, rejected, disabled, accounts, unverifiedPhones] = await Promise.all([
      ...
      this.prisma.memberBankAccount.findMany({
        take: 500,
        orderBy: { createdAt: 'desc' },
        include: {
          user: { select: { id: true, username: true, phone: true, email: true, status: true, phoneVerifiedAt: true } },
        },
      }),
    ...
    const duplicateGroups = Array.from(groups.entries())
      ...
      .map(([accountNumber, rows]) => ({
        accountNumber,
        count: rows.length,
        items: rows.map((row) => ({ ...this.mapMemberBank(row), user: row.user })),
      }));
    const riskyAccounts = accounts
      .filter(
        (item) =>
          item.status === 'PENDING_REVIEW' || item.user?.status !== 'ACTIVE' || item.user?.phoneVerifiedAt === null,
      )
      .slice(0, 50)
      .map((item) => ({ ...this.mapMemberBank(item), user: item.user, flags: this.bankFlags(item, duplicateGroups) }));
    return {
      summary: { pending, active, rejected, disabled, duplicateGroups: duplicateGroups.length, unverifiedPhones },
      duplicateGroups,
      riskyAccounts,
    };
  }
  ...
  async reviewMemberBankAccount(id: string, body: BankBody, admin: any, meta: any) {
    const existing = await this.prisma.memberBankAccount.findUnique({
      where: { id },
    ...
    if (body.status === 'ACTIVE') {
      ...
      if (existing.user?.status !== 'ACTIVE')
        throw new BadRequestException('สมาชิกไม่ได้อยู่สถานะ ACTIVE ไม่ควรอนุมัติบัญชี');
    ...
    const item = await this.prisma.memberBankAccount.update({
      where: { id },
      data: { status: body.status ?? 'ACTIVE', adminNote: body.adminNote ?? undefined },
    });
    await this.audit(
      admin?.id,
      'REVIEW_MEMBER_BANK_ACCOUNT',
      id,
      this.mapMemberBank(existing),
      this.mapMemberBank(item),
      meta,
    );
    return { item: this.mapMemberBank(item) };
  }
  ...
  private requireBankFields(body: BankBody) {
    if (!body.bankName?.trim() || !body.accountName?.trim() || !body.accountNumber?.trim())
      throw new BadRequestException('bankName, accountName and accountNumber are required');
  }
  ...
  private paymentType(bankName: string): PaymentType {
    if (bankName === 'พร้อมเพย์') return 'promptpay';
    if (bankName === 'วอเลต') return 'wallet';
    if (bankName === 'อื่น ๆ') return 'other';
    return 'bank_transfer';
  }
  private matchAmount(item: any, amount: number | null) {
    if (amount === null) return true;
    const min = item.minAmount ? Number(item.minAmount) : 0;
    const max = item.maxAmount ? Number(item.maxAmount) : Infinity;
  ...
  private mapReceiving(item: any) {
    return {
      ...
      minAmount: item.minAmount?.toString?.() ?? null,
      maxAmount: item.maxAmount?.toString?.() ?? null,
      status: item.status,
