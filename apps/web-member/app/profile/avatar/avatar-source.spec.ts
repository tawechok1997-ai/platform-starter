...
test('avatar settings adds only the missing page and reuses existing account action popups', () => {
  ...
  assert.match(page, /<ProfileActionPopupLayer kind=\{popup\}/);
...
test('avatar preference is bounded to the supplied local avatar assets', () => {
  ...
  assert.match(page, /resolveLocalAssetByBasename\(VIP_BADGE_SOURCE, 'mobile'\)[\s\S]*resolveLocalAssetByBasename\(VIP_BADGE_SOURCE, 'pc'\)/);
  assert.match(styles, /max-width:\s*428px/);
  assert.match(styles, /\.avatarGrid\s*\{[\s\S]*grid-template-columns:\s*repeat\(4/);
  assert.match(styles, /\.actions\s*\{[\s\S]*grid-template-columns:\s*repeat\(2/);
});
