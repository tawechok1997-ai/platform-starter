...
export default function SourceGameCategoryPage({ config }: { config: SourceGameCategoryConfig }) {
  ...
  const providers = useMemo(
    () => Array.from(new Map(config.providers.map((item) => [item.code, item])).values()),
    [config.providers],
  );

  const games = useMemo(() => Array.from(new Map(config.games.map((item) => {
    const tags = new Set<SourceGameFilterKey>(item.tags);
    if (item.isNew) tags.add('new');
    if (item.isHot) tags.add('hot');
    return [`${item.provider ?? 'none'}:${item.id}`, { ...item, tags: Array.from(tags) }];
  })).values()), [config.games]);
  ...
  const toggleProvider = (code: string) => {
    setProviderCode((current) => current === code ? null : code);
    setPreviewCode(null);
  };
        ...
        <div className={styles.layout}>
          <aside className={styles.filterPanel} aria-label={`ตัวกรอง${config.title}`}>
              ...
              {config.filters.map((filter) => {
                ...
                return (
                  <label key={filter.key} className={styles.filterOption}>
                    ...
                    <span className={styles.filterLabel}>{filter.label}</span><small>( {filter.count} )</small>
                  </label>
                );
              })}
            </div>
            ...
            {config.showProviderStrip ? <><div className={styles.filterSectionTitle}><strong>ค้นหาค่ายเกม</strong><span>เลือกอย่างใดอย่างหนึ่ง</span></div><div className={`${styles.providerGrid}${providers.length ? '' : ` ${styles.providerGridEmpty}`}`}>{providers.map((provider) => {
              const localCount = games.filter((game) => game.provider === provider.code).length;
              return <button key={provider.code} type="button" className={`${styles.providerButton}${providerCode === provider.code ? ` ${styles.providerActive}` : ''}`} onClick={() => toggleProvider(provider.code)} aria-pressed={providerCode === provider.code} aria-label={`${provider.name} ${localCount} เกม`} title={`${provider.name} (${localCount})`}><span aria-hidden="true" /><img src={provider.badge} alt={provider.name} onError={hideBrokenImage} /></button>;
            })}</div></> : null}

            <div className={styles.filterActions}><div className={styles.filterSummary}><span>พบเกมส์ที่คุณค้นหา</span><strong>{resultCount.toLocaleString('th-TH')} {config.resultUnit}</strong></div><button type="button" className={styles.clearButton} onClick={clearFilters} disabled={untouched}>ล้าง</button></div>
          </aside>
          ...
          <section className={styles.gameArea} aria-label={`รายการ${config.title}`}>
            ...
            })}</div> : <div className={styles.emptyState}><strong>ไม่พบเกมที่ตรงกับตัวกรอง</strong><button type="button" onClick={clearFilters}>ล้างตัวกรอง</button></div>}
          </section>
        </div>
      </section>
    </main>
