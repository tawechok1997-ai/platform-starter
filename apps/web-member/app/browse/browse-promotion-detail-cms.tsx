...
export function BrowsePromotionDetailCms({ id }: { id: string }) {
  ...
  return <main className="browse-page browse-page--detail">
    ...
    <article className="browse-detail">
      <ResponsiveDetailImage key={`${item.id}:${item.desktopImage}:${item.mobileImage}`} desktop={item.desktopImage} mobile={item.mobileImage} fallback={item.fallbackImage} alt={item.title} />
...
function ResponsiveDetailImage({ desktop, mobile, fallback, alt }: { desktop: string; mobile: string; fallback: string; alt: string }) {
  const desktopSource = desktop || fallback;
  const mobileSource = mobile || desktop || fallback;
  const [failedSource, setFailedSource] = useState('');
  const useFallback = Boolean(fallback && failedSource === desktopSource && desktopSource !== fallback);
  const renderedDesktop = useFallback ? fallback : desktopSource;
  const renderedMobile = useFallback ? fallback : mobileSource;

  if (!renderedDesktop) return <span className="browse-detail-image browse-missing-asset">MISSING ASSET</span>;
  return <picture><source media="(max-width: 640px)" srcSet={renderedMobile} /><img className="browse-detail-image" src={renderedDesktop} alt={alt} loading="eager" decoding="async" onError={() => { if (!useFallback && fallback) setFailedSource(desktopSource); }} /></picture>;
}
