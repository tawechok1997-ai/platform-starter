...
type PromoCard = { title: string; subtitle: string; href: string; aliases: string[]; fallback: string; iconKey: keyof SiteIconSettings };

const PROMO_CARDS: PromoCard[] = [
  { title: 'โปรโมชั่นพิเศษ', subtitle: 'โปรโมชั่นใหม่เฉพาะคุณ', href: '/promotions', aliases: ['promotion', 'promo', 'bonus', 'reward', 'โปรโมชั่น'], fallback: '✦', iconKey: 'promotion' },
  { title: 'กิจกรรม', subtitle: 'กิจกรรมตลอด 24 ชั่วโมง', href: '/promotions', aliases: ['activity', 'event', 'mission', 'กิจกรรม'], fallback: '♤', iconKey: 'bonus' },
  { title: 'ข่าวสาร', subtitle: 'ข่าวสารที่คุณไม่ควรพลาด', href: '/notifications', aliases: ['news', 'announcement', 'notice', 'ข่าว'], fallback: '◇', iconKey: 'notification' },
];
    ...
    <section className="desktop-home desktop-reference-home" aria-label="หน้าแรกเดสก์ท็อป">
      ...
      <div className="desktop-home__body">
        <div className="desktop-home__main reference-main-column">
          <section className="reference-promo-row" aria-label="โปรโมชั่น กิจกรรม และข่าวสาร">
            {PROMO_CARDS.map((card, index) => {
              ...
              return <a key={card.title} href={card.href} className={`reference-promo-card reference-promo-card--${index + 1}`}><AssetIcon asset={asset} configured={icons[card.iconKey]} fallback={card.fallback} className="reference-promo-icon" /><span><strong>{card.title}</strong><small>{card.subtitle}</small></span></a>;
            })}
          </section>

          <a href="/promotions" className="reference-tournament-cta"><AssetIcon asset={assets.tournament} configured={icons.promotion} fallback="V" className="reference-tournament-mark" /><span><small>TOURNAMENT</small><strong>เข้าร่วมชิงความเป็นที่ 1</strong></span><b>เข้าแข่งขัน ›</b></a>
          ...
          <section className="reference-number-section"><PanelHeading asset={assets.popular} configured={icons.games} fallback="🔥" title="Top 10 Popular Games" /><div className="reference-number-row">
          ...
          <section className="reference-compact-section"><PanelHeading asset={assets.online} configured={icons.games} fallback="⚡" title="Most Online Now" /><div className="reference-online-row">{online.map((game, index) => <a key={`${game.id}-${index}`} href="/login?next=%2Fgames" className="reference-online-card"><GameImage game={game} /><span><strong>{safeGameName(game)}</strong><small>♟ {(4274 - index * 437).toLocaleString()}</small></span></a>)}</div></section>

          <section className="reference-compact-section" id="live"><PanelHeading asset={assets.live} configured={icons.notification} fallback="🔴" title="Live Now!!" /><div className="reference-live-row">{MATCH_CARDS.map((match, index) => <article key={`${match.league}-${index}`} className="reference-live-card"><header><span>{match.league}</span><b>{match.time}</b></header><div><strong>{match.home}</strong><span>VS</span><strong>{match.away}</strong></div><footer><a href="/login">ดูบอลสด</a><a href="/login">เล่นเกมทันที</a></footer></article>)}</div></section>

          <section className="reference-compact-section"><PanelHeading asset={assets.classic} configured={icons.games} fallback="💧" title="Classic Games" /><div className="reference-classic-row">{classic.map((game) => <GameTile key={game.id} game={game} compact />)}</div></section>
        ...
        <aside className="desktop-home__sidebar reference-sidebar" aria-label="ข้อมูลรางวัลและอันดับ">
          ...
          <section className="reference-side-card reference-mini-games"><header><strong>Mini Game</strong></header><div><a href="/login"><AssetIcon asset={assets.wheel} configured={icons.bonus} fallback="วงล้อ" className="reference-mini-icon" /><span>วงล้อ</span></a><a href="/login"><AssetIcon asset={assets.card} configured={icons.promotion} fallback="ไพ่" className="reference-mini-icon" /><span>ทายการ์ด</span></a></div></section>
        </aside>
      </div>
    </section>
  );
...
function AssetIcon({ asset, configured, fallback, className }: { asset?: CmsAsset | undefined; configured?: string | undefined; fallback: string; className: string }) {
  ...
  return <span className={className} aria-hidden="true">{value ? (isImageValue(value) ? <img src={normalizeUrl(value)} alt="" onError={hideBrokenImage} /> : value) : fallback}</span>;
}
