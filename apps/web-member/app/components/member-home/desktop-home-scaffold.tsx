      ...
      <div className="desktop-home__body">
        <div className="desktop-home__main reference-main-column">
          ...
          <section className="reference-panel reference-tournament-board">
            ...
            <div className="reference-tournament-track" data-drag-scroll="true">{Array.from({ length: 8 }, (_, index) => <article key={index} className="reference-rank-card"><b>{index + 1}</b><strong>{maskName(index)}</strong><span>{20 - index * 2}</span><small>● ● ● ● ●</small></article>)}</div>
          ...
          <section className="reference-number-section"><PanelHeading asset={assets.popular} configured={icons.games} fallback="🔥" title="Top 10 Popular Games" /><div className="reference-number-row" data-drag-scroll="true">
          ...
          <section className="reference-compact-section"><PanelHeading asset={assets.online} configured={icons.games} fallback="⚡" title="Most Online Now" /><div className="reference-online-row" data-drag-scroll="true">{online.length ? online.map((game, index) => <a key={`${game.id}-${index}`} href="/login?next=%2Fgames" className="reference-online-card"><GameImage game={game} /><span><strong>{safeGameName(game)}</strong><small>♟ {(4274 - index * 437).toLocaleString()}</small></span></a>) : ARCHIVE_GAMES.slice(6, 11).map((game, index) => <ArchiveOnlineCard key={game.name} game={game} index={index} />)}</div></section>

          <section className="reference-compact-section" id="live"><PanelHeading asset={assets.live} configured={icons.notification} fallback="🔴" title="Live Now!!" /><div className="reference-live-row" data-drag-scroll="true">{MATCH_CARDS.map((match, index) => <article key={`${match.league}-${index}`} className="reference-live-card"><header><span>{match.league}</span><b>{match.time}</b></header><div><strong>{match.home}</strong><span>VS</span><strong>{match.away}</strong></div><footer><a href="/login">ดูบอลสด</a><a href="/login">เล่นเกมทันที</a></footer></article>)}</div></section>

          <section className="reference-compact-section"><PanelHeading asset={assets.classic} configured={icons.games} fallback="💧" title="Classic Games" /><div className="reference-classic-row" data-drag-scroll="true">{classic.length ? classic.map((game) => <GameTile key={game.id} game={game} compact />) : ARCHIVE_GAMES.slice(11, 17).map((game) => <ArchiveGameTile key={game.name} game={game} compact />)}</div></section>
          ...
          <section className="reference-provider-strip"><h2><AssetIcon asset={assets.partner} configured={icons.affiliate} fallback="" className="reference-heading-icon" />พันธมิตรของเรา</h2><div data-drag-scroll="true">{providers.map((provider, index) => <span key={`${provider.code}-${index}`} className="reference-provider-logo">{provider.logoUrl ? <img src={normalizeUrl(provider.logoUrl)} alt={provider.name || provider.code || 'Provider'} onError={hideBrokenImage} /> : <b>{provider.code || provider.name}</b>}</span>)}</div></section>
        </div>
