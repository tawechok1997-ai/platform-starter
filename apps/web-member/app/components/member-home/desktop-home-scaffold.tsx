...
const ARCHIVE_GAMES: ArchiveGame[] = [
  'Caishen Wins', 'Maya Golden City', 'Roma X', 'Cash Maker', 'Sweet Bonanza 1000', 'Roma Joker',
  'Thai Hi Lo', 'C-Play', 'Golden Empire', 'Coin Spinner', 'Bushido Ways', 'Fortune Gems',
  'Dragon Hatch', 'Cherry 777', 'Grand Spin', 'Wild Bandito', 'Alice Run', 'Penalty Series',
  'Fortune Rabbit', 'Sugar Rush', 'Lucky Gold', 'Royal Fishing',
].map((name, index) => ({ name, imageUrl: `/home-asset/game-${String(index + 1).padStart(2, '0')}.webp` }));
...
export function DesktopHomeScaffold({ content, icons, siteName, showPromotion, games, isGamesLoading, gamesMessage }: DesktopHomeProps) {
  ...
  const jackpotImage = assets.jackpot?.url || V47_ASSETS.jackpot;
    ...
    <section className="desktop-home desktop-reference-home" aria-label="หน้าแรกเดสก์ท็อป">
      ...
      <div className="desktop-home__body">
        <div className="desktop-home__main reference-main-column">
          ...
          <a href="/promotions" className="reference-tournament-cta"><AssetIcon asset={assets.tournament} configured={V47_ASSETS.tournament} fallback="V" className="reference-tournament-mark" /><span><small>TOURNAMENT</small><strong>เข้าร่วมชิงความเป็นที่ 1</strong></span><b>เข้าแข่งขัน ›</b></a>

          <section className="reference-panel reference-tournament-board">
            <PanelHeading asset={assets.tournament} configured={icons.promotion} fallback="🏆" title="ทัวร์นาเมนต์" />
            ...
            <div className="reference-tournament-track" data-drag-scroll>{Array.from({ length: 8 }, (_, index) => <article key={index} className="reference-rank-card"><b>{index + 1}</b><strong>{maskName(index)}</strong><span>{20 - index * 2}</span><small>● ● ● ● ●</small></article>)}</div>
          ...
          <section className="reference-panel reference-featured-section">
            <PanelHeading asset={assets.featured} configured={icons.games} fallback="★" title="เกมไฮไลต์" />
            {isGamesLoading ? <EmptyState label="กำลังโหลดเกมจาก API..." /> : featured.length ? <div className="reference-featured-grid"><GameTile game={featured[0]!} large /><div className="reference-featured-small-grid" data-drag-scroll>{featured.slice(1, 9).map((game) => <GameTile key={game.id} game={game} />)}</div></div> : <ArchiveFeaturedGames message={gamesMessage} />}
          </section>

          <section className="reference-number-section"><PanelHeading asset={assets.popular} configured={icons.games} fallback="🔥" title="Top 10 Popular Games" /><div className="reference-number-row" data-drag-scroll>
            {popular.length ? popular.map((game, index) => <a key={`${game.id}-${index}`} href="/login?next=%2Fgames" className="reference-number-card" title={safeGameName(game)}><GameImage game={game} /><span>{index + 1}</span><strong>{safeGameName(game)}</strong></a>) : ARCHIVE_GAMES.slice(0, 7).map((game, index) => <ArchiveNumberCard key={game.name} game={game} index={index} />)}
          </div></section>

          <section className="reference-compact-section"><PanelHeading asset={assets.online} configured={icons.games} fallback="⚡" title="Most Online Now" /><div className="reference-online-row" data-drag-scroll>{online.length ? online.map((game, index) => <a key={`${game.id}-${index}`} href="/login?next=%2Fgames" className="reference-online-card"><GameImage game={game} /><span><strong>{safeGameName(game)}</strong><small>♟ {(4274 - index * 437).toLocaleString()}</small></span></a>) : ARCHIVE_GAMES.slice(6, 11).map((game, index) => <ArchiveOnlineCard key={game.name} game={game} index={index} />)}</div></section>

          <section className="reference-compact-section" id="live"><PanelHeading asset={assets.live} configured={icons.notification} fallback="🔴" title="Live Now!!" /><div className="reference-live-row" data-drag-scroll>{MATCH_CARDS.map((match, index) => <article key={`${match.league}-${index}`} className="reference-live-card"><header><span>{match.league}</span><b>{match.time}</b></header><div><strong>{match.home}</strong><span>VS</span><strong>{match.away}</strong></div><footer><a href="/login">ดูบอลสด</a><a href="/login">เล่นเกมทันที</a></footer></article>)}</div></section>

          <section className="reference-compact-section"><PanelHeading asset={assets.classic} configured={icons.games} fallback="💧" title="Classic Games" /><div className="reference-classic-row" data-drag-scroll>{classic.length ? classic.map((game) => <GameTile key={game.id} game={game} compact />) : ARCHIVE_GAMES.slice(11, 17).map((game) => <ArchiveGameTile key={game.name} game={game} compact />)}</div></section>

          <section className="reference-guide" id="guide"><PanelHeading asset={assets.guide} configured={icons.support} fallback="?" title="Guide" />{(faqs.length ? faqs : fallbackFaqs()).map((faq) => <details key={faq.question}><summary>{faq.question}</summary><p>{faq.answer}</p></details>)}<a className="reference-guide-more" href="/guide">ดูทั้งหมด</a></section>

          <section className="reference-provider-strip"><h2><AssetIcon asset={assets.partner} configured={icons.affiliate} fallback="" className="reference-heading-icon" />พันธมิตรของเรา</h2><div data-drag-scroll>{providers.map((provider, index) => <span key={`${provider.code}-${index}`} className="reference-provider-logo">{provider.logoUrl ? <img src={normalizeUrl(provider.logoUrl)} alt={provider.name || provider.code || 'Provider'} onError={hideBrokenImage} /> : <b>{provider.code || provider.name}</b>}</span>)}</div></section>
        </div>

        <aside className="desktop-home__sidebar reference-sidebar" aria-label="ข้อมูลรางวัลและอันดับ">
          <section className="reference-side-card reference-jackpot"><header><AssetIcon asset={assets.jackpot} configured={V47_ASSETS.jackpot} fallback="●" className="reference-side-icon" /><strong>Jackpot</strong></header><div><img className="reference-jackpot-art" src={jackpotImage} alt="Jackpot" onError={hideBrokenImage} /><small>JACKPOTS</small><strong>195,574,797</strong><em>ลุ้นรับรางวัลใหญ่</em></div></section>
          <section className="reference-side-card reference-leaderboard"><header><span className="reference-side-title"><AssetIcon asset={assets.leaderboard} configured={icons.vip} fallback="🏆" className="reference-side-icon" /><strong>Leaderboard</strong></span><span>วันนี้</span></header>{Array.from({ length: 5 }, (_, index) => <div key={index}><b>{index + 1}</b><span><strong>{leaderName(index)}</strong><small>ชนะล่าสุด</small></span><em>›</em></div>)}</section>
          <section className="reference-side-card reference-mini-games"><header><strong>Mini Game</strong></header><div><a href="/login"><AssetIcon asset={assets.wheel} configured={V47_ASSETS.miniGameWheel} fallback="วงล้อ" className="reference-mini-icon" /><span>วงล้อ</span></a><a href="/login"><AssetIcon asset={assets.card} configured={V47_ASSETS.miniGameMission} fallback="ไพ่" className="reference-mini-icon" /><span>ทายการ์ด</span></a></div></section>
        </aside>
      </div>
    </section>
  );
...
function ArchiveFeaturedGames({ message }: { message: string }) { return <div className="reference-featured-grid reference-featured-grid--archive" aria-label={message || 'เกมตัวอย่างจากชุด asset'}><ArchiveGameTile game={ARCHIVE_GAMES[0]!} large /><div className="reference-featured-small-grid" data-drag-scroll>{ARCHIVE_GAMES.slice(1, 9).map((game) => <ArchiveGameTile key={game.name} game={game} />)}</div></div>; }
