...
const CASINO_PROVIDERS: readonly CasinoProviderCard[] = [
  {
    code: 'dg',
    name: 'Dream Gaming',
...
export default function MobileCasinoProviderPage() {
  ...
  return (
    <section
      ...
      data-mobile-casino-provider-page="true"
      data-category-launch-mode="provider"
      aria-labelledby="mobile-casino-provider-heading"
    >
      <h2 id="mobile-casino-provider-heading" className={styles.heading}>
        {copy.title} <span>({CASINO_PROVIDERS.length} {copy.providers})</span>
      </h2>
      ...
      <div className={styles.grid}>
        {CASINO_PROVIDERS.map((provider) => {
          ...
          return (
            <a
              key={provider.code}
              href={`/browse/games?category=casino&provider=${encodeURIComponent(provider.code)}`}
              className={className}
              data-provider-launch="true"
              data-provider-code={provider.code}
              data-game-category="casino"
