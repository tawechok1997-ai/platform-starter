...
export default function MobileSourceContent() {
  ...
  const onlineGames = runtime.onlineGames.length ? runtime.onlineGames : FALLBACK_POPULAR_GAMES.slice(0, 6);
  ...
  const tournamentIcon = runtime.icons.tournament || '/images/home/tournament.svg';
  const leaderboardIcon = runtime.icons.leaderboard || '/images/home/leader-board.svg';
    ...
    <div className={styles.root} data-mobile-section-owner="source-content" data-central-catalog={runtime.catalogConnected ? 'connected' : 'fallback'}>
                  ...
                  {padTournamentPlayers(item.players).map((entry, index) => (
                    <div key={`${item.id}-${index}`} className={styles.rankCard}>
                      <div className={styles.rankBadge}>
                        <MappedImage src={index < 3 ? '/images/predict/mobile/rankBadgeTop3.svg' : '/images/predict/mobile/rankBadgeOther.svg'} alt="" />
      ...
      {runtime.jackpot.enabled ? (
        <section className={styles.jackpotBanner} aria-label={runtime.jackpot.title || 'ยอดรางวัลรวม'}>
          <MappedImage src={runtime.jackpot.image} alt="" />
      ...
      {runtime.features.leaderboard ? (
        <section className={styles.leaderboardSection} aria-labelledby="mobile-leaderboard-heading">
          <SectionHeading id="mobile-leaderboard-heading" icon={leaderboardIcon} label={runtime.homeDataTitle ?? 'Leaderboard'} />
            ...
            {leaderboardRows.slice(0, 5).map((row, index) => (
              <div key={`${row.user}-${row.rank}-${index}`} className={styles.tableRow}>
                ...
                <span className={styles.tableGame}>
                  <MappedImage src={row.gameImage || leaderboardIcon} alt="" />
                  <span><b>{row.game}</b><small><MappedImage src={row.providerIcon || leaderboardIcon} alt="" />{row.provider || 'GAME'}</small></span>
                </span>
      ...
      {runtime.section.popular?.enabled !== false ? (
        <GameRailSection
          ...
          icon={runtime.section.popular?.icon || runtime.icons.popular || '/images/home/fire.svg'}
          games={popularGames}
        />
      ) : null}
      ...
      {runtime.section.online?.enabled !== false ? (
        <section className={styles.onlineSection} aria-labelledby="mobile-online-heading">
          <SectionHeading id="mobile-online-heading" icon={runtime.section.online?.icon || runtime.icons.online || '/images/home/mostonline.svg'} label={runtime.section.online?.title || 'Most Online Now'} />
      ...
      {runtime.section.live?.enabled !== false ? (
        <section className={styles.liveSection} aria-labelledby="mobile-live-heading">
          <SectionHeading id="mobile-live-heading" icon={runtime.section.live?.icon || runtime.icons.live || '/images/home/live.svg'} label={runtime.section.live?.title || 'Live Now!!'} />
      ...
      {runtime.section.classic?.enabled !== false ? (
        <GameRailSection
          ...
          icon={runtime.section.classic?.icon || runtime.icons.classic || '/images/home/fire.svg'}
          games={classicGames}
        />
      ) : null}
      ...
      {runtime.features.usageGuide ? (
        <section className={styles.guideSection} aria-labelledby="mobile-guide-heading">
          <SectionHeading id="mobile-guide-heading" icon="/images/home/faq.svg" label={runtime.guideTitle || 'Guide'} />
...
function mapAssetToLocal(src: string) {
  ...
  if (!normalized || normalized.startsWith(SOURCE_ROOT)) return normalized;
  if (normalized.startsWith('https://cdn.zabbet.com/')) {
    try {
      const pathname = new URL(normalized).pathname;
      const name = pathname.split('/').filter(Boolean).pop() ?? '';
      if (pathname.startsWith('/games/') && name) return `${SOURCE_ROOT}/games/${name}`;
      if (pathname.startsWith('/providers/')) return `${SOURCE_ROOT}${pathname}`;
      return `${SOURCE_ROOT}${pathname}`;
    } catch {
      return normalized;
    }
  }
  if (normalized.startsWith('/images/')) return `${SOURCE_ROOT}/${normalized.slice('/images/'.length)}`;
  return normalized;
}
