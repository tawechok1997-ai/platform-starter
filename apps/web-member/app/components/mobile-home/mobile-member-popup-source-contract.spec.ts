...
test('mobile and avatar surfaces mount one popup owner while search remains owner-free', () => {
  for (const source of [mobileLayout, avatarLayout]) {
    assert.match(source, /MobileMemberPopupRuntime/);
    assert.equal((source.match(/<MobileMemberPopupRuntime\s*\/>/g) ?? []).length, 1);
  }
  assert.doesNotMatch(searchLayout, /MobileMemberPopupRuntime/);
  assert.equal((runtime.match(/function SourcePopupShell/g) ?? []).length, 1);
});

test('source popup shell keeps the mobile geometry and chrome', () => {
