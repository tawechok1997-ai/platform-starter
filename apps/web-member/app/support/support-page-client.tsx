    ...
    <div className="member-support-container">
      ...
      <section className="member-support-tickets">
        <div className="member-support-heading">
          <div><p>ประวัติคำร้อง</p><h2>คำร้องของฉัน</h2><span>ระบบรีเฟรชคำร้องทุก 60 วินาที และการตอบกลับคำร้องที่ปิดแล้วจะเปิดเคสกลับให้อัตโนมัติ</span></div>
          <div className="member-support-ticket-filters">{(['ALL', 'OPEN', 'CLOSED'] as const).map((value) => <button type="button" key={value} aria-pressed={ticketFilter === value} onClick={() => setTicketFilter(value)}>{value === 'ALL' ? 'ทั้งหมด' : value === 'OPEN' ? 'กำลังดำเนินการ' : 'ปิดแล้ว'}</button>)}<button type="button" disabled={loading} onClick={() => void load()}>{loading ? 'กำลังโหลด...' : 'รีเฟรช'}</button></div>
        </div>
        {visibleTickets.map((item) => <SupportTicketCard
          key={item.id}
          item={item}
