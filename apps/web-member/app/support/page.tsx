...
const faqItems = [
  ['ฝากเงินใช้เวลานานแค่ไหน', 'หลังส่งสลิป ระบบจะแสดงสถานะรอตรวจสอบ และอัปเดตเมื่อเจ้าหน้าที่ดำเนินการ'],
...
export default function MemberSupportPage() {
  ...
  async function createTicket() {
    ...
    setBusy(true);
    const res = await memberApiFetch('/member/support-tickets', { method: 'POST', body: JSON.stringify({ category, subject, message }) });
    const data = await res.json().catch(() => null);
    setBusy(false);
    if (!res.ok) { setNotice(data?.message ?? 'สร้างคำร้องไม่สำเร็จ'); return; }
    setSubject(''); setMessage(''); setNotice('ส่งปัญหาแล้ว'); await load();
  }
    ...
    <div className="member-support-container">
      ...
      <section className="member-support-grid">
        <MemberCard className="member-support-form">
          ...
          <label><span>ประเภทปัญหา</span><select value={category} onChange={(event) => setCategory(event.target.value)}><option value="deposit">ฝาก</option><option value="withdraw">ถอนเงิน</option><option value="game">เกม</option><option value="account">บัญชี</option><option value="general">ทั่วไป</option></select></label>
          <label><span>หัวข้อ</span><input value={subject} onChange={(event) => setSubject(event.target.value)} placeholder="สรุปปัญหาแบบสั้น ๆ" /></label>
          <label><span>รายละเอียด</span><textarea value={message} onChange={(event) => setMessage(event.target.value)} placeholder="อธิบายสิ่งที่เกิดขึ้นและเวลาที่พบปัญหา" /></label>
          <MemberButton disabled={busy} onClick={() => void createTicket()}>{busy ? 'กำลังส่ง...' : 'ส่งปัญหา'}</MemberButton>
        </MemberCard>
        ...
        <MemberCard className="member-support-faq">
          ...
          <label><span>ค้นหา</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="เช่น ฝากเงิน ถอนเงิน หรือเข้าสู่ระบบ" /></label>
          <div className="member-support-accordion">{visibleFaq.map(([question, answer]) => <details key={question}><summary>{question}</summary><p>{answer}</p></details>)}</div>
        </MemberCard>
      </section>

      <section className="member-support-tickets"><div className="member-support-heading"><div><p>ประวัติคำร้อง</p><h2>คำร้องของฉัน</h2></div></div>
        {items.map((item) => <MemberCard key={item.id} className="member-support-ticket"><div className="member-support-ticket-head"><div><strong>{item.subject}</strong><p>{categoryLabel(item.category)} · {new Date(item.createdAt).toLocaleString('th-TH')}</p></div><span>{statusLabel(item.status)}</span></div><p>{item.message}</p><div className="member-support-thread">{(item.messages ?? []).slice(-5).map((msg, index) => <div key={`${item.id}-${index}`}><strong>{msg.by === 'admin' ? 'แอดมิน' : msg.by === 'member' ? 'คุณ' : 'ระบบ'}</strong><span>{msg.message}</span><small>{new Date(msg.createdAt).toLocaleString('th-TH')}</small></div>)}</div><label><span>ตอบกลับ</span><textarea value={replyText[item.id] ?? ''} onChange={(event) => setReplyText((current) => ({ ...current, [item.id]: event.target.value }))} placeholder="พิมพ์ข้อความเพิ่มเติม" /></label><MemberButton disabled={busy} tone="default" onClick={() => void reply(item.id)}>ตอบกลับ</MemberButton></MemberCard>)}
        {!notice && items.length === 0 && <MemberEmptyState title="ยังไม่มีคำร้อง" description="เมื่อสร้างคำร้อง ระบบจะแสดงสถานะและการตอบกลับไว้ที่นี่" />}
      </section>
    </div>
    <MemberBottomNav />
  </main>;
