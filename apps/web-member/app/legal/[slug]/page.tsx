...
const legalMap: Record<string, { key: string; title: string }> = {
  terms: { key: 'terms', title: 'เงื่อนไขการใช้งาน' },
...
export default function LegalPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main style={{ ...pageStyle, background: backgroundColor, color: textColor }}><section style={{ ...cardStyle, background: cardColor }}><a href="/" style={{ ...backStyle, color: primaryColor }}>← {siteName}</a><span style={{ ...eyebrowStyle, color: primaryColor }}>Legal</span><h1 style={titleStyle}>{item.title}</h1><div style={contentStyle}>{ready ? content : 'กำลังโหลด...'}</div></section></main>;
}
