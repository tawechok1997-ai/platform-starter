  ...
  return <main className="public-auth-page" style={authBrand.style} {...{ 'data-brand-code': String((settings.website as Record<string, unknown> | undefined)?.brand_code ?? 'default') }}>
    ...
    <section className="public-auth-shell" data-auth-mode="login">
      ...
      <form className="public-auth-card" onSubmit={onSubmit} noValidate>
        <div className="public-auth-card-topbar"><div className="public-auth-card__logo"><span>{logoUrl ? <MemberRuntimeImage src={logoUrl} alt={siteName} /> : brandMark}</span><strong style={{ maxWidth: 'min(170px, 44vw)' }}>{siteName}</strong></div><div aria-label="Language" className="public-auth-language"><button type="button" onClick={() => changeLocale('th')} aria-pressed={locale === 'th'} className="public-auth-language__button ui-button ui-button--secondary">ไทย</button><button type="button" onClick={() => changeLocale('en')} aria-pressed={locale === 'en'} className="public-auth-language__button ui-button ui-button--secondary">EN</button></div></div>
        <div className="public-auth-heading"><span className="public-auth-heading__eyebrow">{t.eyebrow}</span><h1>{t.title}</h1><p>{t.subtitle}</p></div>
        ...
        <label className="public-auth-field" htmlFor="login-secret">{t.password}<div className="public-auth-input-wrap"><input id="login-secret" className="public-auth-input ui-input" value={secret} onChange={(event) => { setSecret(event.target.value); if (errors.secret) clearFieldError('secret'); }} type={showSecret ? 'text' : 'password'} disabled={disabled} autoComplete="current-password" placeholder={t.passwordPlaceholder} aria-invalid={Boolean(errors.secret)} /><button type="button" onClick={() => setShowSecret((value) => !value)} className="public-auth-eye ui-button ui-button--secondary" disabled={disabled} aria-label={showSecret ? t.hidePassword : t.showPassword}>{showSecret ? t.hidePassword : t.showPassword}</button></div></label>
        ...
        <div className="public-auth-legal"><span>{t.secureConnection}</span><Link href="/legal/privacy">{t.privacy}</Link><Link href="/legal/terms">{t.terms}</Link></div>
      </form>
    </section>
  </main>;
}
