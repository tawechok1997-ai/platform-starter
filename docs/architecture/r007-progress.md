...
- Added shared `admin-audit.builder.ts` and migrated Admin member, support, Admin session, KYC review/download, watchlist, refresh-reuse, Admin two-factor, Admin login/challenge, and Admin step-up audit payloads to it.
...
- `PromotionsController` now routes read endpoints directly to the query service while mutation/settlement behavior remains in `PromotionsService`.
...
- Promotions/CMS mutation decomposition and audit migration beyond the completed query/mapper slice.
