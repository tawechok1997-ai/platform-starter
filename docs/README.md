...
1. [`GETTING_STARTED.md`](GETTING_STARTED.md) — 30-minute local setup and first-change path.
2. [`../AGENTS.md`](../AGENTS.md) — operating contract for implementation, verification, evidence and release.
3. [`AI_RULES.md`](AI_RULES.md) — rules for generated/assisted code.
4. [`PROJECT_RULES.md`](PROJECT_RULES.md) — architecture, finance, audit and separation rules.
5. [`master-project-worklist.md`](master-project-worklist.md) — project-wide backlog and the only source of truth for remaining checkbox counts.
6. [`operations/codebase-professionalization-audit.md`](operations/codebase-professionalization-audit.md) — current technical-debt, maintainability and handoff-readiness register.
7. [`operations/work-status-reporting.md`](operations/work-status-reporting.md) — canonical rules and commands for reporting remaining work.
8. [`operations/engineering-handoff.md`](operations/engineering-handoff.md) — transfer procedure and required evidence.
9. [`TROUBLESHOOTING.md`](TROUBLESHOOTING.md) — common build, startup, security and asset failures.
...
- [`architecture/admin-form-mutation-safety.md`](architecture/admin-form-mutation-safety.md) — Admin validation, confirmation, dirty-state, duplicate-submit, conflict and partial-failure contract.
- [`architecture/admin-data-view-contract.md`](architecture/admin-data-view-contract.md) — Admin queue/table URL state, sorting, pagination, masking, bulk action, mobile and export contract.
...
- [`architecture/finance-module-ownership.md`](architecture/finance-module-ownership.md), [`architecture/finance-mutation-ownership.md`](architecture/finance-mutation-ownership.md), [`architecture/risk-ownership.md`](architecture/risk-ownership.md), [`architecture/activity-projection-ownership.md`](architecture/activity-projection-ownership.md), [`architecture/member-query-ownership.md`](architecture/member-query-ownership.md) — current ownership boundaries.
...
- [`security/public-assets.md`](security/public-assets.md) — public executable asset quarantine and migration policy.
...
- [`operations/repository-maintenance-audit.md`](operations/repository-maintenance-audit.md), [`maintenance/repository-cleanup-policy.md`](maintenance/repository-cleanup-policy.md), [`operations/repository-size-migration.md`](operations/repository-size-migration.md) — cleanup inventory, safeguards and coordinated history migration.
...
## Current repository hardening gates

The Build workflow now enforces:

- read-only production builds
- generated catalog and Admin reference-asset drift checks
- public executable asset audit
- API bootstrap plus `/health` and `/version` commit identity
- clean tracked working tree after all builds

These checks are release contracts, not optional cleanup commands.
...
- Every documented `pnpm` command must exist in `package.json` or be explicitly shown as a direct `node` command.
