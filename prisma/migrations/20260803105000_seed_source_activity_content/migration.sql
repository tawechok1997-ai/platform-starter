-- Seed the Desktop source activity popup through the public CMS setting.
-- Existing banners, FAQs, popup settings and unrelated announcements are kept.
...
WITH source_content AS (
  SELECT jsonb_build_array(
    ...
    jsonb_build_object(
      ...
      'terms', jsonb_build_array('ยอด Turn และรางวัลเป็นไปตามประกาศของกิจกรรม')
    )
  ) AS announcements
), existing_setting AS (
  SELECT "value_json"
