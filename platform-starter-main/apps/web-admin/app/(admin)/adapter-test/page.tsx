...
export default function AdapterTestPage() {
  ...
  async function loadProviders() { setLoading(true); setMessage('กำลังโหลดค่าย...'); const res = await adminApiFetch('/admin/game-providers'); const data = await res.json().catch(() => null); setLoading(false); if (!res.ok) { setMessage(data?.message ?? 'โหลดค่ายไม่สำเร็จ'); return; } const items = data?.items ?? []; setProviders(items); setProviderId((current) => current || items[0]?.id || ''); setMessage(items.length ? '' : 'ยังไม่มีค่ายสำหรับทดสอบ'); }
  function changeMethod(next: MethodName) { setMethod(next); setPayloadText(defaultPayload[next]); setResult(null); }
  async function run(event?: FormEvent<HTMLFormElement>) { event?.preventDefault(); if (!providerId) { setMessage('เลือกค่ายก่อน'); return; } let payload: Record<string, unknown> = {}; try { payload = payloadText.trim() ? JSON.parse(payloadText) : {}; } catch { setMessage('ข้อมูลทดสอบต้องเป็น JSON ที่ถูกต้อง'); return; } setRunning(true); setMessage('กำลังทดสอบ API...'); setResult(null); const res = await adminApiFetch(`/admin/game-providers/${providerId}/adapter-test/${method}`, { method: 'POST', body: JSON.stringify(payload) }); const data = await res.json().catch(() => null); setRunning(false); setResult(data ?? null); if (!res.ok) { setMessage(data?.message ?? 'ทดสอบ API ไม่สำเร็จ'); return; } setMessage(data?.result?.ok === false ? 'ค่ายตอบว่าไม่สำเร็จ ดูรายละเอียดด้านล่าง' : 'ทดสอบ API สำเร็จ'); }
  return <AdminPage eyebrow="ขั้นสูง" title="ทดสอบ API ค่าย" description="ใช้ตอนเชื่อมค่ายหรือแก้ปัญหา เลือกสิ่งที่ต้องการทดสอบแล้วดูผลลัพธ์" actions={<AdminButton onClick={() => run()} disabled={running || !providerId}>{running ? 'กำลังทดสอบ...' : 'เริ่มทดสอบ'}</AdminButton>}>
    ...
    <AdminCard title={selectedMethod.label} description={selectedMethod.description} action={<AdminBadge tone={selectedMethod.risk === 'money' ? 'danger' : selectedMethod.risk === 'webhook' ? 'warning' : 'success'}>{riskLabel(selectedMethod.risk)}</AdminBadge>}>
      {selectedMethod.risk === 'money' && <AdminNotice>ทดสอบโยกเงินควรใช้ sandbox/simulator เท่านั้น อย่าเอาเงินจริงมาทดลองเหมือนกดปุ่มลิฟต์เล่น</AdminNotice>}
