...
export class AdapterTestController {
  ...
  @RequirePermission('game.providers.manage') @Post(':method') run(@Param('providerId') providerId: string, @Param('method') method: any, @Body() body: Record<string, unknown>) { return this.adapterTestService.run(providerId, method, body ?? {}); }
}
