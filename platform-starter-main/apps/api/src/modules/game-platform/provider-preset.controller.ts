...
export class ProviderPresetController {
  ...
  @RequirePermission('game.providers.manage') @Post('apply') applyPreset(@Body() body: any, @CurrentUser() user: any, @Req() req: any) { return this.providerPresetService.applyPreset(body, user, { ipAddress: req.ip, userAgent: req.headers?.['user-agent'] }); }
}
