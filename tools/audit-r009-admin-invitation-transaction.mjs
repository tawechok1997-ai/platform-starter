...
const checks = [
  [
    'controller imports invitation command service',
    controller.includes(
      "import { AdminInvitationAdminService } from './admin-invitation-admin.service';",
    ),
  ],
  [
    'controller injects invitation command service',
    controller.includes('private readonly invitationCommands: AdminInvitationAdminService'),
  ],
  ...
  [
    'create audit appears before transaction closes',
    createAuditAction >= createTransactionStart && createAuditAction < createTransactionEnd,
  ],
  ...
  [
    'reissue audit appears before transaction closes',
    reissueAuditAction >= reissueTransactionStart && reissueAuditAction < reissueTransactionEnd,
  ],
  [
    'no post-transaction create audit helper call',
    !createMethod.includes("await this.audit(actorAdminId, 'CREATE_ADMIN_INVITATION'"),
  ],
  [
    'no post-transaction reissue audit helper call',
    !reissueMethod.includes("await this.audit(actorAdminId, 'REISSUE_ADMIN_INVITATION'"),
  ],
];
