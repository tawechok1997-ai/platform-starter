...
const files = {
  ...
  authOverlayLock: 'apps/web-member/app/lib/member-document-overlay-lock.ts',
...
for (const behavior of ["event.key === 'Escape'", 'event.origin !== window.location.origin']) requireText('authOverlay', behavior, `auth overlay behavior ${behavior}`);
for (const behavior of ["body.style.overflow = 'hidden'", "body.style.position = 'fixed'", 'window.scrollTo(scrollX, scrollY)']) requireText('authOverlayLock', behavior, `auth overlay lock behavior ${behavior}`);
